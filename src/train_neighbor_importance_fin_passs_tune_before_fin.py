...
def eigengap_analysis_k_1(feature_matrix, max_clusters=25, normalize=True, plot_path=None):
    ...
    if plot_path:
        ...
        y_vals = eigenvals[valid_k_range]
...
def eigengap_analysis(feature_matrix, max_clusters=25, normalize=True, plot_path=None):
    ...
    gaps = np.diff(eigenvals[1:max_clusters + 1])  # skip 0th eigenvalue

    # Only consider k > 5
    valid_k_range = np.arange(2, max_clusters + 1)  # corresponds to k for eigenval indices 1:max_clusters
    filtered_indices = np.where(valid_k_range > 5)[0]

    if len(filtered_indices) == 0:
        raise ValueError("No valid k > 5 found in the given range.")

    filtered_gaps = gaps[filtered_indices]
    best_k_index = filtered_indices[np.argmax(filtered_gaps)]
    best_k = valid_k_range[best_k_index]

    # Plotting (optional)
    if plot_path:
        x_vals = valid_k_range
        #y_vals = eigenvals[1:max_clusters + 1]
        y_vals = eigenvals[valid_k_range]
...
def train(args):
    ...
    assert summary_bio_features.shape[1] == 64, f"Expected 64 summary features, got {summary_bio_features.shape[1]}"
    #best_k, eigenvals = eigengap_analysis(summary_bio_features, max_clusters=25, normalize=True, plot_path=plot_eigengap_path)
