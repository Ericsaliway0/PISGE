...
def plot_pcg_cancer_genes(
    ...
    ax.set_ylabel("Percent of Predicted Cancer Genes", fontsize=14)
...
def process_confirmed_genes(
    ...
    plot_confirmed_neighbor_relevance(
        ...
        scores=relevance_np.sum(axis=1), 
...
def plot_neighbor_relevance_tsn20(
    neighbor_scores,
    gene_name,
    node_id_to_name,
    output_path,
    ...
    """
    Plots top 10 neighbors with relevance score > 0.05, normalized and colored by cluster.

    Args:
        neighbor_scores (dict): {node_id: relevance_score}
        gene_name (str): Confirmed gene name
        node_id_to_name (dict): {node_id: gene_name}
        output_path (str): Save path for the plot
        cluster_labels (dict or np.ndarray, optional): {node_id: cluster_id}
        total_clusters (int): Total number of clusters (for color assignment)
        add_legend (bool): Whether to include cluster legend
    """
    # Filter neighbors with score > 0.05
    filtered = {k: v for k, v in neighbor_scores.items() if v > 0.05}
    if len(filtered) < 10:
        print(f"⚠️ Skipping plot for {gene_name} (only {len(filtered)} neighbors > 0.05)")
        return

    # Get top 10 by raw relevance
    top_10 = dict(sorted(filtered.items(), key=lambda x: -x[1])[:10])
    node_ids = list(top_10.keys())
    raw_scores = list(top_10.values())
    ...
    # Prepare DataFrame
    df = pd.DataFrame({
        "neighbor": [node_id_to_name.get(nid, f"Node {nid}") for nid in node_ids],
        "relevance": norm_scores,
        "cluster": [int(cluster_labels[nid]) if cluster_labels is not None else -1 for nid in node_ids]
    })

    # Plot
    plt.figure(figsize=(2.5, 2.5))
    sns.set_style("white")
    bar_colors = [get_cluster_color(cid, total_clusters) if cid >= 0 else "gray" for cid in df["cluster"]]
    ax = sns.barplot(data=df, x="neighbor", y="relevance", palette=bar_colors, width=0.8)
    ...
    if add_legend and cluster_labels is not None:
        ...
        legend_handles = [
            mpatches.Patch(color=get_cluster_color(cid, total_clusters), label=f"Cluster {cid}")
            for cid in unique_clusters if cid >= 0
        ]
...
def train(args):
    ...
    sns.heatmap(contingency, annot=True, fmt='d', cmap='BuPu', cbar=False)
    ...
    contingency_plot_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_contingency_matrix_epo{args.num_epochs}.png")
    plt.tight_layout()
    ...
    for cluster_type in ['bio', 'topo']:
        ...
        ax.bar(
            [f"{cluster_type.capitalize()}_{i}" for i in enrichment_results[cluster_type].keys()],
            term_counts,
            label=cluster_type
        )

    # Labeling
    ax.set_ylabel("Number of enriched terms", fontsize=12)
    ax.set_title("Functional Coherence: Enriched Term Counts per Cluster", fontsize=14)
...
def save_confirmed_predictions_to_csv(confirmed_predictions, output_dir, model_type, net_type, num_epochs):
    ...
    - model_type, net_type, num_epochs: For naming the output file.
    """
    confirmed_predictions_csv_path = os.path.join(output_dir, f'{model_type}_{net_type}_confirmed_genes_epo{num_epochs}.csv')
