...
def apply_spectral_biclustering(
    ...
    save_path, save_cluster_labels_path, save_total_genes_per_cluster_path,
...
def apply_full_spectral_biclustering_bio_no_progress_bar(
    ...
    if topk_node_indices is None:
        raise ValueError("You must provide `topk_node_indices` corresponding to the rows in summary_bio_features.")
    cluster_labels_tensor[topk_node_indices[high_saliency_indices]] = torch.tensor(row_labels, dtype=torch.long)
    graph.ndata['cluster_bio_summary'] = cluster_labels_tensor
...
def train(args):
    ...
    ## bio
    '''bio_feats_trained = trained_embeddings[:, :1024]
    bio_embeddings_trained = bio_feats_trained.cpu().numpy()
    bio_embeddings_trained = bio_embeddings_trained[topk_node_indices_tensor]
    summary_bio_features_trained, column_names_trained = extract_summary_features_np_bio(bio_embeddings_trained, omics_types, cancer_names)  # shape [num_nodes, 64]    
    
    bio_feats_initial = graph.ndata['feat'][:, :1024]  # biological features
    bio_embeddings_initial = bio_feats_initial.cpu().numpy()
    bio_embeddings_initial = bio_embeddings_initial[topk_node_indices_tensor]
    summary_bio_features_initial, column_names_initial = extract_summary_features_np_bio(bio_embeddings_initial, omics_types, cancer_names)  # shape [num_nodes, 64]


    ## topo
    topo_feats_trained = trained_embeddings[:, 1024:]
    topo_embeddings_trained = topo_feats_trained.cpu().numpy()
    topo_embeddings_trained = topo_embeddings_trained[topk_node_indices_tensor]
    summary_topo_features_trained = extract_summary_features_np_topo(topo_embeddings_trained)  # shape [num_nodes, 64]    
    
    topo_feats_initial = graph.ndata['feat'][:, 1024:]  # topological features
    topo_embeddings_initial = topo_feats_initial.cpu().numpy()
    topo_embeddings_initial = topo_embeddings_initial[topk_node_indices_tensor]
    summary_topo_features_initial = extract_summary_features_np_topo(topo_embeddings_initial)  # shape [num_nodes, 64]
    '''
    ...
    relevance_scores_topk = relevance_scores[topk_node_indices_tensor]
    relevance_scores_topk_bio = relevance_scores_bio[topk_node_indices_tensor]
    output_path_summary_bio = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_summary_bio_epo{args.num_epochs}.csv')
    relevance_matrix_bio, column_names = extract_summary_features_np_bio(relevance_scores_topk_bio.detach().cpu().numpy(), omics_types, cancer_names, output_path_summary_bio)
    '''
    ##################relevance_matrix = 
    # print(summary_bio_topk.shape)
    # relevance_matrix_bio = summary_bio_topk
    ...
    assert relevance_matrix_topo.shape[1] == 64, f"Expected 64 summary features, got {relevance_matrix_topo.shape[1]}"
    '''
    #best_k, eigenvals = eigengap_analysis(relevance_matrix_bio, max_clusters=20, normalize=True, plot_path=plot_eigengap_path)



    ##top5_k_row, top5_k_col = run_eigengap_row_col(relevance_matrix_bio, output_dir, args)
        
    # eigengap_results_bio = eigengap_analysis(
    #     relevance_matrix=relevance_matrix_bio,
    #     args=args,
    #     output_dir=output_dir,
    #     name_prefix='bio'
    # )

    # eigengap_results_topo = eigengap_analysis(
    #     relevance_matrix=relevance_matrix_topo,
    #     args=args,
    #     output_dir=output_dir,
    #     name_prefix='topo',
    #     delta_row=1.0,  # Customize if needed
    #     delta_col=1.0
    # )
    ...
    # graph_bio, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio_initial(
    #     graph=graph,
    #     summary_bio_features_initial=relevance_matrix_bio,  # shape (1000, 64)
    #     #summary_bio_features_initial=summary_bio_features_initial,  # shape (1000, 64)
    #     node_names=node_names,
    #     predicted_cancer_genes=predicted_cancer_genes,
    #     n_clusters=best_k,
    #     save_path=bio_graph_path,
    ...
    #     output_path_heatmap=bio_output_path_heatmap,
    #     topk_node_indices=top_gene_indices,  # <-- add this!
    #     feature_names=column_names_initial
    # )
     
    # graph_bio, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio_trained(
    #     graph=graph,
    #     summary_bio_features_trained=relevance_matrix_bio,  # shape (1000, 64)
    #     #summary_bio_features_trained=summary_bio_features_trained,  # shape (1000, 64)
    #     node_names=node_names,
    #     predicted_cancer_genes=predicted_cancer_genes,
    #     n_clusters=best_k,
    #     save_path=bio_graph_path,
    ...
    #     output_path_heatmap=bio_output_path_heatmap,
    #     topk_node_indices=top_gene_indices,  # <-- add this!
    #     feature_names=column_names_trained
    # )
    ...
    # === TOPO BICLUSTERING ===
    # graph_topo, cluster_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo_initial(
    #     graph=graph,
    #     #topo_embeddings=topo_feats,
    #     summary_topo_features_initial=summary_topo_features_initial,
    #     node_names=node_names,
    #     #feature_names=None,
    #     predicted_cancer_genes=node_names_topk,
    #     n_clusters=best_k,
    ...
    # graph_topo, cluster_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo_trained(
    #     graph=graph,
    #     #topo_embeddings=topo_feats,
    #     summary_topo_features_trained=summary_topo_features_trained,
    #     node_names=node_names,
    #     #feature_names=None,
    #     predicted_cancer_genes=node_names_topk,
    #     n_clusters=best_k,
