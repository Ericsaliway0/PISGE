...
def apply_full_spectral_biclustering_topo_ori(graph, topo_embeddings, node_names,
...
def apply_full_spectral_biclustering_bio_before_modulaize(graph, summary_bio_features, node_names,
                                         predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap):
...
def plot_bio_heatmap_unsort_ori(summary_bio_features, row_labels, col_labels, predicted_indices, output_path):
    from matplotlib.colors import LinearSegmentedColormap
    cancer_names = [
        'Bladder', 'Breast', 'Cervix', 'Colon', 'Esophagus', 'HeadNeck', 'Kidney', 'KidneyPap',
        'Liver', 'LungAd', 'LungSc', 'Prostate', 'Rectum', 'Stomach', 'Thyroid', 'Uterus'
    ...
    # === Heatmap layout
    fig = plt.figure(figsize=(18, 12))
    gs = fig.add_gridspec(nrows=1, ncols=3, width_ratios=[1, 0.2, 13.8])
    ax_cluster = fig.add_subplot(gs[0, 0])
    ax_gap = fig.add_subplot(gs[0, 1])
    ax_heatmap = fig.add_subplot(gs[0, 2])
    ...
    ax_heatmap.set_xticklabels(bio_feat_names_64, rotation=90, fontsize=7)
    ...
    ax_heatmap.set_xlabel("Summary Bio Features (Grouped by Omics)", fontsize=12)
    fig.tight_layout()
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
def plot_bio_heatmap_unsort(summary_bio_features, row_labels, col_labels, predicted_indices, output_path):
...
def plot_topo_heatmap_unsort_no_cluster_bar(features_matrix,
                                    row_labels,
                                    col_labels,
                                    output_path,
                                    title="Heatmap of Topo Features Sorted by Spectral Biclusters",
                                    vmin=-2,
                                    vmax=2,
                                    figsize=(16, 12),
                                    cmap_name="bluish_gray_gradient"):
    """
    Plot a topo biclustering heatmap, sorting rows and columns by their cluster labels.

    Args:
        features_matrix (ndarray): (n_genes x n_features) topo embeddings matrix.
        row_labels (ndarray): Row cluster assignments from spectral biclustering.
        col_labels (ndarray): Column cluster assignments from spectral biclustering.
        output_path (str): Where to save the heatmap.
        title (str): Title of the heatmap.
        vmin (float): Minimum heatmap color scale.
        vmax (float): Maximum heatmap color scale.
        figsize (tuple): Size of the figure.
        cmap_name (str): Custom colormap name.
    """
    ...
    sorted_matrix = features_matrix[np.argsort(row_labels), :][:, np.argsort(col_labels)]

    # Plot
    plt.figure(figsize=figsize)
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=vmin, vmax=vmax)
    plt.title(title)
    plt.xlabel("Features (Clustered)")
    plt.ylabel("Genes (Clustered)")
    plt.tight_layout()
    ...
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"✅ Topo bicluster heatmap saved to {output_path}")

def plot_topo_heatmap_unsort(
    args,
    relevance_scores,
    cluster_labels,
    output_path,
    ...
    def extract_summary_features_np_topo(x):
        return np.array(x)

    relevance_scores = extract_summary_features_np_topo(relevance_scores)

    # Step 1: Sort rows by cluster label, then by total relevance within cluster
    sorted_indices = np.argsort(cluster_labels)
    relevance_scores = relevance_scores[sorted_indices]
    cluster_labels = cluster_labels[sorted_indices]
    ...
    # Step 2: Sort columns if col_labels provided
    if col_labels is not None:
        sorted_order = np.argsort(col_labels)
        sorted_scores = sorted_scores[:, sorted_order]
        feature_names = [f"Topo {i+1}" for i in sorted_order]
    else:
        feature_names = [f"Topo {i+1}" for i in range(relevance_scores.shape[1])]

    # Step 3: Prepare figure layout
    fig = plt.figure(figsize=(17, 18))
    gs = fig.add_gridspec(nrows=2, ncols=2, height_ratios=[17, 2], width_ratios=[0.25, 13.7], hspace=0.3, wspace=0.00)
    ax_cluster = fig.add_subplot(gs[0, 0])
    ax_heatmap = fig.add_subplot(gs[0, 1])
    ...
    # Step 4: Row cluster color stripe
    CLUSTER_COLORS = {
        0: '#1f77b4', 1: '#ff7f0e', 2: '#2ca02c',
        3: '#d62728', 4: '#9467bd', 5: '#8c564b',
        6: '#e377c2', 7: '#7f7f7f', 8: '#bcbd22', 9: '#17becf'
    }
    cluster_colors = [to_rgb(CLUSTER_COLORS[label]) for label in sorted_clusters]
    cluster_colors_array = np.array(cluster_colors).reshape(-1, 1, 3)
    ax_cluster.imshow(cluster_colors_array, aspect='auto')
    ax_cluster.axis("off")

    cluster_counts = Counter(sorted_clusters)
    start_idx = 0
    for cluster_id in sorted(cluster_counts):
        count = cluster_counts[cluster_id]
        mid_idx = start_idx + count // 2
    ...
    bluish_gray_gradient = LinearSegmentedColormap.from_list("bluish_gray_gradient", ["#F0F3F4", "#85929e"])
    vmax = np.percentile(sorted_scores, 99)
    sns.heatmap(sorted_scores, cmap=bluish_gray_gradient, vmin=0, vmax=vmax,
                ax=ax_heatmap, cbar=False)
    ax_heatmap.set_title("Heatmap of Summary Topo Features Sorted by Spectral Biclusters", fontsize=18)
    ax_heatmap.set_yticks([])
    xticks = np.arange(len(feature_names)) + 0.5
    ax_heatmap.set_xticks(xticks)
    ax_heatmap.set_xticklabels(feature_names, rotation=90, fontsize=14)
    ...
    return pd.DataFrame(sorted_scores, index=gene_names, columns=feature_names) if gene_names is not None else pd.DataFrame(sorted_scores, columns=feature_names)
...
def apply_full_spectral_biclustering_topo(graph, summary_topo_features, node_names,
    ...
    topo_features = StandardScaler().fit_transform(summary_topo_features)

    # === Spectral Biclustering
    bicluster = SpectralBiclustering(n_clusters=n_clusters, method='log', random_state=42)
    bicluster.fit(topo_features)
    row_labels = bicluster.row_labels_
    col_labels = bicluster.column_labels_
    graph.ndata['cluster_topo'] = torch.tensor(row_labels, dtype=torch.long)
    ...
    # === Plotting
    plot_tsne(topo_features, row_labels, predicted_indices, n_clusters, output_path_genes_clusters)
    plot_topo_heatmap_unsort(topo_features, row_labels, col_labels, output_path_heatmap)

    return graph, row_labels, col_labels, total_genes_per_cluster, pred_counts

def plot_topo_heatmap_unsort(topo_features, row_labels, col_labels, output_path):
    import os
    import numpy as np
    import matplotlib.pyplot as plt
    import seaborn as sns
    ...
    topk = 1000
    predicted_indices = np.argsort(row_labels)[:topk]
    topo_topk = topo_features[predicted_indices]
    row_labels_topk = row_labels[predicted_indices]
    sorted_indices = np.argsort(row_labels_topk)
    sorted_matrix = topo_topk[sorted_indices, :]
    sorted_labels = row_labels_topk[sorted_indices]
    ...
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=-2, vmax=2,
                ax=ax_heatmap, cbar=False)
    ax_heatmap.set_title("Heatmap of Topological Features Sorted by Spectral Biclusters", fontsize=18)
    ax_heatmap.set_yticks([])

    xticks = np.arange(topo_features.shape[1]) + 0.5
    ax_heatmap.set_xticks(xticks[::32])  # Show fewer ticks for clarity
    ax_heatmap.set_xticklabels([f"T{i}" for i in xticks[::32]], rotation=90, fontsize=10)

    # Legend placeholder (no omics, but we still format like the bio plot)
    handles = [Patch(color=CLUSTER_COLORS[i], label=f"Cluster {i}") for i in sorted(cluster_counts)]
    ax_legend.legend(handles=handles, loc='center', ncol=6, fontsize=14, frameon=False)
    ...
    fig.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"✅ Topo heatmap saved to {output_path}")
...
def plot_topo_heatmap_unsort(topo_features, row_labels, col_labels, output_path):
    ...
    # Generate formatted feature names: 01, 02, ..., 64
    topo_feat_names = [f"{i:02d}" for i in range(1, topo_features.shape[1] + 1)]

    # Use topk rows based on lowest cluster label sorting
    topk = 1000
    predicted_indices = np.argsort(row_labels)[:topk]
    topo_topk = topo_features[predicted_indices]
    row_labels_topk = row_labels[predicted_indices]
    ...
    xticks = np.arange(len(topo_feat_names)) + 0.5
    ax_heatmap.set_xticks(xticks)
    ax_heatmap.set_xticklabels(topo_feat_names, rotation=90, fontsize=14)
