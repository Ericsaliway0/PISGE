...
def compute_relevance_scores_no_normalize(model, graph, features, node_indices=None, use_abs=True):
    """
...
def compute_relevance_scores_bio(
    model,
    graph,
    features,
    node_indices,
...
def plot_confirmed_neighbors_bio(
    ...
    for gene in confirmed_genes:
        ...
        plot_path = os.path.join(
            "results/gene_prediction/neighbor_feature_contributions_bio/",
            f"{args.model_type}_{args.net_type}_{gene}_bio_neighbor_relevance_epo{args.num_epochs}.png"
        )
...
def plot_confirmed_neighbors_topo(
    ...
    for gene in confirmed_genes:
        ...
        plot_path = os.path.join(
            "results/gene_prediction/neighbor_feature_contributions_topo/",
            f"{args.model_type}_{args.net_type}_{gene}_topo_neighbor_relevance_epo{args.num_epochs}.png"
        )
...
def train(args):
    ...
    # 🔹 Extract 64D summary feature vectors
    summary_bio = extract_summary_features_np_bio(relevance_scores_bio)
    summary_topo = extract_summary_features_np_topo(relevance_scores_topo)
    
    '''summary_bio_topk = summary_bio[top_gene_indices]
    summary_topo_topk = summary_topo[top_gene_indices]'''

    # 🔹 Feature names
    bio_feature_names = [f"{o}_{c:02d}" for o in ["mf", "cnv", "mut", "exp"] for c in range(16)]
    topo_feature_names = [f"TOPO_{i+1:02d}" for i in range(summary_topo.shape[1])]
    ...
    groupwise_saliency = {
        "bio": relevance_scores[:, :1024].cpu().numpy(),
        "topo": relevance_scores[:, 1024:].cpu().numpy()
    }

    bio_means = np.mean(groupwise_saliency["bio"], axis=0)
    topo_means = np.mean(groupwise_saliency["topo"], axis=0)
    ...
    plot_bio_topo_saliency(
        ...
        save_path=os.path.join(
            save_dir,
            f"{args.model_type}_{args.net_type}_bio_topo_comparison_lineplot_{args.num_epochs}.png"
        )
    )
...
def train_bio_topo(args):
    ...
    # 🔹 Output paths
    confirmed_file = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_BIO_confirmed_genes_epo{args.num_epochs}.txt")
    confirmed_file = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_TOPO_confirmed_genes_epo{args.num_epochs}.txt")
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            # Extract 64D summary feature vectors for top-k only
            summary_topo_topk = extract_summary_features_np_bio(relevance_scores_topk.detach().cpu().numpy())
    ...
    total_genes_per_cluster = {i: np.sum(cluster_labels_bio == i) for i in range(best_k)}
    ...
    plot_pcg_cancer_genes(
        clusters=total_genes_per_cluster.keys(), predicted_cancer_genes_count=bio_pred_counts,
        total_genes_per_cluster=total_genes_per_cluster, node_names=node_names,
        cluster_labels=cluster_labels_bio, output_path=output_path_predicted
    )
    ...
    plot_kcg_cancer_genes(
        ...
        total_genes_per_cluster=total_genes_per_cluster, node_names=node_names,
        cluster_labels=cluster_labels_bio, output_path=output_path_kcg
    )
