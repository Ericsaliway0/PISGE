...
def plot_top_gene_ridge_from_all_omics(
    ...
    topk_feature = cancer_feature[:len(node_names_topk)]  # assuming matching order
    row_labels = apply_full_spectral_biclustering_cancer(topk_feature, n_clusters=best_k)
    ...
    df = pd.DataFrame(topk_feature)
    df["Gene"] = node_names_topk
...
def plot_bio_biclustering_heatmap(
    ...
    gs = fig.add_gridspec(nrows=30, ncols=50)
    #ax_bar    = fig.add_subplot(gs[0, 2:45])       
    ax_bar = fig.add_subplot(gs[0:10, 2:45])
    '''ax        = fig.add_subplot(gs[1:13, 2:45])
    ax_curve  = fig.add_subplot(gs[1:13, 45:48], sharey=ax)
    ax_cbar   = fig.add_subplot(gs[5:9, 49])'''
    ax = fig.add_subplot(gs[10:29, 2:45])        
    ax_curve = fig.add_subplot(gs[10:29, 45:48], sharey=ax)

    ax_cbar = fig.add_subplot(gs[12:18, 49])    
    ...
    # Skip normalization — just use raw means and clip alpha
    mean_vals = np.array([omics_means[om] for om in omics_order])

    # Rescale mean values into alpha range safely (clip if needed)
    for i, omics in enumerate(omics_order):
        raw_alpha = omics_means[omics]
        alpha = np.clip(raw_alpha, 0.3, 1.0)  # clip to valid alpha range
        ...
        ax.bar(
            ...
            edgecolor='black',
            linewidth=1,
            alpha=alpha  # use clipped raw mean
        )
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            for cancer_type in cancer_types:
                ...
                cluster_dict = make_cluster_dict(row_labels, node_names_topk)
            ...
            target_idx = node_names_topk.index(target_gene_name)
