...
def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names, predicted_cancer_genes, n_clusters,
    ...
    threshold = np.percentile(row_sums, 20)  # or try 30%
    ...
    normalized = scaler.fit_transform(saliency_matrix)

    # Keep values above +1 std deviation
    saliency_matrix = np.where(normalized > 1.0, normalized, 0)'''
