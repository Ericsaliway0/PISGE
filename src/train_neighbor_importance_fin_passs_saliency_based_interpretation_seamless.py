...
def cluster_and_visualize_predicted_genes(graph, predicted_cancer_genes, node_names, 
    ...
    kmeans = KMeans(n_clusters=num_clusters, random_state=55, n_init=12)
    ...
    tsne = TSNE(n_components=2, perplexity=30, random_state=55)
...
def apply_full_spectral_biclustering_cancer(cancer_feature, n_clusters):
    ...
    bicluster = SpectralBiclustering(n_clusters=n_clusters, method='log', random_state=55)
...
def plot_tsne(features, row_labels, predicted_indices, n_clusters, output_path):
    tsne = TSNE(n_components=2, perplexity=min(30, len(features) - 1), random_state=55)
...
def apply_full_spectral_biclustering_topo(graph, summary_topo_features, node_names,
    ...
    # === Spectral Biclustering
    bicluster = SpectralBiclustering(n_clusters=n_clusters, method='log', random_state=99)
...
def plot_salience_matrix(
    relevance_scores,
    output_path,
    omics_order=['cna', 'ge', 'meth', 'mf'],
    cancer_names=[
        'Bladder', 'Breast', 'Cervix', 'Colon', 'Esophagus', 'HeadNeck',
        'Kidney', 'KidneyPap', 'Liver', 'LungAd', 'LungSc', 'Prostate',
        'Rectum', 'Stomach', 'Thyroid', 'Uterus'
    ],
    gene_names=None
):
    """
    Plot salience matrix heatmap (genes x features) with no clustering.

    Parameters:
    - relevance_scores: np.ndarray of shape (num_genes, 64)
    - output_path: str, where to save the figure
    - omics_order: list of 4 omics types
    - cancer_names: list of 16 cancer types
    - gene_names: list of gene names (optional)
    """
    num_genes=1000
    num_features = 64
    assert num_features == len(omics_order) * len(cancer_names), \
        "Expected 64 features (4 omics × 16 cancers)."
    ...
    relevance_scores = np.squeeze(relevance_scores)  # ensure 2D
    scores = np.clip(relevance_scores, a_min=0, a_max=np.percentile(relevance_scores, 99))


    plt.figure(figsize=(24, min(0.5 * num_genes, 30)))
    ax = sns.heatmap(
        scores,
        cmap="YlGnBu",
        xticklabels=feature_names,
        yticklabels=gene_names if gene_names else False,
        cbar_kws={"label": "Salience Score"}
    )

    ax.set_xlabel("Omics Feature (Omic:Cancer)", fontsize=14)
    ax.set_ylabel("Genes", fontsize=14)
    ax.tick_params(axis='x', labelrotation=90, labelsize=10)
    ax.tick_params(axis='y', labelsize=10)

    plt.title("Gene–Omics–Cancer Salience Matrix", fontsize=16)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
...
def train_with_relevance_tracking_(args, model, graph, embeddings, labels, train_mask, output_dir):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    graph = graph.to(device)
    features = embeddings.to(device)
    ...
    # Plotting
    plt.figure(figsize=(8, 5))
    plt.plot(bio_scores, label='Bio', color='#1f77b4')
    plt.plot(topo_scores, label='Topo', color='#ff7f0e')
    plt.xlabel('Epoch')
    plt.ylabel('Mean Relevance Score')
    plt.title('Relevance Scores per Feature Group')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(heatmap_path_relevance_scores))
    plt.close()
...
def plot_bio_biclustering_heatmap_unsort(
    ...
    # Optional: color y-axis by cluster (if given)
...
def plot_topo_biclustering_heatmap_clusters_unsort(
    ...
    feature_names = [f"Topo-{i}" for i in range(relevance_scores.shape[1])]
...
def train(args):
    ...
    for epoch in tqdm(range(args.num_epochs), desc="Training Progress", unit="epoch"):
        ...
        model.train()
        logits = model(graph, features).view(-1)
        
        if train_mask.sum() == 0:
            raise ValueError("train_mask contains no True values.")
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        heatmap_path_salience_matrix = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_salience_matrix_{tag}_epo{args.num_epochs}.png"
        )
        ...
        if tag == "bio":
            ...
            relevance_scores = extract_summary_features_np_bio(relevance_scores_bio)
    
            '''plot_salience_matrix(
                relevance_scores=relevance_scores,  
                output_path=heatmap_path_salience_matrix,
                gene_names=node_names_topk          
            )'''
            
            '''cluster_dict = cluster_to_genes 
