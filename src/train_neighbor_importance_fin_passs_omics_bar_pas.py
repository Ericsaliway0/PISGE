...
def apply_full_spectral_biclustering_bio_x(graph, bio_embeddings, node_names,
    ...
    print(f"Running FULL Spectral Biclustering with {n_clusters} clusters...")
    ...
    # Assign row clusters to graph
    row_labels = bicluster.row_labels_
    col_labels = bicluster.column_labels_
    graph.ndata['cluster_bio'] = torch.tensor(row_labels, dtype=torch.long)
    print("Spectral Biclustering complete. Assigned both row and column clusters.")
    ...
    plt.xlabel("t-SNE Dimension 1", fontsize=14)
    plt.ylabel("t-SNE Dimension 2", fontsize=14)
    plt.title("t-SNE of Spectral Biclustering (Bio) Clusters")
    plt.savefig(output_path_genes_clusters, bbox_inches="tight")
    plt.close()

    print(f"✅ t-SNE visualization saved to {output_path_genes_clusters}")

    # ↓ Sorted bicluster matrix heatmap
    ...
    plt.title("Heatmap of Bio Features Sorted by Spectral Biclusters")
    ...
    plt.close()
    print(f"✅ Bicluster heatmap saved to {output_path_heatmap}")
...
def plot_topo_biclustering_heatmap(
    ...
    ax_bar.set_xlim(0, len(feature_means))
    ax_bar.set_ylim(0, 0.03)
    ax_bar.set_yticks([0.005, 0.015, 0.25])
    ax_bar.set_yticklabels(["0.0", "0.5", "1.0"], fontsize=12)
    ax_bar.tick_params(axis='y', direction='out', length=3, pad=2)
    ax_bar.set_xticks([])
...
def plot_clusterwise_sorted_heatmap_bio_sorted(
    ...
    legend_handles.append(Patch(color='#7b241c', label='LRP Sum', alpha=0.8))
    ax_legend.legend(
...
def plot_pcg_cancer_genes(
    ...
    # Labels and formatting
    ax.set_ylabel("Percent of PCGs", fontsize=16)
    plt.xlabel("")  # No xlabel here
    ax.set_xticks(clusters)
    ax.set_xticklabels(clusters, fontsize=16)
...
def plot_enriched_term_counts(enrichment_results, output_path, model_type, net_type, num_epochs, bio_color='#1f77b4', topo_color='#ff7f0e'):
    ...
    # Labeling
    ax.set_ylabel("Number of enriched terms", fontsize=24)
    ax.set_title("Functional Coherence: Enriched Term Counts per Cluster", fontsize=24)

    # Set custom x-tick labels and colors
    ax.set_xticks(range(len(xtick_labels)))
    ax.set_xticklabels(xtick_labels, rotation=90, fontsize=24)
...
def plot_contingency_matrix(cluster_labels_bio_topk, cluster_labels_topo_topk, ari_score, nmi_score, output_dir, args):
    ...
    plt.figure(figsize=(8, 8))
    sns.heatmap(contingency, annot=True, fmt='d', cmap='BuPu', cbar=False, annot_kws={"fontsize": 24})

    plt.title(f"Contingency Matrix: Bio vs Topo\n(ARI={ari_score:.2f}, NMI={nmi_score:.2f})", fontsize=24)
    plt.xlabel("Topo clusters", fontsize=24)
    plt.ylabel("Bio clusters", fontsize=24)
