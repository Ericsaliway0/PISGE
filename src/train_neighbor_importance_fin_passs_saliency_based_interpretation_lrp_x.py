...
def compute_lrp_scores(model, graph, features, node_indices=None, use_abs=True):
    """
    Computes LRP-based relevance scores for selected nodes.

    Args:
        model: Trained GIN model
        graph: DGL graph
        features: Input node features (Tensor or ndarray)
        node_indices: Optional indices of target nodes (if None, auto-select based on prediction)
        use_abs: Whether to return absolute relevance scores (for visualization)
    ...
    # Forward pass
    with torch.no_grad():
        x1 = model.gin1(graph, features)
        a1 = F.relu(x1)
        x2 = model.gin2(graph, a1)
        a2 = F.relu(x2)
        logits = model.mlp(a2)
        probs = torch.sigmoid(logits.squeeze())
    ...
    for idx in node_indices:
        ...
        x2 = model.gin2(graph, a1)
        a2 = F.relu(x2)
        output = model.mlp(a2)[idx]

        # Step 2: Backprop from target output
        output.backward(retain_graph=True)

        # Step 3: Compute relevance approximation as input gradient * input
        lrp = features.grad * features
        if use_abs:
            lrp = lrp.abs()
        relevance_scores[idx] = lrp[idx].detach()

    return relevance_scores

def compute_lrp_relevance_scores_(model, graph, features, node_indices=None, use_abs=True):
    """
    Computes LRP relevance scores for selected nodes using sigmoid probabilities.

    Args:
        model: Trained GNN model (must support relevance propagation)
        graph: DGL graph
        features: Input node features
        node_indices: List/Tensor of node indices to compute relevance for. If None, auto-select using probs > 0.5
        use_abs: Whether to use absolute relevance values (recommended for visualization)
    ...
    # Auto-select nodes if none provided
    if node_indices is None:
        node_indices = torch.nonzero(probs > 0.5, as_tuple=False).squeeze()
        if node_indices.ndim == 0:
            node_indices = node_indices.unsqueeze(0)
    ...
    for idx in node_indices:
        # Zero all previous relevance
        model.zero_grad()

        # Re-run forward for fresh state (some LRP methods require it)
        logits = model(graph, features)

        # Get scalar output for node idx
        output = torch.sigmoid(logits[idx].squeeze())

        # LRP: Backpropagate relevance
        R = lrp(model, graph, features, target_idx=idx)

        # Optionally take absolute value
        relevance = R[idx]
        relevance_scores[idx] = relevance.abs() if use_abs else relevance
...
def lrp_linear_eps(input, weight, output, relevance, epsilon=1e-6):
    z = input @ weight.T  # [N, out_features]
    z += epsilon * ((z >= 0).float() * 2 - 1)  # Stabilize denominator
    s = relevance / z  # [N, out_features]
    c = s @ weight     # [N, in_features]
    return input * c   # Element-wise product

def lrp_linear_zplus(input, weight, output, relevance):
...
def compute_lrp_relevance_scores(model, graph, features, node_indices=None, method='epsilon', use_abs=True):
    model.eval()
    ...
    relevance = torch.zeros_like(features)

    for idx in node_indices:
        R = torch.zeros_like(probs)
        R[idx] = probs[idx]
        model(graph, features)  # Run forward to fill cache
        r_input = model.relprop(R, method=method)
        if use_abs:
            r_input = r_input.abs()
        relevance[idx] = r_input[idx]
...
def train(args):
    ...
    #relevance_scores = compute_lrp_relevance_scores(model, graph, features)
    relevance_scores = compute_lrp_relevance_scores(model, graph, features, method='zplus')
    ...
    relevance_scores_topk_topo = relevance_scores_bio[topk_node_indices_tensor]
    summary_topo_topk = extract_summary_features_np_bio(relevance_scores_topk_topo.detach().cpu().numpy())
    ...
    bio_scores = saliency_matrix_bio#groupwise_saliency["bio"]
    topo_scores = saliency_matrix_topo#groupwise_saliency["topo"]
    save_dir = os.path.join(output_dir, "gnnexplainer/")
    os.makedirs(save_dir, exist_ok=True)
