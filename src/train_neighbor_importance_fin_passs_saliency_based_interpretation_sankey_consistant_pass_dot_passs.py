...
def plot_dynamic_sankey_bio_clusterlevel_not_fixed_colors(
    ...
    sankey_fig.update_layout(
        title_text=f"Cluster → Neighbor Cluster (Bio) - {args.model_type}_{args.net_type}",
        font_size=10,
        width=1200,
        height=800
    )
...
def plot_dynamic_sankey_topo_clusterlevel(
    ...
    sankey_fig.update_layout(
        title_text=f"Cluster → Neighbor Cluster (Topo) - {args.model_type}_{args.net_type}",
        font_size=10,
        width=1200,
        height=800
    )
...
def plot_top_gene_ridge_from_all_omics(
    ...
    for cluster in clusters:
        ...
        ax.set_title(f"Feature Profile Curves — Cluster {cluster} ({cancer_target})", fontsize=18)
...
def plot_survival_by_cluster(
    ...
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"✅ Kaplan–Meier survival plot saved to: {output_path}")
...
def draw_dot_plot_with_ratio_no_bar(terms, cancer_type, source, top_n=20):
    ...
    ax.set_xlabel("Gene Ratio (log scale)", fontsize=14)
    ax.set_ylabel("Enriched Pathway", fontsize=14)
    ax.set_title(f"Dot Plot — {cancer_type.upper()} ({source})", fontsize=16)
    ax.set_xscale("log")
    ...
    plt.savefig(out_path, dpi=300)
    plt.close()
    print(f"✅ Saved dot plot with gene ratio and LogP: {out_path}")
...
def draw_dot_plot_with_ratio_(terms, cancer_type, source, top_n=20):
    ...
    scatter.set_title(f"Dot Plot (Ratio) — {cancer_type.upper()} ({source})", fontsize=18)
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    ax.set_yticklabels(df["Term"], fontsize=14)
    ax.set_xlabel("Gene Ratio (log scale)", fontsize=16)
    ax.set_title(f"{cancer_type.upper()} — {source}", fontsize=16, pad=12)
    ax.set_xscale("log")
    ...
    ax_logp.set_xlabel("-log10(p-value)", fontsize=14, labelpad=10)

    # === Legend for Clusters ===
    handles = [mpatches.Patch(color=CLUSTER_COLORS[c], label=f"Cluster {c}") for c in sorted(df["Cluster"].unique())]
    ax.legend(handles=handles, bbox_to_anchor=(1.05, 1), loc="upper left", title="Cluster", fontsize=10, title_fontsize=12)
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    # 🔢 Select top N by LogP and reverse so top term appears at top
    ...
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"✅ Saved dot plot with right-side bar: {out_path}")
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    ax1.set_title(f"Dot Plot — {cancer_type.upper()} ({source})", fontsize=18)
    ...
    ax2.barh(y=y_positions, width=df["LogP"], color=df["Color"], alpha=0.6, edgecolor="black")
    for i, logp in enumerate(df["LogP"]):
        ax2.text(logp + 0.2, i, f"{logp:.2f}", va='center', fontsize=14)
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    ax1.set_xlabel("Gene Ratio (log scale)", fontsize=14)
    ax1.set_ylabel("Enriched Pathway", fontsize=14)
    ax1.set_xscale("log")
...
def plot_dot_plots_for_all_sources(terms_by_source, cancer_type, top_n=20):
    ...
    for source, terms in terms_by_source.items():
        print(f"📊 Plotting dot+bar for: {source}")
...
def plot_confirmed_neighbors_bio(
    ...
    # ✨ Filter predicted genes that exist in relevance_scores
    valid_genes = [g for g in predicted_cancer_genes if g in topk_name_to_index]
    
    # ✨ Sort by total saliency (descending)
    ...
    # ✅ Save one CSV for all gene-neighbor relationships
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    for gene in combined_genes:
        ...
        for neighbor_idx, score in neighbor_scores.items():
            ...
            # ✅ Skip if cluster is -1 (unassigned)
    ...
    with open(neighbors_csv_path, "w", newline="") as csvfile:
        ...
        writer.writerows(all_neighbor_rows)
    print(f"\U0001F4C4 Saved gene-neighbor relevance data to: {neighbors_csv_path}")
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    ax1.set_xscale("log")
    ax1.set_xlabel("Gene Ratio (log scale)", fontsize=24)
    ax1.set_ylabel("Enriched Pathway", fontsize=24)
    ax1.set_title(f"{cancer_type.upper()} — {source}\n-log10(p-value)", fontsize=26)
    ax1.tick_params(axis='both', labelsize=24)
    ax1.grid(axis="x", linestyle="--", alpha=0.5)
    ax1.set_yticklabels(ax1.get_yticklabels(), fontsize=24)
    ...
    ax2.set_xlabel("Sum of -log10(p-value)", fontsize=24)
    ax2.tick_params(axis='x', labelsize=20)
    ax2.tick_params(axis='y', labelleft=False)
    ax2.grid(axis='x', linestyle='--', alpha=0.5)
    ax2.set_yticklabels(ax2.get_yticklabels(), fontsize=30)
    ...
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"✅ Saved stacked bar + dot plot aligned: {out_path}")
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    # Select top N by LogP (keep all entries for selected terms)
    top_terms = df.sort_values("LogP", ascending=False)["Term"].unique()[:top_n]
    df = df[df["Term"].isin(top_terms)]

    # Fix term order (bottom to top)
    term_order = list(reversed(top_terms))
    df["Term"] = pd.Categorical(df["Term"], categories=term_order, ordered=False)
    ...
    ax1.set_xlabel("Gene Ratio (log scale)", fontsize=24)
    ax1.set_ylabel("Enriched Pathway", fontsize=24)
    # ax1.set_title(f"{cancer_type.upper()} — {source}", fontsize=22)
    ax1.set_title(f"{cancer_type.upper()} — {source}\n-log10(p-value)", fontsize=26)
    ax1.tick_params(axis='both', labelsize=24)
    ax1.grid(axis="x", linestyle="--", alpha=0.5)
    ax1.set_yticklabels(ax1.get_yticklabels(), fontsize=30)  # ✅ Pathway names
    ...
    for term in term_order:
        # clusters = df[df["Term"] == term].sort_values("Cluster")
        clusters = df[df["Term"] == term].sort_values("LogP", ascending=True)
        y = term
        x_start = 0
        ...
        for _, row in clusters.iterrows():
            ...
            # --- Parameters ---
            LABEL_PAD = 0.8                   # Distance from bar edge for outside labels
            MIN_BAR_FOR_INSIDE_LABEL = 1.2    # Threshold to decide inside vs outside

            # Label color: black only for segment with largest p-value
            label_color = "black" if cluster == min_cluster else "white"

            if logp >= MIN_BAR_FOR_INSIDE_LABEL:
                # Inside label, centered in bar
                ax2.text(
                    x=x_start + logp / 2,
                    y=y,
                    s=f"{logp:.2f}",
                ...
                # Outside label to the LEFT of bar
                ax2.text(
                    x=x_start - LABEL_PAD,
                    y=y,
                    s=f"{logp:.2f}",
    ...
    # Automatically extend x-axis left if needed
    xmin = min(0, df.groupby("Term")["LogP"].sum().min() - 1.0)
    xmax = df.groupby("Term")["LogP"].sum().max() + 1.0
    ax2.set_xlim(left=xmin, right=xmax)
    ...
    # ax2.set_xlim(left=min(-1, -0.5 * min_logp))

    ax2.set_xlabel("Sum of -log10(p-value)", fontsize=24)
    ax2.tick_params(axis='x', labelsize=20)
    ax2.tick_params(axis='y', labelleft=False)  # Hide y-axis tick labels (shared with ax1)
    ax2.grid(axis='x', linestyle='--', alpha=0.5)
    ax2.set_yticklabels(ax2.get_yticklabels(), fontsize=30)


    plt.subplots_adjust(left=0.3, wspace=0.05, top=0.98, bottom=0.02)  # reduce outer margin
    # Adjust y-limits to tightly fit bars without large top/bottom gaps
    ax1.set_ylim(-0.5, len(term_order) - 0.5)
    ax2.set_ylim(-0.5, len(term_order) - 0.5)
...
def draw_dot_plot_with_ratio_xlabel_pvalue_all_black_pass(terms, cancer_type, source, top_n=20):
    if not terms:
        print(f"No enrichment terms for {cancer_type.upper()} ({source})")
        return
    ...
    ax1.tick_params(axis='both', labelsize=24)
    ax1.grid(axis="x", linestyle="--", alpha=0.5)
    ax1.set_yticklabels(ax1.get_yticklabels(), fontsize=24)  # ✅ Pathway names
    ...
    for term in term_order:
        ...
        for _, row in clusters.iterrows():
            ...
            # --- Parameters ---
            LABEL_PAD = 0.5                   # Distance from bar edge for outside labels
            MIN_BAR_FOR_INSIDE_LABEL = 1.2    # Threshold to decide inside vs outside
    ...
    ax2.tick_params(axis='y', labelleft=False)  # Hide y-axis tick labels (shared with ax1)
    ax2.grid(axis='x', linestyle='--', alpha=0.5)
    ax2.set_xticklabels(ax2.get_xticklabels(), fontsize=24)  # ✅ Pathway names
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    selected_known_genes = ["BRCA2", "EGFR"]
    # selected_known_genes = ["EGFR", "SRC", "BRCA2", "HDAC2"]
    selected_novel_genes = ["EIF2B3", "TOM1L2", "SYNCRIP", "GEMIN5", "WDR24", "ZNF598", "TAF8", "SEC61A1", "FUBP1", "TRIP13"]
...
def train(args):
    ...
    for cluster_type, row_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            tag = "bio"  # or whatever tag you're using
            ...
            for cancer_type in cancer_list:
                ...
                for source in sources:
                    print(f"📊 Plotting {source} enrichment for {cancer_type}...")
