...
def plot_bio_topo_saliency_cuberoot(bio_scores, topo_scores, title="", save_path=None):
    ...
    plt.legend(handles=legend_handles, loc='upper left', frameon=False, fontsize=16)
...
def compute_relevance_scores_norm(
    ...
    Returns:
        dict: node_idx → dict of {group_name: relevance_tensor}
    """
    model.eval()
    features = features.clone().detach().requires_grad_(True)
    ...
    for node_idx in tqdm(node_indices, desc="Computing saliency relevance"):
        model.zero_grad()
        node_score = output[node_idx].squeeze()
        node_score.backward(retain_graph=True)
        ...
        # Always extract 'bio' as the first 1024 features
        group_scores['bio'] = saliency[:1024]

        # If other feature groups are defined, extract them too
        if feature_groups:
            for group_name, (start, end) in feature_groups.items():
                group_scores[group_name] = saliency[start:end]
...
def best_balanced_biclustering_(data, n_clusters, min_cluster_size=10, max_attempts=200):
    """
    ...
    for attempt in range(max_attempts, desc="Searching for balanced biclustering"):
...
def best_balanced_biclustering(data, n_clusters, n_trials=20, min_cluster_size=10):
    ...
    if best_model is None:
        raise RuntimeError(f"No valid clustering found with min_cluster_size ≥ {min_cluster_size} after {n_trials} trials.")
...
def compute_relevance_scores_norm(
    ...
    node_indices,
    normalize=True,
    feature_slice=(0, 1024)  # Optional slice for e.g., bio features
):
    """
    Computes normalized saliency-based relevance scores for selected nodes.

    Args:
        model: Trained GNN model
        graph: DGL graph
        features: Input node features (torch.Tensor)
        node_indices: List/Tensor of node indices to compute relevance for
        normalize: Whether to normalize per-node saliency
        feature_slice: Tuple (start, end) to select a slice of features (e.g. (0, 1024))

    Returns:
        relevance_scores: Tensor of shape [num_nodes, num_features], 0s for non-selected nodes
    """
    model.eval()
    features = features.clone().detach().requires_grad_(True)
    num_nodes, num_features = features.shape

    relevance_scores = torch.zeros((num_nodes, feature_slice[1] - feature_slice[0]), dtype=features.dtype)

    output = model(graph, features)

    for idx in tqdm(node_indices, desc="Computing normalized saliency"):
        model.zero_grad()
        if features.grad is not None:
            features.grad.zero_()

        node_score = output[idx].squeeze()
        node_score.backward(retain_graph=True)

        saliency = features.grad[idx].detach().abs()  # (F,)
        if normalize:
            saliency = saliency / (saliency.sum() + 1e-9)

        relevance_scores[idx] = saliency[feature_slice[0]:feature_slice[1]]
    ...
    for trial in tqdm(range(args.num_trials), desc="🔁 Trials", unit="trial"):
        ...
        if len(top_gene_indices) == 1000:
            ...
            for trial in range(args.num_trials):  # e.g., args.num_trials = 5
                ...
                # )
                # relevance_bio = compute_relevance_scores_norm(
                #     model, graph, features, node_indices=some_nodes, normalize=True, feature_slice=(0, 1024)
                # )

                relevance_scores_bio = compute_relevance_scores_norm(
                    model, graph, features, node_indices=gene_indices, normalize=True, feature_slice=(0, 1024)
                )
                
                relevance_scores_topo = compute_relevance_scores_norm(
                    model, graph, features, node_indices=gene_indices, normalize=True, feature_slice=(1024, 2048)
                )
                ...
                top_two = [item[0] for item in sorted_omics[:2]]
