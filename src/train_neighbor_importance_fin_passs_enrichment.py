...
def plot_confirmed_neighbor_relevance(
    ...
    """
    Plots top-10 neighbor relevance scores for confirmed genes.
    Also saves a CSV containing neighbor name, index, relevance score, and optional features/scores.
    Mode can be 'bio' or 'topo'.
    """
    import os
    ...
    for gene in confirmed_genes:
        ...
        csv_path = os.path.join(
            "results/gene_prediction/neighbor_feature_contributions/",
            f"{args.model_type}_{args.net_type}_{gene}_{mode}_neighbor_relevance_epo{args.num_epochs}.csv"
        )
        os.makedirs(os.path.dirname(csv_path), exist_ok=True)

        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Neighbor Index", "Neighbor Name", "Relevance Score", f"{mode.capitalize()} Score"])  # Add more fields if needed

            for nid, rel_score in top_neighbors.items():
                neighbor_name = node_id_to_name[nid]
                neighbor_score = scores[nid]
                writer.writerow([nid, neighbor_name, rel_score, neighbor_score])

        print(f"📄 Saved neighbor relevance CSV for {gene} → {csv_path}")
...
def train(args):
    ...
    # === Normalize column-wise for contrast ===
    norm_data = heatmap_data.copy()
    norm_data = norm_data / norm_data.max().replace(0, 1)

    # === Custom warm colormap for each column ===
    colormaps = {
        #'bio': get_cmap('YlGnBu'),  # warmer greenish tone
        'bio': get_cmap('Blues'),
        'topo': get_cmap('YlOrRd'),   # warmer reddish tone 
    }
    '''colormaps = {
        'bio': get_cmap('Blues'),
        'topo': get_cmap('BuPu'),
    ...
    for i, col in enumerate(norm_data.columns):
        cmap_type = 'bio' if col.startswith('bio') else 'topo'
        cmap = colormaps[cmap_type]
        norm_vals = norm_data[col].values
        colors[:, i, :] = cmap(norm_vals)

    # === Plot ===
    ##fig, ax = plt.subplots(figsize=(0.35 * len(norm_data.columns), 0.3 * len(norm_data)))
    fig, ax = plt.subplots(figsize=(12, 0.25 * len(heatmap_data)))

    # Narrower patches = smaller width figure relative to height
    ...
    ax.set_yticklabels(norm_data.index, fontsize=13)

    # Color x-tick labels
    for xtick, col in zip(ax.get_xticklabels(), norm_data.columns):
        xtick.set_color('darkgreen' if col.startswith('bio') else 'darkred')

    # Titles and axis labels
    ax.set_title("Top Enriched Pathways per Cluster (p < 0.05)", fontsize=15, pad=12)
    ax.set_xlabel("Cluster", fontsize=14)
    ax.set_ylabel("Pathway (Source)", fontsize=14)
    ...
    fig.legend(
        ...
        ncol=2,
        frameon=False,
        bbox_to_anchor=(0.5, -0.05)  # Moves the legend below the plot
    )
