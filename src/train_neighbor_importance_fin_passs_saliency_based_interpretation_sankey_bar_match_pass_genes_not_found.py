    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            ...
            for neighbor_idx, neighbor_score in neighbors.items():
                ...
                # Always create a distinct node for neighbor role to preserve flow direction
                neighbor_unique_label = f"{neighbor_name}"
                if neighbor_unique_label not in label_to_idx:
                    label_to_idx[neighbor_unique_label] = len(all_labels)
                    all_labels.append(neighbor_unique_label)
                    saliency = relevance_scores[neighbor_idx].sum().item()
                    color = CLUSTER_COLORS.get(neighbor_cluster, "#000000")
                    all_colors.append(color)
                    font_sizes.append(16 if saliency > 0.5 else 10)

                neighbor_node_idx = label_to_idx[neighbor_unique_label]
