...
def save_topk_neighbors_with_clusters_and_weights(
    graph,
    node_names_topk,
    topk_node_indices,
    row_labels,
    ...
    """
    Save all top-k node neighbors that are also top-k, with their clusters and edge weights,
    to a CSV in the specified output directory.
    """
    os.makedirs(output_dir, exist_ok=True)
    save_path = os.path.join(output_dir, f"topk_neighbors_clusters_with_weights_{cancer_type}.csv")

    topk_node_set = set(topk_node_indices)
    node_idx_to_name = {idx: name for idx, name in zip(topk_node_indices, node_names_topk)}
    cluster_assignments = {idx: row_labels[i] for i, idx in enumerate(topk_node_indices)}

    edge_weights = graph.edata['weight'] if 'weight' in graph.edata else torch.ones(graph.num_edges())
    ...
    edge_weight_dict = {
        (int(s), int(d)): float(w) for s, d, w in zip(src_nodes, dst_nodes, edge_weights)
    }
    ...
    for i, node_idx in enumerate(topk_node_indices):
        ...
        cluster_id = row_labels[i]

        neighbors = list(set(graph.successors(node_idx).tolist() + graph.predecessors(node_idx).tolist()))

        for nbr in neighbors:
            if nbr in topk_node_set:
                neighbor_name = node_idx_to_name[nbr]
                neighbor_cluster = cluster_assignments[nbr]

                # Find edge weight (handle undirected)
                weight = edge_weight_dict.get((node_idx, nbr), edge_weight_dict.get((nbr, node_idx), None))

                rows.append({
                    "Gene": gene_name,
                    "Gene Cluster": cluster_id,
                    "Neighbor": neighbor_name,
                    "Neighbor Cluster": neighbor_cluster,
                    "Edge Weight": weight
                })

    df = pd.DataFrame(rows)
    df.to_csv(save_path, index=False)
    print(f"✅ Saved top-k neighbor cluster info with weights to: {save_path}")

def get_topk_gene_neighbors(graph, topk_node_indices, undirected=True):
    """
    Returns a dictionary mapping each top-k node index to a list of its neighbor indices,
    optionally treating the graph as undirected.
    """
    neighbors_dict = {}

    for node_idx in topk_node_indices:
        if undirected:
            nbrs = list(set(graph.successors(node_idx).tolist() + graph.predecessors(node_idx).tolist()))
        else:
            nbrs = graph.successors(node_idx).tolist()

        neighbors_dict[node_idx] = nbrs

    return neighbors_dict

def save_topk_gene_neighbors(graph, node_names, node_names_topk, topk_node_indices, row_labels, output_dir):
    """
    Save neighbors (also in top-k) of each top-k gene with cluster info and edge weights.
    """
    edge_weights = graph.edata['weight'].numpy() if 'weight' in graph.edata else None
    src, dst = graph.edges()
    ...
    rows = []
    for i in topk_node_indices:
        gene_name = idx_to_name[i]
        gene_cluster = row_labels[i] if i < len(row_labels) else -1

        # Neighbor indices (undirected)
        neighbors = set(dst[src == i]).union(set(src[dst == i]))
        neighbors &= topk_set
        ...
        for j in neighbors:
            ...
            neighbor_name = idx_to_name[j]
            neighbor_cluster = row_labels[j] if j < len(row_labels) else -1

            weight = 1.0
            if edge_weights is not None:
                for k in range(len(src)):
                    if (src[k] == i and dst[k] == j) or (src[k] == j and dst[k] == i):
                        weight = edge_weights[k]
                        break

            rows.append([
                gene_name, gene_cluster,
                neighbor_name, neighbor_cluster,
                weight
            ])

    os.makedirs(output_dir, exist_ok=True)
    save_path = os.path.join(output_dir, "topk_gene_neighbors_with_clusters.csv")
    df = pd.DataFrame(rows, columns=[
        "Gene", "Gene_Cluster", "Neighbor", "Neighbor_Cluster", "Edge_Weight"
    ])
    df.to_csv(save_path, index=False)
    ...
    # === ✅ Step 6: Save neighbor relationships for top-k genes with cluster info and edge weights
    print("🔎 Saving top-k gene neighbors with cluster info and edge weights...")
    ...
    topk_set = set(topk_node_indices)
    idx_to_name = {i: name for i, name in enumerate(node_names_topk)}
    global_idx_to_name = {i: name for i, name in enumerate(graph.ndata['_ID'].tolist())} if '_ID' in graph.ndata else idx_to_name

    rows = []
    for i in topk_node_indices:
        gene_name = global_idx_to_name.get(i, f"node_{i}")
        gene_cluster = row_labels[i] if i < len(row_labels) else -1
        ...
        for j in neighbors:
            if i == j:
                continue
            neighbor_name = global_idx_to_name.get(j, f"node_{j}")
            neighbor_cluster = row_labels[j] if j < len(row_labels) else -1

            weight = 1.0
            if edge_weights is not None:
                edge_mask = ((src == i) & (dst == j)) | ((src == j) & (dst == i))
                weight_vals = edge_weights[edge_mask.nonzero()[0]] if edge_mask.any() else [1.0]
                weight = weight_vals[0] if len(weight_vals) else 1.0
    ...
    df_neighbors = pd.DataFrame(rows, columns=[
        "Gene", "Gene_Cluster", "Neighbor", "Neighbor_Cluster", "Edge_Weight"
    ])
    save_path = os.path.join(output_dir, "topk_gene_neighbors_with_clusters.csv")
    df_neighbors.to_csv(save_path, index=False)
    print(f"📁 Saved top-k gene neighbors: {save_path}")

def save_topk_gene_neighbors_by_saliency(
    graph,
    summary_bio_features,
    node_names_topk,
    row_labels,
    ...
    topk_set = set(topk_node_indices)
    global_idx_to_name = {i: name for i, name in enumerate(node_names_topk)}

    rows = []
    for i in topk_node_indices:
        gene_name = global_idx_to_name.get(i, f"node_{i}")
        gene_cluster = row_labels[i] if i < len(row_labels) else -1
        gene_saliency = summary_bio_features[i].sum() if i < len(summary_bio_features) else float("nan")
        ...
        for j in neighbors:
            ...
            neighbor_name = global_idx_to_name.get(j, f"node_{j}")
            neighbor_cluster = row_labels[j] if j < len(row_labels) else -1
            neighbor_saliency = summary_bio_features[j].sum() if j < len(summary_bio_features) else float("nan")

            weight = 1.0
            if edge_weights is not None:
                edge_mask = ((src == i) & (dst == j)) | ((src == j) & (dst == i))
                edge_indices = edge_mask.nonzero()[0]
                weight = edge_weights[edge_indices[0]] if len(edge_indices) else 1.0

            neighbor_info.append((neighbor_name, neighbor_cluster, neighbor_saliency, weight, j))

        neighbor_info.sort(key=lambda x: x[2], reverse=True)
        top_neighbors = neighbor_info[:top_n_neighbors]

        for neighbor_name, neighbor_cluster, neighbor_saliency, weight, _ in top_neighbors:
            rows.append([
                gene_name, gene_cluster, gene_saliency,
                neighbor_name, neighbor_cluster, neighbor_saliency, weight
            ])

    df_neighbors = pd.DataFrame(rows, columns=[
        "Gene", "Gene_Cluster", "Gene_Saliency",
        "Neighbor", "Neighbor_Cluster", "Neighbor_Saliency",
        "Edge_Weight"
    ])
    save_path = os.path.join(output_dir, f"top{top_n_neighbors}_gene_neighbors_by_saliency.csv")
    df_neighbors.to_csv(save_path, index=False)
    print(f"📄 Saved top-{top_n_neighbors} gene-neighbor data with saliency to: {save_path}")
    return save_path
...
def train(args):
    ...
    save_topk_gene_neighbors_by_saliency(
        graph=graph,
        summary_bio_features=relevance_matrix_bio,
        node_names_topk=node_names_topk,
        topk_node_indices=top_gene_indices,
