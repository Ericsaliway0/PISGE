...
def save_and_plot_confirmed_genes_(
    node_names_topk,
...
def plot_gene_feature_contributions_(
    ...
    ax.set_title(f"{gene_name} Feature Contributions", fontsize=12)
    plt.tight_layout()
...
def plot_gene_feature_contributions_x_label_Omics(
    gene_name,
    relevance_vector,
    feature_names,
    output_path=None,
    score=None  # ← pass predicted probability
):
    """
    Plot a heatmap of feature contributions for a single gene.
    ...
    Parameters:
        gene_name (str): The name of the gene (e.g. 'NRAS').
        relevance_vector (np.ndarray): 1D array of relevance scores for each feature (shape: 64).
        feature_names (list of str): List of feature names like 'BRCA_mf', 'BRCA_cna', etc.
        output_path (str, optional): Path to save the plot.
        score (float, optional): Predicted probability of this gene.
    """
    assert len(relevance_vector) == 64, "Expected 64 feature contributions (4 omics × 16 cancers)."
    ...
    # Save omics barplot (optional external function)
    if output_path:
        barplot_path = output_path.replace(".png", "_omics_barplot.png")
        try:
            plot_omics_barplot(df, barplot_path)
        except Exception as e:
            print(f"⚠️ Skipped omics barplot due to error: {e}")

    # Split into cancer and omics
    df[['Cancer', 'Omics']] = df['Feature'].str.split('_', expand=True)
    ...
    # Add gene name with prediction probability
    ##title_str = f"{gene_name} (prediction probability = {score:.3f})" if score is not None else gene_name
    ##plt.suptitle(title_str, fontsize=18, fontweight='bold', x=0.05, y=1.15, ha='left')

    # Add gene name and optional prediction score
    title_str = f"{gene_name}"
    if score is not None:
        title_str += f"  |  Probability: {score:.3f}"

    plt.title(title_str, fontsize=14, fontweight='bold', loc='left', pad=10)
...
def plot_feature_importance_x_label_feature(
    relevance_vector,
    feature_names=None,
    node_name=None,
    top_k=8,
    ...
    sns.barplot(
        ...
        x="feature",
        y="relevance",
        ##hue="feature",              # <- fix: assign x to hue
        palette="Blues_d",          # <- your color map or dict
        dodge=False,                # <- prevent group spacing
        legend=False,
        width=0.3
    )
    plt.ylabel("Relevance Score")
    plt.title(f"{node_name} Neighbor Importances")
    plt.xticks(rotation=90, ha='right')
    plt.tight_layout()
...
def train(args):
    ...
    ##print("Is the graph bidirectional?============================ ", graph.is_bidirectional())
    ...
    # lrp_node_features: originally only gene node features (1791,)
    # graph.num_nodes() == 1980
    # Assume: lrp_node_features has shape [1789, 2048]
    # Assume: gene_node_indices contains the 1789 indices of gene nodes in the graph
    ...
    '''gene_node_indices = [i for i, name in enumerate(node_names) if name in gene_names_set]


    full_features = torch.zeros(graph.num_nodes(), lrp_node_features.shape[1])  # fill with zeros
    full_features[lrp_node_indices] = torch.tensor(lrp_node_features)  # assign gene features to correct positions

    relevance_scores = compute_lrp_scores(model, graph, lrp_node_features)'''
    print("Generating feature importance plots...")
    ...
    save_cluster_legend(output_path_legend, CLUSTER_COLORS, num_clusters=10)'''
    ...
    save_and_plot_confirmed_genes(
        ...
        #num_epochs=200,                # or just pass an integer
        confirmed_gene_path="data/ncg_8886.txt"                                  # optional, this is also the default
    )
    ...
    # Limit to first 8 confirmed genes
    confirmed_genes_to_plot = confirmed_genes[:8]
    ##print('confirmed_genes_to_plot==================================================== ', confirmed_genes_to_plot)
    ...
    for gene in confirmed_genes:
        ...
        if gene in node_names_topk:
            idx_in_topk = node_names_topk.index(gene)
            relevance_vector = reduced_scores[idx_in_topk]
        else:
            print(f"⚠️ Gene {gene} not found in topk predictions.")
            continue
        ...
        plot_feature_importance(
            ...
            node_name=gene,
            output_path=plot_path,
            gene_names=top_names,        # These are the top neighbors
            top_k=len(top_names)         # Set this to match how many neighbors you're plotting
        )
