...
def plot_neighbor_relevance_by_mode(
    ...
    # If it’s a vector, take the sum
    gene_score_tensor = neighbor_scores[node_idx]

    if gene_score_tensor.ndim > 0:
        gene_score = gene_score_tensor.sum().item()
    else:
        gene_score = gene_score_tensor.item()
...
def plot_neighbor_relevance(
    ...
    if row_labels is not None:
        # Infer gene's cluster from name (if included) or from first neighbor
        #main_cluster_id = int(row_labels.get(neighbor_ids[0], 0))
        main_cluster_id = int(row_labels[neighbor_ids[0]])
...
def get_neighbors_gene_names(graph, node_names, name_to_index, genes):
    ...
    """
    # For fast reverse lookup: gene names → indices
    ...
    for gene in genes:
        ...
        # ✅ Filter neighbor indices to those in genes_idx_set
...
def train(args):
    ...
    for gene in node_names_topk:
        plot_neighbor_relevance_by_mode(
            ...
            node_id_to_name=node_id_to_name,
            graph=graph,
            row_labels=row_labels,#graph.ndata["cluster_bio"], 
            total_clusters=best_k,            
            args=args
        )
