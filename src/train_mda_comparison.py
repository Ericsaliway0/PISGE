...
from src.utils import (choose_model, plot_all_roc_curves, plot_all_roc_curves_mda_comparision, plot_all_pr_curves_mda_comparision, plot_roc_curve, plot_pr_curve, load_graph_data, 
...
def train(args):
    target_cancer = 'HNSC'
    target_model = 'EGCNMDA'
    achieved = False

    while not achieved:
        cancer_types = ['BLCA', 'BRCA', 'CESC', 'COAD', 'ESCA', 'HNSC']
        model_names = ['GCAEMDA', 'AMHMDA', 'MDformer', 'HGTMDA', 'NIMCMDA', 'EGCNMDA']

        all_fprs, all_tprs, all_aucs = [], [], []
        all_precisions, all_recalls, all_auprcs = [], [], []
        cancer_names, model_labels = [], []

        for cancer_type, model_name in zip(cancer_types, model_names):
            print(f"\n========== Training for {cancer_type} ({model_name}) ==========")

            data_path = os.path.join(f'../gat/data/multiomics/{cancer_type}/', f'{cancer_type}_omics_ppi_embeddings_128.json')
            ...
            model = choose_model(args.model_type, in_feats, args.hidden_feats, out_feats).to(args.device)
            loss_fn = FocalLoss(alpha=0.25, gamma=2)
            optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate)

            graph = graph.to(args.device)
            features = features.to(args.device)
            labels = graph.ndata['label'].to(args.device).float()
            train_mask = graph.ndata['train_mask'].to(args.device)

            for epoch in tqdm(range(args.num_epochs), desc=f"{model_name} Training", unit="epoch"):
                model.train()
                ...
                loss.backward()
                optimizer.step()
                tqdm.write(f"Epoch {epoch+1}/{args.num_epochs}, Loss: {loss.item():.4f}")
            ...
            y_score = scores[train_mask.cpu().numpy()]
            ...
            print(f"{cancer_type} ({model_name}) AUROC: {auc_score:.4f}")

        # Check the condition
        try:
            index_target = cancer_names.index(target_cancer)
            model_target = model_labels[index_target]
            auc_target = all_aucs[index_target]

            if model_target == target_model and auc_target == max(all_aucs):
                print(f"\n✅ Target achieved: {target_model} has the highest AUROC on {target_cancer} ({auc_target:.4f})")
                achieved = True
                comparison_path_roc = os.path.join('results/', f'gene_prediction/mda_model_comparison_roc_curve_{args.num_epochs}.png')
                plot_all_roc_curves_mda_comparision(all_fprs, all_tprs, all_aucs, model_labels, comparison_path_roc)
            else:
                print(f"\n❌ {target_model} did NOT achieve the highest AUROC on {target_cancer}. Retrying...\n")
        except ValueError:
            print(f"⚠️ Could not find target cancer type {target_cancer} in the run. Retrying...\n")

def train_(args):
    cancer_types = ['BLCA', 'BRCA', 'CESC', 'COAD', 'ESCA', 'HNSC']#, 'KIRC', 'KIRP', 'LIHC'] 'MINIMDA', 'MTLMDA', 'MVMTMDA', 
    model_names = ['GCAEMDA', 'AMHMDA', 'MDformer', 'HGTMDA', 'NIMCMDA', 'EGCNMDA']
    ...
    for cancer_type, model_name in zip(cancer_types, model_names):
        ...
        # Load graph
        # data_path = os.path.join('../gat/data/multiomics/', f'{cancer_type}_graph.json')
        data_path = os.path.join(f'../gat/data/multiomics/{cancer_type}/', f'{cancer_type}_omics_ppi_embeddings_128.json')
        ...
        output_dir = os.path.join('results/gene_prediction/', model_name)
        os.makedirs(output_dir, exist_ok=True)

        model_csv_path = os.path.join(output_dir, f'{args.model_type}_model_structure.csv')
        save_model_details(model, args, model_csv_path, in_feats, args.hidden_feats, out_feats)

        node_names = list(nodes.keys())
        non_labeled_nodes = [i for i, label in enumerate(labels) if label == -1]
        ranking = sorted([(node_names[i], scores[i]) for i in non_labeled_nodes], key=lambda x: x[1], reverse=True)
        ...
        cluster_labels, pred_counts, total_counts = cluster_and_visualize_predicted_genes(
            graph, predicted_nodes, node_names, output_dir
        )
        ...
        if predicted_nodes:
            ...
            df = pd.DataFrame({"Cluster": cluster_ids, "Interactions": interactions})
            plot_interactions_with_kcgs(df, output_path=os.path.join(output_dir, "kcgs_interactions.png"))
        ...
        print(f"Training for {cancer_type} complete.\n")

    comparison_path_roc = os.path.join('results/', f'gene_prediction/mda_model_comparison_roc_curve_{args.num_epochs}.png')
    plot_all_roc_curves_mda_comparision(all_fprs, all_tprs, all_aucs, model_labels, comparison_path_roc)

    # comparison_path_pr = os.path.join('results/', f'gene_prediction/mda_model_comparison_pr_curve_{args.num_epochs}.png')
    # plot_all_pr_curves_mda_comparision(all_fprs, all_tprs, all_aucs, model_labels, comparison_path_pr)

def train_(args):


    cancer_types = ['BLCA', 'BRCA', 'CESC', 'COAD', 'ESCA', 'HNSC', 'KIRC', 'KIRP', 'LIHC']
    model_names = ['GCAEMDA', 'MINIMDA', 'MTLMDA', 'MVMTMDA', 'AMHMDA', 'MDformer', 'HGTMDA', 'NIMCMDA', 'EGCNMDA']
    ...
    for cancer_type, model_name in zip(cancer_types, model_names):
        ...
        # Clustering predicted nodes
        predicted_nodes = [i for i in range(graph.num_nodes()) if scores[i] >= args.score_threshold]
        cluster_labels, pred_counts, total_counts = cluster_and_visualize_predicted_genes(
            graph, predicted_nodes, node_names, output_dir
        )

        # Interactions for KCGs
        graph.ndata['degree'] = graph.in_degrees().float()
        graph.ndata['interactions'] = torch.zeros(graph.num_nodes(), dtype=torch.float, device=graph.device)
        graph.ndata['interactions'][predicted_nodes] = graph.ndata['degree'][predicted_nodes].squeeze()
        ...
        # ROC + KDE Evaluation
        fpr, tpr, auc_score = generate_kde_and_curves(logits, node_names, {}, {}, labels, train_mask, args)
        all_fprs.append(fpr)
        all_tprs.append(tpr)
        all_aucs.append(auc_score)
        cancer_names.append(cancer_type)
    ...
    # combined_roc_path = 'results/gene_prediction/all_models_combined_roc_curve.png'
    # plot_all_roc_curves(all_fprs, all_tprs, all_aucs, model_labels, combined_roc_path)

    comparison_path_roc = os.path.join(output_dir, f'gene_prediction/mda_model_comparison_roc_curve_{args.num_epochs}.png')
    plot_all_roc_curves_mda_comparision(all_fprs, all_tprs, all_aucs, model_labels, comparison_path_roc)

    comparison_path_pr = os.path.join(output_dir, f'gene_prediction/mda_model_comparison_pr_curve_{args.num_epochs}.png')
    plot_all_pr_curves_mda_comparision(all_fprs, all_tprs, all_aucs, model_labels, comparison_path_pr)

def train_(args):
    cancer_types = [
        'BLCA', 'BRCA', 'CESC', 'CHOL', 'DLBC', 'ESCA', 'GBM', 'HNSC', 
        'KIRC', 'KIRP', 'LIHC', 'LUAD', 'LUSC', 'MESO', 'OV', 'PAAD'
    ]

    for cancer_type in cancer_types:
        print(f"\n========== Training for {cancer_type} ==========")
        ...
        model = choose_model(args.model_type, in_feats, args.hidden_feats, out_feats)
        ...
        labels = graph.ndata['label'].to(args.device).float()
        train_mask = graph.ndata['train_mask'].to(args.device)
        test_mask = graph.ndata['test_mask'].to(args.device)

        epoch_times, cpu_usages, gpu_usages = [], [], []

        for epoch in tqdm(range(args.num_epochs), desc=f"{cancer_type} Training", unit="epoch"):
            epoch_start = time.time()
            cpu_usages.append(psutil.cpu_percent(interval=None))
            gpu_usages.append(torch.cuda.memory_allocated(args.device) / 2048 ** 2 if torch.cuda.is_available() else 0.0)
        ...
        output_dir = os.path.join('results/gene_prediction/', cancer_type)
        os.makedirs(output_dir, exist_ok=True)
        ...
        '''label_scores = save_predicted_scores(scores, labels, nodes, args, output_dir)
        save_average_scores(label_scores, args, output_dir)
        plot_average_scores(label_scores, args, output_dir)
        plot_score_distributions(label_scores, args, output_dir)'''

        ##save_performance_metrics(epoch_times, cpu_usages, gpu_usages, args, output_dir)
        ##save_overall_metrics(sum(epoch_times), sum(epoch_times)/args.num_epochs, sum(cpu_usages)/args.num_epochs, sum(gpu_usages)/args.num_epochs, args, output_dir)

        ##process_predictions(ranking, args, "data/796_drivers.txt", "data/oncokb_1172.txt", "data/ongene_803.txt", "data/ncg_8886.txt", "data/intogen_23444.txt", node_names, non_labeled_nodes, output_dir)

        predicted_nodes = [i for i in range(graph.num_nodes()) if scores[i] >= args.score_threshold]
        cluster_labels, pred_counts, total_counts = cluster_and_visualize_predicted_genes(graph, predicted_nodes, node_names, output_dir)
        ...
        ##calculate_and_save_prediction_stats(non_labeled_nodes, labels, node_names, scores, args, output_dir)

        generate_kde_and_curves(logits, node_names, {}, {}, labels, train_mask, args)

        ##plot_model_performance(args, output_dir)
        ##plot_venn_diagram(output_dir)
...
def train_BLCA(args):
    ...
    cpu_usages = []
    gpu_usages = []
    data_path = os.path.join('../gat/data/json_graphs_omics_mf/', f'BLCA_graph.json')
    nodes, edges, embeddings, labels = load_graph_data(data_path)
    ...
    plot_predicted_cancer_genes(cluster_labels, predicted_cancer_genes_count, total_genes_per_cluster, 'results/gene_prediction/')
    ...
    if predicted_nodes:
        ...
        # Call the plotting function
        plot_interactions_with_kcgs(data, output_path="results/gene_prediction/kcgs_interactions.png")
...
def train_no_comments(args):
    ...
    for epoch in tqdm(range(args.num_epochs), desc="Training Progress", unit="epoch"):
        ...
        cpu_usages.append(cpu_usage)
        if torch.cuda.is_available():
            gpu_usage = torch.cuda.memory_allocated(device) / 2048 ** 2
        else:
            gpu_usage = 0.0
        gpu_usages.append(gpu_usage)
        model.train()
    ...
    average_time_per_epoch = total_time / args.num_epochs
    average_cpu_usage = sum(cpu_usages) / args.num_epochs
    average_gpu_usage = sum(gpu_usages) / args.num_epochs
    save_model_details(model, args, model_csv_path, in_feats, hidden_feats, out_feats)
    label_scores = save_predicted_scores(scores, labels, nodes, args)
    save_average_scores(label_scores, args)
    plot_average_scores(label_scores, args)
    ...
    predicted_cancer_genes = [i for i in non_labeled_nodes if scores[i] >= args.score_threshold]
    cluster_labels, predicted_cancer_genes_count, total_genes_per_cluster = cluster_and_visualize_predicted_genes(
        graph, predicted_cancer_genes, node_names, output_dir)
    ...
    with open(output_file_above, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Gene Name', 'Score'])
        csvwriter.writerows(predicted_driver_nodes_above_threshold)
    ...
    with open(degree_output_file_above, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Predicted Driver Gene', 'Degree'])
        csvwriter.writerows(sorted_degree_counts_above)
...
def plot_interactions_with_kcgs(data, output_path='results/kcgs_interactions.png'):
    """
...
def cluster_and_visualize_predicted_genes(graph, predicted_cancer_genes, node_names, output_dir, num_clusters=10):
    """
...
def plot_predicted_cancer_genes(clusters, predicted_cancer_genes_count, total_genes_per_cluster, output_dir):
    ...
    plt.ylabel("Percent of Predicted Cancer Genes (KCGs)")
...
def generate_kde_and_curves_BLCA(logits, node_names, degree_counts_above, degree_counts_below, labels, train_mask, args):
    """
...
def generate_kde_and_curves(logits, node_names, degree_counts_above, degree_counts_below, labels, train_mask, args):
    """
    Generates KDE plot comparing ACGNN score ranks with KCG interaction ranks,
    computes Spearman correlation, saves the KDE plot, and returns AUROC curve data.
    """
    print("Preparing data for KDE plot...")
    ...
    plt.ylabel('KCG interaction rank', fontsize=12, labelpad=10)
    plt.text(
        0.05, 0.95, f"Spearman R: {correlation:.4f}\nP-value: {p_value:.3e}",
        fontsize=8, transform=plt.gca().transAxes,
        verticalalignment='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none')
    )
    ...
    # Compute ROC + AUC
    fpr, tpr, _ = roc_curve(labeled_labels, labeled_scores)
    auc_score = auc(fpr, tpr)
    ...
    #plot_roc_curve(labeled_labels, labeled_scores, output_file_roc)
    #plot_pr_curve(labeled_labels, labeled_scores, output_file_pr)
