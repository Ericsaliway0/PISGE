...
def eigengap_analysis(feature_matrix, max_clusters=25, normalize=True, plot_path=None):
    ...
    gaps = np.diff(eigenvals[1:max_clusters + 1])
    best_k = np.argmax(gaps) + 2  # +1 for diff shift, +1 to ensure k > 1

    # Clamp to max_clusters
    best_k = max(2, min(best_k, max_clusters))
...
def train(args):
    ...
    relevance_scores_bio_np = relevance_scores_bio.cpu().numpy()
    summary_bio_features = extract_summary_features_np_bio(relevance_scores_bio_np)
