...
def analyze_sankey_structure(
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        scores_arr = np.array([np.linalg.norm(relevance_scores[topk_name_to_index[g]]) for g in genes if g in topk_name_to_index])
...
def apply_full_spectral_biclustering_bio(
    ...
    # === ✅ Step 6: Save gene → cluster assignment with relevance score
    gene_assignment_path = os.path.join(
        output_dir, f"{args.model_type}_{args.net_type}_bio_summary_gene_cluster_assignments_epo{args.num_epochs}.csv"
    )

    with open(gene_assignment_path, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Gene_Name", "Index", "Cluster", "Relevance_Score"])
        for i, cluster in zip(topk_node_indices, row_labels):
            gene_name = node_names_topk[i] if i < len(node_names_topk) else f"node_{i}"
            rel_score = summary_bio_features[i].sum().item() if i < len(summary_bio_features) else float("nan")
            writer.writerow([gene_name, i, cluster, round(rel_score, 5)])
...
def apply_full_spectral_biclustering_bio(
    ...
    # Assign row cluster labels back to graph
    row_labels_tensor = torch.full((graph.num_nodes(),), -1, dtype=torch.long)
    row_labels_tensor[topk_node_indices] = torch.tensor(row_labels, dtype=torch.long)
    graph.ndata['cluster_bio_summary'] = row_labels_tensor
    ...
    # Save gene → cluster assignment CSV (sorted by cluster then saliency descending)
    gene_assignment_path = os.path.join(
        output_dir,
    ...
    assignment_rows = []
    for node_idx, cluster in zip(topk_node_indices, row_labels):
        if node_idx >= len(summary_bio_features):
            continue
        gene_name = node_names_topk[node_idx] if node_idx < len(node_names_topk) else f"node_{node_idx}"
        rel_score = summary_bio_features[node_idx].sum().item()
        assignment_rows.append([gene_name, node_idx, cluster, round(rel_score, 5)])

    assignment_rows.sort(key=lambda x: (x[2], -x[3]))  # Sort by cluster, then descending score
...
def save_topk_gene_cluster_and_neighbors(
    graph,  # DGLGraph or dict with edges
    topk_node_indices,  # List[int]
    node_names_topk,    # List[str]
    row_labels,         # List[int]
    summary_bio_features,  # np.ndarray
    output_dir,
    model_type,
    net_type,
    num_epochs
    ...
    node_idx_to_name = {idx: name for idx, name in zip(topk_node_indices, node_names_topk)}
    name_to_node_idx = {v: k for k, v in node_idx_to_name.items()}

    # Build graph using networkx
    G = nx.Graph()
    G.add_edges_from(graph.edges())  # if using DGLGraph
    # G.add_edges_from(graph['edges'])  # if using dict

    # Collect neighbor stats
    all_rows = []
    for i, node_idx in enumerate(topk_node_indices):
        gene = node_names_topk[i]
        cluster = row_labels[i]
        rel_score = summary_bio_features[i].sum().item()

        neighbors_topk = [n for n in G.neighbors(node_idx) if n in topk_set]

        neighbor_infos = []
        for neighbor_idx in neighbors_topk:
            if neighbor_idx not in node_idx_to_name:
                continue
            neighbor_name = node_idx_to_name[neighbor_idx]
            j = topk_node_indices.index(neighbor_idx)
            neighbor_cluster = row_labels[j]
            neighbor_score = summary_bio_features[j].sum().item()
            neighbor_infos.append(f"{neighbor_name} (clust {neighbor_cluster}, score {round(neighbor_score, 3)})")
        ...
        all_rows.append([
            ...
            cluster,
            round(rel_score, 5),
            "; ".join(neighbor_infos) if neighbor_infos else "None"
        ])

    # Sort by cluster, then by saliency score (descending)
    all_rows.sort(key=lambda x: (x[2], -x[3]))

    # Save to CSV
    out_path = os.path.join(
        output_dir,
        f"{model_type}_{net_type}_topk_gene_cluster_neighbors_epo{num_epochs}.csv"
    )
    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Gene_Name", "Graph_Index", "Cluster", "Relevance_Score", "TopK_Neighbors_Info"])
        writer.writerows(all_rows)

    print(f"📄 Saved cluster+neighbor assignment for top-k genes: {out_path}")
    return out_path
...
def apply_full_spectral_biclustering_bio(
    ...
    with open(neighbor_info_path, "w", newline="") as f:
        ...
        for i, cluster in zip(topk_node_indices, row_labels):
            ...
            # rel_score = summary_bio_features[i].sum().item()
            local_idx = topk_node_indices.index(i)
            rel_score = summary_bio_features[local_idx].sum().item()
            

            neighbors, edge_ids = graph.successors(i), graph.edge_ids(i, graph.successors(i))
            for j, e_id in zip(neighbors.tolist(), edge_ids.tolist()):
                if j not in topk_set:
                    continue
                neighbor_name = node_names_topk[j]
                # neighbor_cluster = int(row_labels[topk_node_indices.index(j)]) if j in topk_node_indices else -1
                neighbor_cluster = int(row_labels[topk_node_indices.index(j)])
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    node_id_to_name = {i: name for i, name in enumerate(node_names_topk)}
    cluster_assignments = graph.ndata["cluster_bio"].numpy()
    node_index_set = set(name_to_index[g] for g in node_names_topk if g in name_to_index)
    ...
    # === Build gene → cluster + neighbor mapping
    for gene in combined_genes:
        if gene not in topk_name_to_index or gene not in name_to_index:
            continue

        local_idx = topk_name_to_index[gene]
        global_idx = name_to_index[gene]

        if local_idx >= len(row_labels) or global_idx >= len(cluster_assignments):
            continue

        gene_cluster = cluster_assignments[global_idx]
        cluster_label = f"Cluster {gene_cluster}"
        cluster_to_genes.setdefault(cluster_label, []).append(gene)
        ...
        for neighbor in neighbors:
            # 🔒 Exclude self and any in combined set
            if neighbor == gene or neighbor in combined_genes:
                continue
            ...
            if neighbor not in name_to_index:
                continue
            neighbor_graph_idx = name_to_index[neighbor]
            if neighbor_graph_idx >= len(cluster_assignments):
                continue
            if cluster_assignments[neighbor_graph_idx] == -1:
                continue
    ...
    # === Save neighbor data (CSV)
    all_neighbor_rows = []
    for gene, neighbors in gene_to_neighbors.items():
        if gene not in name_to_index:
            continue
        graph_node_idx = name_to_index[gene]
        if graph_node_idx >= len(cluster_assignments):
            continue
        gene_cluster = int(cluster_assignments[graph_node_idx])

        for neighbor_idx, score in neighbors.items():
            if neighbor_idx >= len(node_names_topk) or neighbor_idx >= len(row_labels):
                continue
            neighbor_name = node_names_topk[neighbor_idx]
            if neighbor_name not in name_to_index:
                continue
            neighbor_graph_idx = name_to_index[neighbor_name]
            if neighbor_graph_idx >= len(cluster_assignments):
                continue
            neighbor_cluster = int(cluster_assignments[neighbor_graph_idx])
            if neighbor_cluster == -1:
                continue
            all_neighbor_rows.append([
                gene,
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            ...
            neighbors = gene_to_neighbors.get(gene, {})
            for neighbor_idx, neighbor_score in neighbors.items():
                if neighbor_idx >= len(node_names_topk):
                    continue
                neighbor_name = node_names_topk[neighbor_idx]
                if neighbor_name == gene or neighbor_name not in name_to_index:
                    continue
    ...
    # === Plot auxiliary stats
    # sankey_stats = analyze_sankey_structure(
    #     source, target, value, label_to_idx, node_names, cluster_to_genes,
    #     gene_to_neighbors, row_labels, name_to_index, relevance_scores,
    #     topk_name_to_index=topk_name_to_index  # 👈 add this
    # )
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    for gene in combined_genes:
        ...
        local_idx = topk_name_to_index[gene]
        global_idx = name_to_index[gene]
        if global_idx >= len(cluster_assignments):
            continue
        ...
        if neighbor_scores:
            ...
            neighbor_scores = {}  # make sure it's empty
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes:
            neighbors = gene_to_neighbors.get(gene, {})
            if not neighbors:
                continue  # 🚫 Skip genes with no valid neighbors!

            if cluster_label not in label_to_idx:
                cluster_idx = len(all_labels)
                label_to_idx[cluster_label] = cluster_idx
                all_labels.append(cluster_label)
                cluster_id = int(cluster_label.split()[-1])
                all_colors.append(CLUSTER_COLORS.get(cluster_id, "#000000"))
                font_sizes.append(24)
            ...
            if gene not in label_to_idx:
                gene_idx = len(all_labels)
                label_to_idx[gene] = gene_idx
                all_labels.append(gene)
                rel_idx = topk_name_to_index[gene]
            ...
            # Add Gene → Neighbor → Neighbor Cluster
            for neighbor_idx, neighbor_score in neighbors.items():
                neighbor_name = node_names_topk[neighbor_idx]
                if neighbor_name == gene or neighbor_name not in name_to_index:
                    continue
                ...
                neighbor_graph_idx = name_to_index[neighbor_name]
                neighbor_cluster = cluster_assignments[neighbor_graph_idx]
                if neighbor_cluster == -1:
                    continue
                neighbor_cluster_label = f"nClust {neighbor_cluster}"
    ...
    try:
        ...
        fig.write_image(png_save_path, scale=2, width=600, height=1200)
        print(f"🖼️ PNG also saved: {png_save_path}")
    except Exception as e:
        print(f"⚠️ PNG save failed: {e} — install 'kaleido' for image export!")
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    all_gene_set = set(combined_genes)  # 👈 Track all genes to exclude them as neighbors
    ...
    for gene in combined_genes:
        ...
        for n in neighbors:
            if n == gene:
                continue  # 🔒 skip self
            if n in all_gene_set:
                continue  # 🔒 skip any gene in combined_genes
            if n not in topk_name_to_index:
                continue
            rel_idx = topk_name_to_index[n]
            if rel_idx >= relevance_scores.shape[0]:
                continue
            neighbor_graph_idx = name_to_index.get(n)
            if neighbor_graph_idx is None:
                continue
            neighbor_cluster = cluster_assignments[neighbor_graph_idx]
            if neighbor_cluster == -1:
                continue
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            ...
            neighbors = gene_to_neighbors.get(gene, {})
            valid_neighbors = {
                idx: score for idx, score in neighbors.items()
                if node_id_to_name[idx] != gene and
                node_id_to_name[idx] not in combined_genes and
                cluster_assignments[name_to_index.get(node_id_to_name[idx], -1)] != -1
            }

            if not valid_neighbors:
                continue  # 🚫 Skip this gene if no valid neighbors

            # 🟢 Safe to add gene node and link to cluster
            if gene not in label_to_idx:
                label_to_idx[gene] = len(all_labels)
                all_labels.append(gene)
                rel_idx = topk_name_to_index[gene]
                saliency = relevance_scores[rel_idx].sum().item()
                gene_cluster = cluster_assignments[name_to_index[gene]]
                color = CLUSTER_COLORS.get(gene_cluster, "#000000")
                all_colors.append(color)
                font_sizes.append(16 if saliency > 0.5 else 10)
            ...
            for neighbor_idx, neighbor_score in neighbors.items():
                ...
                ## neighbor_cluster = row_labels[neighbor_idx]
                neighbor_graph_idx = name_to_index.get(node_id_to_name[neighbor_idx])
                neighbor_cluster = cluster_assignments[neighbor_graph_idx] if neighbor_graph_idx is not None else -1
                if neighbor_cluster == -1:
                    continue
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    all_gene_set = set(combined_genes)
    ...
    for gene in combined_genes:
        ...
        for n in neighbors:
            if n == gene or n in all_gene_set:
                continue
            if n not in node_names_topk:
                continue
            if n not in topk_name_to_index:
                continue
    ...
    for gene, neighbors in gene_to_neighbors.items():
        ...
        for neighbor_idx, score in neighbors.items():
            neighbor_name = node_id_to_name[neighbor_idx]
            neighbor_graph_idx = name_to_index.get(neighbor_name)
            neighbor_cluster = int(cluster_assignments[neighbor_graph_idx])
            if neighbor_cluster == -1:
                continue
            all_neighbor_rows.append([gene, neighbor_name, str(neighbor_idx), float(score), neighbor_cluster])

    csv_path = os.path.join(
        output_dir, f"{args.model_type}_{args.net_type}_gene_neighbors_epo{args.num_epochs}.csv"
    )
    ...
    for cluster_label, genes in cluster_to_genes.items():
        for gene in genes:
            ...
            for neighbor_idx, neighbor_score in neighbors.items():
                ...
                if neighbor_name not in label_to_idx:
                    neighbor_node_idx = len(all_labels)
                    label_to_idx[neighbor_name] = neighbor_node_idx
                    all_labels.append(neighbor_name)
                    all_colors.append(CLUSTER_COLORS.get(neighbor_cluster, "#000000"))
                    font_sizes.append(14)
                else:
                ...
                if neighbor_cluster_label not in label_to_idx:
                    neighbor_cluster_idx = len(all_labels)
                    label_to_idx[neighbor_cluster_label] = neighbor_cluster_idx
                    all_labels.append(neighbor_cluster_label)
                    all_colors.append(CLUSTER_COLORS.get(neighbor_cluster, "#000000"))
                    font_sizes.append(18)
                else:
    ...
    print(f"✅ Sankey saved: {save_html}")
    ...
    try:
        ...
        fig.write_image(png_path, scale=2, width=600, height=1200)
        print(f"🖼️ PNG also saved: {png_path}")
    except Exception as e:
        print(f"⚠️ PNG save failed: {e} — install 'kaleido' to export PNGs.")
...
def plot_confirmed_neighbors_bio(
    ...
    for gene in top_scored_genes:
        ...
        for neighbor in neighbors:
            if neighbor == gene:
                continue
            if neighbor not in node_names_topk:
                continue
            if neighbor not in topk_name_to_index:
                continue
        ...
        for neighbor_idx, score in top_neighbors.items():
            ...
            neighbor_cluster = int(cluster_assignments[neighbor_graph_idx])
            all_neighbor_rows.append([
                gene, neighbor_name, str(neighbor_idx), round(score, 5), neighbor_cluster
            ])
        ...
        plot_neighbor_relevance(
            ...
            node_id_to_name=node_id_to_name,
            output_path=plot_path,
            row_labels=cluster_assignments,
            total_clusters=total_clusters,
    ...
    print(f"✅ Combined neighbor CSV saved: {csv_path}")
...
#     output_path = Path(output_path)
#     cluster_subdir = output_path.parent
#     cluster_subdir.mkdir(parents=True, exist_ok=True)
...
#     plt.ylabel("Rel. Score", fontsize=9)
#     plt.xticks(rotation=90, fontsize=8)
#     plt.yticks(fontsize=8)

#     sns.despine()
#     if add_legend and cluster_ids and CLUSTER_COLORS:
#         unique_clusters = sorted(set(cluster_ids))
#         legend_handles = [
#             mpatches.Patch(color=CLUSTER_COLORS.get(cid % total_clusters, "gray"), label=f"C{cid}")
#             for cid in unique_clusters
#         ]
...
#     plt.savefig(output_path, dpi=300, bbox_inches="tight")
#     plt.close()
#     print(f"✅ Neighbor relevance plot saved to {output_path}")
...
#     name_to_index,
#     predicted_cancer_genes,
#     node_names_topk,  # the actual top-k
#     row_labels,
#     total_clusters,
#     output_dir,
#     relevance_scores,
...
#     # ✅ Safe mapping ONLY for topk
#     topk_name_to_index = {name: i for i, name in enumerate(node_names_topk)}
#     node_id_to_name = {i: name for i, name in enumerate(node_names)}
#     cluster_assignments = graph.ndata["cluster_bio"].numpy()

#     # ✅ Filter to keep indices within topk
#     top_scored_genes = sorted(
#         [g for g in predicted_cancer_genes if g in topk_name_to_index],
#         key=lambda g: relevance_scores[topk_name_to_index[g]].sum(),
#         reverse=True
...
#     for gene in top_scored_genes:
#         if gene not in name_to_index or gene not in topk_name_to_index:
#             print(f"⚠️ Skipping gene not in topk index: {gene}")
#             continue
...
#     all_neighbor_rows = []
#     clusterwise_rows = defaultdict(list)  # ✅ cluster → rows
...
#             clusterwise_rows[gene_cluster].append(row)
...
#     # ✅ Save cluster-wise CSVs
#     for cluster_id, rows in clusterwise_rows.items():
#         cluster_dir = os.path.join(output_dir, f"cluster_{cluster_id}")
#         os.makedirs(cluster_dir, exist_ok=True)
#         cluster_csv_path = os.path.join(
#             cluster_dir,
#             f"{args.model_type}_{args.net_type}_bio_cluster{cluster_id}_confirmed_neighbors_epo{args.num_epochs}.csv"
#         )
#         with open(cluster_csv_path, "w", newline="") as csvfile:
#             writer = csv.writer(csvfile)
#             writer.writerow(["Source_Gene", "Neighbor_Name", "Neighbor_Index", "Relevance_Score", "Cluster"])
#             writer.writerows(rows)
#         print(f"✅ Cluster {cluster_id} CSV saved: {cluster_csv_path}")
...
def plot_confirmed_neighbors_bio(
    ...
    # 🔑 Always use the same index source as your embeddings
    topk_name_to_index = {name: i for i, name in enumerate(node_names_topk)}
    node_id_to_name = {i: name for i, name in enumerate(node_names_topk)}
    cluster_assignments = graph.ndata["cluster_bio"].numpy()

    # ✅ Filter by index in bounds
    top_scored_genes = sorted(
        [
            g for g in predicted_cancer_genes
            if g in topk_name_to_index and topk_name_to_index[g] < relevance_scores.shape[0]
        ],
        key=lambda g: relevance_scores[topk_name_to_index[g]].sum(),
        reverse=True
    )[:top_k]
    ...
    for gene in top_scored_genes:
        ...
        for neighbor in neighbors:
            ...
            neighbor_graph_idx = name_to_index.get(neighbor)
            if neighbor_graph_idx is None or cluster_assignments[neighbor_graph_idx] == -1:
                continue
        ...
        for neighbor_idx, score in top_neighbors.items():
            neighbor_name = node_id_to_name.get(neighbor_idx, "UNK")
            neighbor_graph_idx = name_to_index.get(neighbor_name, None)
            if neighbor_graph_idx is None:
                continue
            neighbor_cluster = int(cluster_assignments[neighbor_graph_idx])
            all_neighbor_rows.append([
        ...
        # 🗂️ Save to cluster-wise directory
        plot_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{gene}_neighbor_relevance_epo{args.num_epochs}.png"
        )
    ...
    # ✅ Combined CSV in main output_dir
    os.makedirs(output_dir, exist_ok=True)
    csv_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_bio_top{top_k}_confirmed_neighbors_epo{args.num_epochs}.csv"
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    output_dir,
    CLUSTER_COLORS,
    top_n_neighbors=3  # 👈 NEW: control how many neighbors per gene
):
    import os
    import csv
    import plotly.graph_objects as go
    ...
    for gene in combined_genes:
        ...
        if neighbor_scores:
            # ✅ Limit number of neighbors per gene
            neighbor_scores = dict(
                sorted(neighbor_scores.items(), key=lambda x: -x[1])[:top_n_neighbors]
            )
        else:
            neighbor_scores = {}
