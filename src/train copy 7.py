...
def plot_feature_importance_Non(
    relevance_vector,
    feature_names=None,
    node_name=None,
    top_k=9,
...
def plot_feature_importance_horizontal_x(relevance_vector, feature_names, node_name, top_k=10, output_path=None):
    # Get top-k feature indices
    top_indices = np.argsort(relevance_vector)[-top_k:][::-1]

    # Get corresponding names (with bounds checking)
    top_labels = [feature_names[i] if i < len(feature_names) else f"Unknown {i}" for i in top_indices]
    top_values = relevance_vector[top_indices]

    # Plot
    plt.figure(figsize=(10, 6))
    bars = plt.barh(range(top_k), top_values[::-1], color='steelblue')
    plt.yticks(range(top_k), labels=top_labels[::-1])
...
def plot_feature_importance_unknown_names(
    relevance_vector,
    feature_names=None,
    node_name=None,
    top_k=9,
    ...
    """
    Plots top-K most relevant features for a given node.

    Args:
        relevance_vector (array or tensor): Feature-level relevance scores
        feature_names (list): Optional list of feature names
        node_name (str): Node (gene) name for the plot title
        top_k (int): Number of top features to plot
        gene_names (list): Optional alternative labeling source
    ...
    # --- 🏷️ Prepare feature labels ---
    if feature_names is None:
        feature_names = [f"Feature {i}" for i in range(len(relevance_vector))]

    if gene_names is not None:
        top_labels = [
            gene_names[i].capitalize() if i < len(gene_names) else f"Unknown {i}"
            for i in top_indices
        ]
    else:
        top_labels = [
            feature_names[i] if i < len(feature_names) else f"Unknown {i}"
            for i in top_indices
        ]
...
def plot_feature_importance(
    ...
    def extract_group(label):
        parts = label.split("-")
        return parts[1] if color_by == "omics" else parts[0]

    groups = [extract_group(label) for label in top_labels]
...
def train(args):
    ...
    gene_score = scores[node_idx]  # Predicted cancer score
    
    # --- 🔬 Gradient-based feature relevance ---
    ...
    relevance_vector = relevance_scores[node_idx].cpu().numpy()
    ...
    # - graph: DGLGraph
    # - features: torch.Tensor of shape [num_nodes, num_features]
    # - node_names: list of gene names indexed by node ID
    # - name_to_index: dict mapping gene name → node ID
    # - scores: output of model(graph, features)
    # - feature_names: optional, names of input features (can be just ["f1", ..., "f2048"])
    # - args: argparse with fields model_type, net_type, num_epochs
    # - get_neighbors_gene_names, compute_relevance_scores, plot_feature_importance: your utilities
    ...
    '''explainer = GNNExplainer(
        model=model,
        num_hops=3,  #GNN's number of layers
    ...
    feature_names_by_cancer = []
    counter = 0
    for cancer in cancer_types:
        for omics in omics_types:
            for i in range(16):
                label = f"{cancer}-{omics}-{i} ({counter})"
                feature_names_by_cancer.append(label)
                counter += 1
    ...
    for gene in confirmed_genes:
        ...
        try:
            ...
            plot_feature_importance(
                relevance_vector=relevance_scores[idx],
                feature_names=feature_names_by_cancer,
                node_name=node_names[idx],
                top_k=9,
                output_path=os.path.join(
                    save_dir,
                    f"{args.model_type}_{args.net_type}_{gene}_feature_relevance_epo{args.num_epochs}.png"
                )
            )
            ...
            # --- 🔍 GNNExplainer: edge and feature attribution ---
            '''explanation = explainer.explain_node(node_idx, graph, features)

            # Extract masks
            edge_mask = explanation['edge_mask']
            feat_mask = explanation['feat_mask']

            # Visualize subgraph with edge importance
            ...
            plot_path = os.path.join(
                save_dir,
                f"{args.model_type}_{args.net_type}_{gene}_gnnexplainer_epo{args.num_epochs}.png"
            )
            plt.savefig(plot_path)
            plt.close()

            # --- 🧠 Top neighbors by edge importance ---
            top_k = 10
            top_edges = edge_mask.argsort(descending=True)[:top_k]
            u, v = graph.find_edges(top_edges)

            top_neighbors = [node_names[vid.item()] for vid in v]
            edge_scores = [edge_mask[i].item() for i in top_edges]

            print(f"  Top neighbors (by edge relevance):")
            for n, s in zip(top_neighbors, edge_scores):
                print(f"    {n} ({s:.4f})")

            # Plot edge importance bar chart
            plot_feature_importance(
                relevance_vector=edge_scores,
                gene_names=top_neighbors,
                node_name=gene,
                output_path=os.path.join(
                    save_dir,
                    f"{args.model_type}_{args.net_type}_{gene}_neighbor_edge_importance_epo{args.num_epochs}.png"
                )
            )'''

        except Exception as e:
            print(f"❌ Error explaining {gene}: {e}")
            continue
...
def train_(args):
    ...
    for gene in confirmed_genes:
        ...
        relevance_scores = compute_relevance_scores(
            model, graph, features, node_indices=[node_idx]
        )
        relevance_vector = relevance_scores[node_idx].cpu().numpy()
        ...
        try:
            plot_feature_importance(
                relevance_vector=relevance_vector,  # ✅ Correct index
                feature_names=feature_names_by_cancer,
                node_name=node_names[node_idx],              # ✅ Correct index
                top_k=9,
                output_path=os.path.join(
                    save_dir,
                    f"{args.model_type}_{args.net_type}_{gene}_feature_relevance_epo{args.num_epochs}.png"
...
def train_saliency_pass_fail_gnnexplainer(args):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    output_dir = 'results/gene_prediction/'
    os.makedirs(output_dir, exist_ok=True)
