...
def plot_top_gene_ridge_from_all_omics(
    ...
    for cluster in clusters:
        ...
        g.set_titles("")
        g.set(xlim=(xmin, xmax), xlabel="", ylabel="", yticks=[])
        ...
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=-0.3, left=0.3, right=0.95, top=0.93)
        g.fig.suptitle(f"Cluster {cluster}", x=0.6, fontsize=24)
    ...
    def make_json_safe(obj):
        ...
        eli
        f isinstance(obj, (np.integer, np.floating)):
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            for cancer_type in cancer_types:
                ...
                terms = collect_top_enrichments(cancer_type=cancer_type, tag="bio", source="KEGG", top_n=20)
                draw_horizontal_bar_plot(terms, cancer_type=cancer_type, source="KEGG")
