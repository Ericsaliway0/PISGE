...
def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names, predicted_cancer_genes, n_clusters,
    ...
    output_path_unclustered_heatmap = output_path_heatmap.replace(".png", "_unclustered.png")
    '''plot_bio_heatmap_raw_unsorted(summary_bio_features, predicted_indices, output_path_unclustered_heatmap)
    plot_tsne(summary_bio_features, row_labels, predicted_indices, n_clusters[0], output_path_genes_clusters)
    plot_bio_heatmap_unsort(summary_bio_features, row_labels, col_labels, predicted_indices, output_path_heatmap)'''
...
def train(args):
    ...
    relevance_scores_topk = relevance_scores_bio[topk_node_indices_tensor]
    ...
    best_k, eigenvals = eigengap_analysis(saliency_matrix, max_clusters=20, normalize=True, plot_path=plot_eigengap_path)
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            plot_bio_biclustering_heatmap(
                args=args,
                relevance_scores=saliency_matrix,#relevance_scores_topk.detach().cpu().numpy(),
