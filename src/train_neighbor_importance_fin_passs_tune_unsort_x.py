...
def train(args):
    ...
    relevance_scores_bio_64= extract_summary_features_np_bio(relevance_scores_bio)
    ...
    relevance_scores_topk_norm = StandardScaler().fit_transform(relevance_scores_topk)
    ...
    graph, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio(
        ...
        summary_bio_features=relevance_scores_topk,
        summary_bio_features_norm=relevance_scores_topk_norm,
        node_names=node_names,
        #feature_names=None,
    ...
    # === Spectral biclustering
    bicluster = SpectralBiclustering(n_clusters=n_clusters, method='log', random_state=42)
    bicluster.fit(summary_bio_topk)
    cluster_labels_bio = bicluster.row_labels_
    col_labels = bicluster.column_labels_
    graph.ndata['cluster_bio_summary'] = torch.tensor(cluster_labels_bio, dtype=torch.long)
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        cancer_feature_64 = extract_all_omics_for_cancer(relevance_scores_topk, cancer_type)
        row_labels = apply_full_spectral_biclustering_cancer(cancer_feature_64, n_clusters=best_k)
        ...
        if tag == "bio":
            ...
            plot_bio_heatmap_unsort(relevance_scores_topk, relevance_scores_topk_norm, cluster_labels_bio, col_labels, predicted_indices, output_path_heatmap)
