...
def plot_gene_feature_contributions_not_split_degree_name(gene_name, relevance_vector, feature_names, output_path=None):
    """
...
def plot_tsne_predicted_genes(graph, node_names, scores, output_path, args):
    ...
    n_samples = predicted_embeddings.shape[0]
    if n_samples < 2:
        print(f"[t-SNE skipped] Only {n_samples} predicted gene(s) ≥ threshold {args.score_threshold}")
        return
    perplexity = min(30, max(1, n_samples // 3))  # safe rule
...
def save_and_plot_confirmed_genes(
    ...
    for gene_name in confirmed_genes:
        ...
        plot_gene_feature_contributions(
            gene_name=gene_name,
            relevance_vector=relevance_vector,
            feature_names=feature_names,  # assumes "Cancer (OMICS)" format
            output_path=plot_path
        )
