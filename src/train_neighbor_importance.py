...
def plot_neighbor_relevance_(neighbor_scores, gene_name="Gene", node_id_to_name=None, output_path=None):
    # Sort and normalize
...
def plot_neighbor_relevance_great_than_005(
    neighbor_scores, 
    gene_name="Gene", 
    node_id_to_name=None, 
    output_path=None
    ...
    """
    Plot top 10 relevance scores (> 0.05) of neighbors for a given gene.
    ...
    # Filter out scores <= 0.05
    filtered_scores = {k: v for k, v in neighbor_scores.items() if v > 0.05}

    if not filtered_scores:
        print(f"⚠️ No neighbor relevance scores > 0.05 for {gene_name}. Skipping plot.")
        return

    # Sort by descending relevance and take top 10
    sorted_scores = dict(sorted(filtered_scores.items(), key=lambda x: -x[1])[:10])
    ...
    # Plot
    plt.figure(figsize=(max(3, 0.3 * len(df)), 2.5))  # Adjust width based on number of bars
    sns.set_style("white")
    ...
    # Rotate x-tick labels and center-align them
    ...
    for gene in confirmed_genes:
        ...
        for n in neighbors:
            if n in name_to_index:
                nid = name_to_index[n]
                score = relevance_scores[nid].sum().item()
                neighbor_scores_dict[nid] = score
