...
def plot_neighbor_relevance_by_mode(
    ...
    # topk_name_to_index = {n: i for i, n in enumerate(node_id_to_name.values())}

    # relevance_vals = [relevance_scores[i].sum().item() for i in neighbor_indices]
    # scores_dict = dict(zip(neighbor_indices, relevance_vals))
    # ✅ Match logic with get_top_neighbors
    scores_dict = get_top_neighbors(
        gene,
        neighbors,
        name_to_index,
        topk_name_to_index={n: i for i, n in enumerate(node_id_to_name.values())},
...
def plot_neighbor_relevance_ori(
    ...
    for nid in neighbor_ids:
        if row_labels is not None:
            ...
            colors.append(CLUSTER_COLORS.get(cluster_id % total_clusters, "lightgray"))
        else:
    ...
    if add_legend and row_labels is not None:
        ...
        legend_handles = [
            mpatches.Patch(color=CLUSTER_COLORS.get(cid % total_clusters, "lightgray"), label=f"Cluster {cid}")
...
def apply_full_spectral_biclustering_bio_ori(
...
def plot_bio_biclustering_heatmap(
    ...
    # Save ordered sum of relevance scores
    sum_scores = sorted_scores.sum(axis=1)
    sum_scores_df = pd.DataFrame({
        "Gene": row_gene_names,
        "Cluster": sorted_clusters,
        "SumScore": sum_scores
    })

    sum_csv_path = output_path.replace(".png", "_sum_ordered_scores.csv")
    sum_scores_df.to_csv(sum_csv_path, index=False)
    print(f"🔢 Sum of ordered relevance scores saved to {sum_csv_path}")
...
def plot_neighbor_relevance_itself_neighbor(
    ...
    for nid in neighbor_ids:
        if row_labels is not None and not str(nid).startswith("dummy_"):
            ...
            colors.append(CLUSTER_COLORS.get(cid % total_clusters, "lightgray"))
        else:
            colors.append("white")
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    for gene in combined_genes:
        ...
        # 📁 Create CSV of all gene-neighbor pairs with saliency score
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    for gene in combined_genes:
        ...
        neighbor_scores = get_top_neighbors(
            ...
            relevance_scores,
            node_id_to_name,
            output_dir=None,  # Defer single file save
            k=10
        )
        # if neighbor_scores:
        #     neighbor_scores = dict(sorted(neighbor_scores.items(), key=lambda x: -x[1])[:5])
        ...
        for neighbor_idx, score in neighbor_scores.items():
            neighbor_name = node_id_to_name.get(neighbor_idx, f"IDX_{neighbor_idx}")
            all_neighbor_rows.append([gene, neighbor_name, score])
...
def plot_confirmed_neighbors_bio(
    ...
    all_neighbor_rows = []  # 📝 Collect all neighbor relationships
    ...
    for gene in predicted_cancer_genes:
        ...
        # Get top neighbors with CSV output suppressed (we’ll save them all later)
        neighbor_scores = get_top_neighbors(
            gene,
            neighbors,
            name_to_index,
            ...
            relevance_scores,
            node_id_to_name,
            output_dir=None,  # ← avoid per-gene CSVs
            k=10
        )
        ...
        # Save this gene's neighbors to the shared list
    ...
    # ✅ Save all confirmed gene neighbors to a single CSV
    combined_csv_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_bio_all_confirmed_neighbors_epo{args.num_epochs}.csv"
    )
...
# Shared function to extract top neighbors by relevance

def get_top_neighbors_ori(
    gene,
    neighbors,
    name_to_index,
    topk_name_to_index,
...
def get_top_neighbors(
    ...
    if output_dir is not None:
        # os.makedirs(output_dir, exist_ok=True)
        file_path = os.path.join(output_dir, f"{gene}_top_neighbors.csv")
        with open(file_path, "w", newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["Neighbor_Index", "Neighbor_Name", "Relevance_Score"])
            for idx, score in top_k.items():
                writer.writerow([idx, node_id_to_name.get(idx, "UNK"), score])
        print(f"📝 Saved neighbors of {gene} to {file_path}")
...
# Modify both Sankey and Bar plot code to use this shared method:
# 1. In plot_collapsed_clusterfirst_multilevel_sankey_bio():
# Replace:
#     neighbor_scores = {}
#     for n in neighbors: ...
# With:
...
# 2. In plot_confirmed_neighbors_bio():
# Replace:
#     neighbor_scores_dict = {}
#     for n in neighbors: ...
# With:
#     neighbor_scores_dict = get_top_neighbors(
#         gene,
#         neighbors,
#         name_to_index,
#         topk_name_to_index,
...
# Then pass `neighbor_scores_dict` to plot_neighbor_relevance() directly
# to ensure the bar plot and Sankey plot use the exact same data.
...
def train(args):
    ...
    for gene in confirmed_genes:
        plot_neighbor_relevance_by_mode(
            ...
            relevance_scores=relevance_scores_bio,
            mode='BIO',
            neighbor_scores=relevance_scores,
