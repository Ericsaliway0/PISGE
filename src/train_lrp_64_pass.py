...
def plot_lrp_cluster_heatmap_no_color_pass(
    all_cluster_scores: torch.Tensor,
    feature_names: list,
    cluster_tensor: torch.Tensor,
    output_dir: str,
    ...
    scores_np = scores_np[:, :len(feature_names)]  # Trimming the scores_np if needed
    feature_names_trimmed = feature_names[:scores_np.shape[1]]  # Trimming feature names to match the shape of scores_np
...
def train(args):
    ...
    best_k = find_optimal_k(node_features, k_range=(2, 16))
...
def find_optimal_k(node_features, k_range=(2, 16)):
    """
...
def plot_spectral_biclustering_heatmap(
    ...
    ax = fig.add_subplot(gs[:-1, :18])       # heatmap: cols 0–17
    ax_bar = fig.add_subplot(gs[:-1, 18:20]) # bar:     cols 18–19
    ax_legend = fig.add_subplot(gs[-1, :])   # legend:  bottom row, full width
