...
def train(args):
    ...
    # Build matrix with -log10(p-value), filter long names and p < 0.05
    for cluster_type in ['bio', 'topo']:
    ...
    # Fill NaNs with 0 and filter by max -log10(p-value) > 1
    heatmap_data = heatmap_data.fillna(0)
    heatmap_data = heatmap_data[heatmap_data.max(axis=1) > 1]

    # Select 50 rows evenly
    ...
    # Color-code columns by cluster type
    col_colors = pd.Series(heatmap_data.columns).map(lambda x: 'cornflowerblue' if x.startswith('bio') else 'salmon')

    # === Split by type ===
    bio_cols = [col for col in heatmap_data.columns if col.startswith('bio')]
    topo_cols = [col for col in heatmap_data.columns if col.startswith('topo')]

    bio_data = heatmap_data[bio_cols]
    topo_data = heatmap_data[topo_cols]

    # === Plot side by side ===
    fig, axs = plt.subplots(1, 2, figsize=(0.5 * (len(bio_cols) + len(topo_cols)), 0.3 * len(heatmap_data)), sharey=True)
    ...
    axs[0].set_title("BIO Clusters", fontsize=14)
    axs[0].set_xlabel("Cluster", fontsize=12)
    axs[0].set_ylabel("Pathway (Source)", fontsize=12)
    ...
    axs[1].set_title("TOPO Clusters", fontsize=14)
    axs[1].set_xlabel("Cluster", fontsize=12)
    axs[1].set_ylabel("")  # y-axis shared with bio
    axs[1].tick_params(axis='x', labelrotation=90)
    axs[1].set_yticklabels([])  # Don't repeat y-labels
    sns.despine(ax=axs[1], trim=True)

    # === Custom legend ===
    legend_patches = [
        Patch(facecolor='cornflowerblue', label='BIO'),
        Patch(facecolor='salmon', label='TOPO')
    ]
