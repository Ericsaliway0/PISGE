...
from src.utils import (choose_model, plot_all_roc_curves, plot_roc_curve, plot_pr_curve, load_graph_data, 
...
def train(args):
    ...
    all_fprs, all_tprs, all_aucs, cancer_names = [], [], [], []
    ...
    for cancer_type in cancer_types:
        ...
        # Call KDE + ROC/PR function and collect ROC data
    ...
    combined_roc_path = 'results/gene_prediction/all_cancers_combined_roc_curve.png'
    plot_all_roc_curves(all_fprs, all_tprs, all_aucs, cancer_names, combined_roc_path)



def plot_auc_distributions_per_cancer(all_auc_values, cancer_names, output_path):
    """
    Creates a boxplot of AUC distributions per cancer type.
    
    all_auc_values: list of lists, each sublist contains AUCs for one cancer type
    cancer_names: list of names matching all_auc_values
    output_path: path to save the final figure
    """

    # Flatten data for seaborn boxplot
    data = []
    for auc_list, cancer in zip(all_auc_values, cancer_names):
        for auc in auc_list:
            data.append({'Cancer': cancer, 'AUC': auc})

    df = pd.DataFrame(data)

    # Set color palette (can adjust for different aesthetics)
    palette = sns.color_palette("Spectral", n_colors=len(cancer_names))

    plt.figure(figsize=(18, 6))
    sns.boxplot(data=df, x='Cancer', y='AUC', palette=palette)
    plt.xticks(rotation=90)
    plt.title("All Test Datasets – AUC Distributions", fontsize=14, weight='bold')
    plt.xlabel("All Test Patients – # of Predicted Driver Genes", fontsize=12)
    plt.ylabel("AUC", fontsize=12)
    plt.tight_layout()
    
    plt.savefig(output_path, dpi=300)
    plt.show()
    print(f"AUC boxplot saved to {output_path}")
