...
def train(args):
    ...
    # graph_bio, row_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio_initial(
    #     graph=graph,
    ...
    # graph_bio, row_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio_trained(
    #     graph=graph,
    ...
    # Assign cluster labels
    #graph.ndata['cluster'] = torch.tensor(row_labels_bio)
    ...
    # === TOPO BICLUSTERING ===
    # graph_topo, row_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo_initial(
    #     graph=graph,
    #     #topo_embeddings=topo_feats,
    ...
    # graph_topo, row_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo_trained(
    #     graph=graph,
    #     #topo_embeddings=topo_feats,
    ...
    # Assign cluster labels
    #graph.ndata['cluster'] = torch.tensor(row_labels_topo)
    ...
    # Plot known cancer gene percentages
    kcg_counts_topo = {
        i: sum((np.array(row_labels_topo) == i) & np.isin(range(len(row_labels_topo)), list(gt_indices)))
        for i in range(best_k)
    }
    ...
    for cluster_type, row_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            '''plot_topo_biclustering_heatmap_clusters_unsort(
                args=args,
                relevance_scores=relevance_scores_topk.detach().cpu().numpy(),
                row_labels=row_labels_topk.cpu().numpy(),
                gene_names=node_names_topk,
                output_path= heatmap_path_clusters_unsort,
                col_labels=col_labels_topo
            )'''
