...
CLUSTER_COLORS = {
    ...
    4: '#6F2DBD',  5: '#FF7F0E',  6: '#32CD32',  7: '#FFD700',
    8: '#8A2BE2',  9: '#E377C2', 10: '#FF1493', 11: '#A52A2A'
}
...
def train(args):
    ...
    output_path_genes_clusters = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_predicted_clusters.png')
    ...
    output_path_percent = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_cluster_ground_truth.png')
    ...
    output_path_predicted = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_cluster_predicted.png')
...
def plot_tsne_with_clusters(data, cluster_labels, cluster_colors, output_path):
    """
    Plots a t-SNE visualization with clusters colored according to the given cluster_colors.

    Args:
        data (pd.DataFrame or np.ndarray): Data for t-SNE transformation, should have shape (n_samples, n_features).
        cluster_labels (list or np.ndarray): Array of cluster labels for each data point.
        cluster_colors (list): List of colors corresponding to each cluster.
        output_path (str): Path to save the t-SNE plot.
    """
    ...
    # Create scatter plot, using cluster colors
    sns.scatterplot(x='TSNE1', y='TSNE2', hue='Cluster', palette=cluster_colors,
                    data=tsne_df, s=100, edgecolor=None, legend=None)

    # Labels and title
    plt.title("t-SNE with Clusters", fontsize=16)
...
# Define cluster colors (You can modify this based on your preferences)
CLUSTER_COLORS = [
    '#0077B6', '#0000FF', '#00B4D8', '#48EAC4', '#6F2DBD', 
    '#FF7F0E', '#32CD32', '#FFD700', '#8A2BE2', '#E377C2', 
    '#FF1493', '#A52A2A'
...
def plot_predicted_cancer_genes(clusters, predicted_cancer_genes_count, total_genes_per_cluster, node_names, cluster_labels, output_path, cluster_colors):
    """
...
def plot_predicted_cancer_genes_ground_truth(clusters, predicted_cancer_genes_count, total_genes_per_cluster, ground_truth_cancer_genes, node_names, cluster_labels, output_path, cluster_colors):
    """
