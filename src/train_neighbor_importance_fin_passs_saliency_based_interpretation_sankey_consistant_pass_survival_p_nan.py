...
def plot_ridge_violin_by_cluster_(
    ...
    if save_path:
        plt.savefig(save_path, dpi=300)
        print(f"✅ Ridge violin plot saved to: {save_path}")
...
def plot_ridge_violin_by_cluster(
    ...
    print("🔄 Melting scores into long-form dataframe...")
    melt_rows = []
...
def plot_survival_by_cluster(cluster_assignments, survival_file, save_path=None):
    ...
    Args:
        cluster_assignments (dict): sample_id (TCGA barcode) → cluster_id
...
def train__(args):
    ...
    print(f"✅ Expression matrix saved to: {expression_output_path}")
    ...
    survival_data_path = 'data/TCGA-PAAD.survival.tsv'
...
def run_survival_analysis_by_cluster(row_labels_bio, node_names_topk, survival_file, expression_file, output_dir):
...
def train(args):
    ...
    run_survival_analysis_by_cluster(
        ...
        node_names_topk=node_names_topk,
        survival_file="data/TCGA-PAAD.survival.tsv",
        expression_file="data/TCGA-PAAD.expression.tsv",
