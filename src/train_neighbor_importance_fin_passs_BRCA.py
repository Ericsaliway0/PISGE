...
def plot_top_gene_ridge_from_specific_omics_cutoff(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path,
    ...
    for cluster in clusters:
        ...
        g.map(sns.kdeplot, "Value", bw_adjust=0.8, fill=True, alpha=0.8)
        g.map(sns.kdeplot, "Value", bw_adjust=0.8, color="black", lw=1)
        ...
        g.fig.set_size_inches(8, g.fig.get_size_inches()[1])  # Make figure wider (default ~6–7)
...
def plot_top_gene_ridge_from_specific_omics(
    bio_features_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
    omics_target='cna',
    ...
    def extract_specific_omics_cancer(bio_features_np, omics_target='mf', cancer_target='BRCA'):
        omics_types = ['cna', 'ge', 'meth', 'mf']
        cancer_types = ['BLCA', 'BRCA', 'CESC', 'COAD', 'ESCA', 'HNSC', 'KIRC', 'KIRP',
                        'LIHC', 'LUAD', 'LUSC', 'PRAD', 'READ', 'STAD', 'THCA', 'UCEC']
        o_idx = omics_types.index(omics_target)
        ...
        return bio_features_np[:, start:end]

    print(f"🔍 Extracting 16 features for {omics_target.upper()}-{cancer_target}...")
    specific_features = extract_specific_omics_cancer(bio_features_np, omics_target, cancer_target)  # [num_genes, 16]

    # Prepare DataFrame
    df = pd.DataFrame(specific_features)
    df["Gene"] = node_names
    df["Cluster"] = cluster_labels
    df["MeanFeatureValue"] = df.iloc[:, :-2].mean(axis=1)
    ...
    for cluster in clusters:
        ...
        # Get global min/max for consistent x-axis, with buffer
        xmin = plot_df["Value"].min() - 0.5
        xmax = plot_df["Value"].max() + 0.5

        # Pad each gene's feature values with xmin and xmax
        pad_df = []
        for gene in gene_order:
            pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmin})
            pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmax})
        ...
        g.set_titles("")
        g.set(xlabel="Feature Value", ylabel="", yticks=[])

        # Add gene names to y-axis
        for ax, gene in zip(g.axes.flat, gene_order):
            ax.set_ylabel(gene, rotation=0, ha='right', va='center', fontsize=12, labelpad=10)
        ...
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=0.3, left=0.3, top=0.93)
        g.fig.set_size_inches(10, len(gene_order) * 0.5 + 1)  # wider and taller

        # Save plot
        out_path = os.path.join(output_path, f"{omics_target.upper()}-{cancer_target}_cluster_{cluster}_top{top_n}_genes_ridge.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✅ Ridge plot saved: {out_path}")

def plot_ridge_all_omics_for_cancer(
    bio_features_np,
    node_names,
    cluster_labels,
    output_path,
    ...
    for omics_target in ['cna', 'ge', 'meth', 'mf']:
        print(f"\n📊 Plotting for {omics_target.upper()} - {cancer_target}...")
        plot_top_gene_ridge_from_specific_omics(
            bio_features_np=bio_features_np,
            node_names=node_names,
            cluster_labels=cluster_labels,
            output_path=output_path,
            omics_target=omics_target,
            cancer_target=cancer_target,
            top_n=top_n
        )
...
def train(args):
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            plot_ridge_all_omics_for_cancer(
                bio_features_np=bio_embeddings_np,
                node_names=node_names,
                cluster_labels=cluster_labels,
