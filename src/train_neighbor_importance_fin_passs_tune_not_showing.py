...
def apply_full_spectral_biclustering_bio_ori(graph, summary_bio_features, node_names,
                                         predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap):
...
def apply_full_spectral_biclustering_topo_ori(graph, summary_topo_features, node_names,
                                          predicted_cancer_genes, n_clusters,
                                          save_path, save_cluster_labels_path,
                                          save_total_genes_per_cluster_path, save_predicted_counts_path,
                                          output_path_genes_clusters, output_path_heatmap):
...
def plot_bio_biclustering_heatmap_ori(
    args,
    relevance_scores,
    cluster_labels,
    omics_splits,
    ...
    # 🔹 Remove frame around the feature contribution bar
    ax_bar.set_facecolor("none")  # transparent background
    ...
    # 🔹 LRP curve
    lrp_sums = sorted_scores.sum(axis=1)
    lrp_sums = lrp_sums / lrp_sums.max()  # No mean subtraction
    lrp_sums *= 0.9  
...
def plot_bio_heatmap_raw_no_omics_name(features, predicted_indices, save_path):
    import matplotlib.pyplot as plt
    import seaborn as sns
    import numpy as np

    plt.figure(figsize=(10, 6))
    ax = sns.heatmap(features, cmap="RdBu_r", center=0,
                     yticklabels=False, cbar_kws={"label": "Feature Value"})

    # Mark predicted cancer genes (optional red bars on the side)
    for idx in predicted_indices:
        ax.add_patch(plt.Rectangle((0, idx), features.shape[1], 1,
                                   fill=False, edgecolor='black', lw=0.5))

    plt.title("Unclustered Bio Feature Heatmap")
    plt.xlabel("64 Bio Summary Features")
    plt.ylabel("Genes")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()

def plot_bio_heatmap_raw_color_coolwarm(features, predicted_indices, save_path):
    import matplotlib.pyplot as plt
    import seaborn as sns
    import numpy as np
    ...
    # Define omics group splits and labels
    group_labels = ['Methylation', 'Expression', 'Mutation', 'CNV']
    group_sizes = [16, 16, 16, 16]
    group_boundaries = np.cumsum(group_sizes)[:-1]

    # Create omics group color bar
    group_colors = ['#9ECAE1', '#A1D99B', '#FDAE6B', '#DADAEB']  # pastel blue, green, orange, purple

    # Plot the heatmap
    ax = sns.heatmap(features, cmap="RdBu_r", center=0, yticklabels=False, xticklabels=False,
                     cbar_kws={"label": "Feature Value"})

    # Draw vertical lines to separate omics groups
    for boundary in group_boundaries:
        ax.axvline(x=boundary, color='black', lw=1)

    # Add omics group labels manually
    for i, (label, size) in enumerate(zip(group_labels, group_sizes)):
        center = sum(group_sizes[:i]) + size / 2
        ax.text(center, -features.shape[0]*0.01, label, ha='center', va='bottom', fontsize=9, fontweight='bold')

    # Highlight predicted gene rows with black rectangle
    for idx in predicted_indices:
        ax.add_patch(plt.Rectangle((0, idx), features.shape[1], 1,
                                   fill=False, edgecolor='black', lw=0.5))

    plt.title("Unclustered Bio Feature Heatmap with Omics Groups", fontsize=12)
    plt.xlabel("64 Bio Summary Features")
    plt.ylabel("Genes")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
...
def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names,
    ...
    plot_tsne(summary_bio_features, row_labels, predicted_indices, n_clusters, output_path_genes_clusters)
    plot_bio_heatmap_sorted(summary_bio_features, row_labels, col_labels, predicted_indices, output_path_heatmap)
...
def plot_bio_topo_saliency(bio_scores, topo_scores, title="", save_path=None):
    """
    Plot saliency relevance for bio and topo features with mean values in a color-patched legend.
    Applies nonlinear scaling to emphasize low values and compress high values.
    """
    import numpy as np
    import matplotlib.pyplot as plt
    from matplotlib.patches import Patch
    ...
    # 🔹 Apply transformation to emphasize small values
    bio_scores_scaled = np.sqrt(bio_scores_norm)
    topo_scores_scaled = np.sqrt(topo_scores_norm)
    ...
    plt.ylim(0, 1)  # Still normalized after sqrt
    plt.xlabel("Feature Index (0–1023)", fontsize=12)
    plt.ylabel("Transformed Relevance Score (√Norm)", fontsize=12)
    plt.title(title, fontsize=14)
    sns.despine()
    plt.tight_layout()
...
def plot_bio_heatmap_sorted(summary_bio_features, row_labels, col_labels, predicted_indices, output_path):
    cancer_names = [
        'Bladder', 'Breast', 'Cervix', 'Colon', 'Esophagus', 'HeadNeck', 'Kidney', 'KidneyPap',
        'Liver', 'LungAd', 'LungSc', 'Prostate', 'Rectum', 'Stomach', 'Thyroid', 'Uterus'
    ]
    ...
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=-2, vmax=2,
                ax=ax_heatmap, cbar=False)
    ax_heatmap.set_title("Heatmap of Summary Bio Features Sorted by Cluster Labels", fontsize=18)
    ax_heatmap.set_yticks([])
    ...
    fig.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"✅ Sorted summary bio heatmap saved to {output_path}")
...
def plot_bio_heatmap_unsort(
    ...
    gs = fig.add_gridspec(
        nrows=3, ncols=2,
        height_ratios=[1, 17, 2],  # omics bar, heatmap, legend
        width_ratios=[0.25, 13.7],
        hspace=0.3, wspace=0.00
    )

    ax_omics = fig.add_subplot(gs[0, 1])      # omics summary color bar
    ax_cluster = fig.add_subplot(gs[1, 0])    # cluster color bar (left)
    ...
    # === Omics summary bar ===
    omics_colors = {'cna': '#9370DB', 'ge': '#228B22', 'meth': '#00008B', 'mf': '#b22222'}
    omics_labels = [name.split('_')[0] for name in bio_feat_names_64]
    omics_rgb = [to_rgb(omics_colors[o]) for o in omics_labels]
    omics_rgb_array = np.array(omics_rgb).reshape(1, len(omics_rgb), 3)
    ax_omics.imshow(omics_rgb_array, aspect='auto')
    ax_omics.set_xticks([])
    ax_omics.set_yticks([])
    ax_omics.axis('off')
    ...
    # === X-axis (cancer) tick labels and coloring ===
    xticks = np.arange(len(bio_feat_names_64)) + 0.5
    ax_heatmap.set_xticks(xticks)
    cancer_labels_only = [name.split('_')[1] for name in bio_feat_names_64]
    ax_heatmap.set_xticklabels(cancer_labels_only, rotation=90, fontsize=18)
...
'''
    ...
    if len(bio_scores) > 0 and len(topo_scores) > 0:
        ...
        plot_bio_topo_saliency_cuberoot(
            ...
            topo_means,
            title="Average Feature Relevance for Confirmed Genes (Bio vs Topo)",
            save_path=os.path.join(save_dir, f"{args.model_type}_{args.net_type}_bio_topo_comparison_lineplot_log_{args.num_epochs}.png")
        )
