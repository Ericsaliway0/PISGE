...
def plot_spectral_biclustering_heatmap(
    ...
    output_path,
    omics_colors=None,
    gene_names=None  # <-- Required for per-gene plots
):
    import numpy as np
    import matplotlib.pyplot as plt
    import seaborn as sns
    ...
    # Add your extract function here or import it
    relevance_scores = extract_summary_features_np(relevance_scores)
    ...
    # 🔹 Save cluster-wise feature contributions
    plot_clusterwise_feature_contributions(
    ...
    # 🔹 Save individual gene contribution plots
    if gene_names is not None:
        per_gene_dir = os.path.join(os.path.dirname(output_path), "per_gene_feature_contributions")
        os.makedirs(per_gene_dir, exist_ok=True)
        for gene, vec in zip(gene_names, relevance_scores):
            plot_gene_feature_contributions_2(
                gene_name=gene,
                relevance_vector=vec,
                feature_names=feature_names,
                output_path=os.path.join(per_gene_dir, f"{gene}_features.png")
            )

    return pd.DataFrame(relevance_scores, index=gene_names, columns=feature_names)


def plot_gene_feature_contributions(relevance_df, output_path, omics_colors=None):
    """
    Plot heatmap of gene-level feature contributions.

    Parameters:
    - relevance_df: pd.DataFrame, with gene names as index, features as columns
    - output_path: str, path to save the plot
    - omics_colors: dict, mapping of omics prefixes to colors
    """
    # Default omics color mapping
    if omics_colors is None:
        omics_colors = {
            'MF': '#D62728',     # Mutation Features
            'CNA': '#1F77B4',    # Copy Number Alteration
            'METH': '#2CA02C',   # Methylation
            'GE': '#9467BD'      # Gene Expression
        }
    ...
    df_long = relevance_df.reset_index().melt(id_vars='index', var_name='Feature', value_name='Contribution')
    df_long.rename(columns={'index': 'Gene'}, inplace=True)

    # Split feature into Omics and CancerType (robust split with underscore handling)
    ...
    df_long[['Omics', 'CancerType']] = df_long['Feature'].apply(lambda x: pd.Series(split_feature(x)))
    ...
    sns.boxplot(
        ...
        x='Omics',
        y='Contribution',
        palette=omics_colors,
        showfliers=False
    )

    plt.title("Gene Feature Contributions by Omics Type", fontsize=16)
    plt.xlabel("Omics Type", fontsize=14)
    ...
    plt.close()
    print(f"✅ Saved gene feature contribution plot to {plot_path}")
...
def plot_gene_feature_contributions_2(gene_name, relevance_vector, feature_names, output_path=None):
    """
...
def plot_gene_feature_contributions_1(
    gene_name,
    relevance_scores,
    gene_names,
    feature_names,
...
def plot_gene_feature_contributions_x(
    gene_name,
    relevance_vector,
    feature_names,
    output_path
    ...
    """
    Plot feature contributions for a given gene using a barplot.

    Args:
        gene_name (str): Name of the gene.
        relevance_vector (np.ndarray): Vector of relevance scores for the gene (length 64).
        feature_names (list): List of feature names (length 64), e.g., ['BRCA_mf', ..., 'KIRP_meth'].
        output_path (str): Path to save the plot.
    """
    # Create DataFrame from relevance vector and feature names
    df = pd.DataFrame({
        'Feature': feature_names,
        'Contribution': relevance_vector
    })

    # Filter out rows that don't have the expected format (exactly one underscore)
    valid_split = df['Feature'].str.count('_') == 1
    df_valid = df[valid_split].copy()

    # Optional: log malformed features
    invalid_features = df[~valid_split]['Feature'].tolist()
    if invalid_features:
        print(f"⚠️ Warning: {len(invalid_features)} malformed features skipped for {gene_name}:")
        for f in invalid_features:
            print(f"  - {f}")

    # Split 'Feature' into 'Cancer' and 'Omics'
    df_valid[['Cancer', 'Omics']] = df_valid['Feature'].str.split('_', expand=True)
    ...
    plt.title(f'Feature Contributions for {gene_name}')
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
    print(f"✅ Saved feature contribution plot for {gene_name} to {output_path}")
...
def save_and_plot_confirmed_genes_not_real_cancer_name(
    node_names_topk,
...
def save_and_plot_confirmed_genes(
    ...
    # Replace technical feature names with readable cancer + omics format
    omics_order = ['mf', 'cna', 'ge', 'meth']
    cancer_names = [
        'Breast', 'Kidney', 'Rectum', 'Prostate', 'Stomach', 'HeadNeck', 'LungAd', 'Thyroid',
        'Bladder', 'Esophagus', 'Liver', 'Uterus', 'Colon', 'LungSc', 'Cervix', 'KidneyPap'
    ]
    pretty_feature_names = [f"{cancer} ({omics.upper()})" for omics in omics_order for cancer in cancer_names]
    ...
    for gene_name in confirmed_genes:
        ...
        plot_gene_feature_contributions_2(
            gene_name=gene_name,
            relevance_vector=relevance_vector,
            feature_names=pretty_feature_names,  # use pretty names here
            output_path=plot_path
        )
