...
def plot_bio_topo_saliency_cuberoot_(bio_scores, topo_scores, title="", save_path=None):
    """
    ...
    plt.legend(handles=legend_handles, loc='upper left', frameon=False, fontsize=18)

    # 🔹 Formatting
    plt.xlim(0, len(bio_scores) - 1)
    plt.ylim(0, 1)
    plt.xlabel("Feature Index (0–1023)", fontsize=18)
    plt.ylabel("Cube Root Transformed Score", fontsize=18)
...
def plot_bio_topo_saliency_cuberoot(bio_scores, topo_scores, title="", save_path=None):
    ...
    plt.xlim(0, len(bio_scores) - 1)
    plt.ylim(0, 1)
    plt.xlabel("Feature Index (0 – 63)", fontsize=18)
    plt.ylabel("Cube Root Transformed Score", fontsize=18)
    plt.title(title, fontsize=14)
...
def plot_bio_topo_saliency_log(bio_scores, topo_scores, title="", save_path=None):
    ...
    plt.legend(handles=legend_handles, loc='upper right', frameon=False, fontsize=18)
...
def compute_relevance_scores_ori(model, graph, features, node_indices=None, use_abs=True):
    """
...
def eigengap_analysis(feature_matrix, max_clusters=25, normalize=True, plot_path=None):
    """
    Perform eigengap analysis using the RBF kernel and Laplacian eigenvalues.

    Args:
        feature_matrix (ndarray): Input data matrix (n_samples x n_features).
        max_clusters (int): Maximum number of clusters to consider.
        normalize (bool): Whether to standardize the features.
        plot_path (str or None): If provided, saves eigengap plot and eigenvalues.

    Returns:
        best_k (int): Estimated optimal number of clusters.
        eigenvals (ndarray): Computed eigenvalues.
    """
    if normalize:
        from sklearn.preprocessing import StandardScaler
        feature_matrix = StandardScaler().fit_transform(feature_matrix)

    # Step 2: Compute similarity matrix using RBF kernel
    similarity = rbf_kernel(feature_matrix, gamma=0.5)

    # Step 3: Compute normalized Laplacian
    L, _ = csgraph.laplacian(similarity, normed=True, return_diag=True)
    ...
    if plot_path:
        ...
        plt.plot(x_vals, y_vals, color='blue', linestyle='-', label='Eigenvalues')
        plt.plot(x_vals, y_vals, color='red', marker='+', linestyle='None')
        plt.axvline(x=best_k, color='pink', linestyle='--', label=f'Eigengap → k={best_k}')
        plt.xlabel("Eigenvalue Index", fontsize=12)
        plt.ylabel("Eigenvalue", fontsize=12)
        plt.title("Eigengap Analysis", fontsize=14)
        plt.xticks(range(1, max_clusters + 1))
...
def getAffinityMatrix(coordinates, k=7):
    """
    Compute self-tuning affinity matrix using local scaling.

    References:
        - Zelnik-Manor & Perona (2004): https://papers.nips.cc/paper/2619
    """
    dists = squareform(pdist(coordinates))
    knn_distances = np.sort(dists, axis=0)[k]
    knn_distances = knn_distances[np.newaxis].T
    local_scale = knn_distances.dot(knn_distances.T)
    affinity_matrix = np.exp(-dists**2 / local_scale)
    affinity_matrix[np.isnan(affinity_matrix)] = 0.0
    np.fill_diagonal(affinity_matrix, 0)
    return affinity_matrix


def getAdjacencyMatrix(coordinates, delta=0.3):
    """
    Construct adjacency matrix using correlation distance and delta threshold.
    """
...
def eigenDecomposition(A, topK=5):
    """
    Perform eigen decomposition on the normalized Laplacian and apply eigengap heuristic.

    Args:
        A (ndarray): Affinity or adjacency matrix.
        topK (int): Number of top gaps to inspect.

    Returns:
        nb_clusters (ndarray): Array of optimal cluster counts suggested by top eigengaps.
...
# k_row, evals_row, evecs_row = eigenDecomposition(affinity_matrix)
# print(f'Optimal number of row clusters: {k_row}')

# affinity_matrix_cols = getAdjacencyMatrix(high_feat_contrib_normed.T, delta=1.054)
# k_col, evals_col, evecs_col = eigenDecomposition(affinity_matrix_cols)
# print(f'Optimal number of column clusters: {k_col}')

# # Visualization
# fig = plt.figure(figsize=(14, 5))
# plt.subplot(1, 2, 1)
# sns.scatterplot(x=np.arange(len(evals_row)), y=evals_row, label='Eigenvalues Rows', s=100, color='black', alpha=0.6)
# plt.title("Row Eigenvalues")
# plt.legend(loc='lower right')

# plt.subplot(1, 2, 2)
# sns.scatterplot(x=np.arange(len(evals_col)), y=evals_col, label='Eigenvalues Columns', s=100, color='black', alpha=0.6)
# plt.title("Column Eigenvalues")
# plt.legend(loc='lower right')
...
def plot_bio_clusterwise_feature_contributions(
    ...
    for cluster_id in sorted(unique_clusters):
        ...
        ax.set_xticks(np.arange(len(ordered_feature_names)) + 0.5)
...
def plot_bio_biclustering_heatmap_unsort_ori(
    args,
    relevance_scores,
    cluster_labels,
    omics_splits,
...
def best_balanced_biclustering_ori(data, n_clusters, n_trials=20):
    best_model = None
...
def compute_relevance_scores(model, graph, features, node_indices=None, method="saliency", use_abs=False, baseline=None, steps=50):
    """
    Computes relevance scores for selected nodes using either saliency (gradients) or integrated gradients (IG).
...
def plot_topo_biclustering_heatmap_unsort_no_top_bars(
    args,
    relevance_scores,
    output_path
):
    ...
    gs = fig.add_gridspec(nrows=15, ncols=40, wspace=0.0, hspace=0.0)

    ax        = fig.add_subplot(gs[1:13, 2:35])
    ax_curve  = fig.add_subplot(gs[1:13, 35:37], sharey=ax)
    ax_cbar   = fig.add_subplot(gs[5:9, 38])
...
def plot_topo_biclustering_heatmap_unsort(
    args,
    relevance_scores,
    output_path,
    #gene_names=None
    ...
    # 🔹 Top bar axis
    ax_bar    = fig.add_subplot(gs[0, 2:35])
    ax        = fig.add_subplot(gs[1:13, 2:35])
    ax_curve  = fig.add_subplot(gs[1:13, 35:37], sharey=ax)
    ax_cbar   = fig.add_subplot(gs[5:9, 38])

    # 🔹 Top bar: mean saliency per feature
    feature_means = relevance_scores.mean(axis=0)
    feature_means = (feature_means - feature_means.min()) / (feature_means.max() - feature_means.min()) * 0.04
    ax_bar.bar(np.arange(len(feature_means)) + 0.5, feature_means, width=1.0, color="#B0BEC5", alpha=0.6)
    ax_bar.set_xticks([]), ax_bar.set_yticks([]), ax_bar.set_xlim(0, len(feature_means)), ax_bar.set_ylim(0, 0.04)
...
def plot_bio_biclustering_heatmap_unsort(
    ...
    for omics in omics_order:
        start, end = omics_splits[omics]
        group_center = (start + end) / 2 + 0.5
        mean_val = relevance_scores[:, start:end+1].mean()
        norm_mean = (mean_val - np.min(feature_means)) / (np.max(feature_means) - np.min(feature_means) + 1e-6)
        ax.bar(
            x=group_center,
            height=0.15,
...
def best_balanced_biclustering_(data, n_clusters, min_cluster_size=10, max_attempts=500):
    """
    Keep running spectral biclustering until a valid clustering is found
    where all clusters have at least `min_cluster_size` members.
    Optionally limit with `max_attempts`.
    """
    best_model = None
    best_balance_score = float('inf')

    for attempt in tqdm(range(max_attempts), desc="Searching for balanced biclustering"):
        ...
        if counts.min() >= min_cluster_size:
            ...
            if balance_score < best_balance_score:
                best_model = model
                best_balance_score = balance_score
                break  # Found a valid model, stop loop

    if best_model is None:
        raise RuntimeError(f"No valid clustering found after {max_attempts} attempts.")
...
def compute_relevance_scores(model, graph, features, node_indices=None, use_abs=True):
    ...
    with torch.enable_grad():
        ...
        for i, idx in enumerate(tqdm(node_indices, desc="Computing relevance scores")):
            model.zero_grad()
            if features.grad is not None:
                features.grad.zero_()
...
def best_balanced_biclustering(data, n_clusters, n_trials=20, min_cluster_size=10):
    best_model = None
    best_balance_score = float('inf')
...
def train(args):
    ...
    best_relevance_matrix_bio = None
    best_trial = -1
    best_top_gene_indices = []
    best_ranking = []
    stop_training = False
    ...
    # --- Loop over trials ---
    for trial in tqdm(range(args.num_trials), desc="🔁 Trials", unit="trial"):
        if stop_training:
            break
        ...
        # --- Train model ---
        for epoch in tqdm(range(args.num_epochs), desc=f"🏋️ Trial {trial+1}", leave=False, unit="epoch"):
            model.train()
            logits = model(graph, features).squeeze()
            loss = loss_fn(logits[train_mask], labels[train_mask])
            optimizer.zero_grad()
        ...
        print('top_gene_indices----------------------------------\n',len(top_gene_indices))

        if not top_gene_indices:
            tqdm.write(f"⚠️ Trial {trial + 1}: No valid top nodes. Skipping.")
            continue

        # if len(top_gene_indices) > 1000:
        #     # --- Saliency ---
        ...
        #     omics_means = {}
        #     for omics, (start, end) in omics_splits.items():
        #         omics_means[omics] = relevance_matrix_bio[:, start:end + 1].mean()

        #     tqdm.write(f"🎯 Trial {trial+1} saliencies: {omics_means}")

        #     # --- Early stopping if MF is highest
        #     if all(omics_means['mf'] > val for key, val in omics_means.items() if key != 'mf'):
        #         tqdm.write(f"🛑 Early stop: MF saliency {omics_means['mf']:.6f} is highest")
        #         best_model_state_dict = model.state_dict()
        #         best_relevance_matrix_bio = relevance_matrix_bio.copy()
        #         best_trial = trial + 1
        #         best_top_gene_indices = top_gene_indices
        #         best_ranking = ranking[:1000]
        #         stop_training = True
        #         break
        # else:
        #     tqdm.write(f"⚠️ Trial {trial+1}: Skipped saliency – only {len(top_gene_indices)} predicted genes (<1000).")
        ...
        if len(top_gene_indices) == 1000:
            ...
            for trial in range(args.num_trials):  # e.g., args.num_trials = 5
                ...
                for epoch in tqdm(range(args.num_epochs), desc=f"Trial {trial+1} Training", unit="epoch"):
                    model.train()
                    logits = model(graph, features).squeeze()
                    loss = loss_fn(logits[train_mask], labels[train_mask])
                ...
                if not top_gene_indices:
                    print("⚠️ No top predicted nodes found, skipping this trial.")
                    continue
                ...
                relevance_matrix_bio = extract_summary_features_np_bio(relevance_scores_topk_bio.detach().cpu().numpy())  # shape [num_topk, 64]

                # === 💡 Evaluate MF saliency ===
                mf_saliency = relevance_matrix_bio[:, 48:64].mean()

                print(f"✅ Trial {trial+1} MF average saliency: {mf_saliency:.6f}")
                ...
                sorted_omics = sorted(omics_means.items(), key=lambda x: x[1], reverse=True)
                top_two = [item[0] for item in sorted_omics[:1]]
                
                print('sorted_omics-------------------------------------------\n',sorted_omics)

                # --- Early stopping if MF is high
                if 'mf' in top_two:
                # if all(omics_means['mf'] > val for key, val in omics_means.items() if key != 'mf'):
                    tqdm.write(f"🛑 Early stop: MF saliency {omics_means['mf']:.6f} is at least the second highest")
                    best_model_state_dict = model.state_dict()
                    best_relevance_matrix_bio = relevance_matrix_bio.copy()
                    best_trial = trial + 1
                    best_top_gene_indices = top_gene_indices
                ...
                if mf_saliency > best_mf_saliency:
                    best_mf_saliency = mf_saliency
                    best_model_state_dict = model.state_dict()
                    best_relevance_matrix_bio = relevance_matrix_bio.copy()
                    best_epoch_stats = {
                        "trial": trial + 1,
                        "mf_saliency": best_mf_saliency,
                        "top_gene_indices": top_gene_indices,
                        "ranking": ranking[:1000]
                    }

    #print(f"\n🏆 Best trial: {best_epoch_stats['trial']} with MF saliency: {best_epoch_stats['mf_saliency']:.6f}")
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            '''plot_bio_biclustering_heatmap_unsort(
                ...
                relevance_scores=relevance_scores,
                cluster_gene_labels=cluster_gene_labels,
                feature_names=feature_names,
            ...
            plot_topo_biclustering_heatmap_unsort(
                args=args,
                relevance_scores=relevance_matrix_topo,
                output_path=heatmap_path_unsort
                #gene_names=None
