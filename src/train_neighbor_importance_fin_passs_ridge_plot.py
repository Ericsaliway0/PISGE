...
def extract_summary_features_np_bio(bio_features_np):
    ...
    for o_idx in range(4):  # 4 omics
        for c_idx in range(16):  # 16 cancers
            base = o_idx * 16 * 16 + c_idx * 16
            group = bio_features_np[:, base:base + 16]  # shape [num_nodes, 16]
            max_vals = group.max(axis=1, keepdims=True)
            summary_features.append(max_vals)
...
def plot_top_gene_ridge_from_bio_features__(
    ...
    summary_features = extract_summary_features_np_bio(bio_features_np)  # [num_genes, 64]
...
def plot_top_gene_ridge_from_bio_features_no_showing_up(
    ...
    print("🔍 Extracting 64 summary features from 1024 bio features...")
    summary_features = extract_summary_features_np_bio(bio_features_np)  # shape [num_genes, 64]
...
def plot_top_gene_ridge_from_bio_features(
    ...
    for cluster in clusters:
        ...
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=0.3, left=0.35)
        g.fig.suptitle(f"Cluster {cluster}", x=0.5, y=1.01, fontsize=18, ha='center')
...
def plot_top_gene_ridge_from_bio_features_(
    ...
    for cluster in clusters:
        ...
        # Remove default titles (row labels)
        g.set_titles("")

        # Add external y-labels
        for ax, gene in zip(g.axes.flat, gene_order):
            ax.set_ylabel(gene, rotation=0, ha='right', va='center', fontsize=12, labelpad=60)

        # Final styling
        g.set(xlabel="Feature Value (Max across 16 per Cancer)", ylabel="")
        g.set(yticks=[])
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=0.3, left=0.32)  # extra left margin for gene labels
...
def train(args):
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            # Call plotting
            '''plot_saliency_ridge_by_cluster_and_omics(
                saliency_scores=summary_bio_topk,
                feature_names=bio_feat_names,
                cluster_assignments=cluster_labels_topk,  # array of shape (num_genes,)
                output_path=saliency_ridge_path
            )'''
            ...
            for i, omics in enumerate(omics_types):
                ...
                omics_data[omics] = relevance_scores_bio[topk_node_indices_tensor, start:end].detach().cpu().numpy()

            plot_top_gene_omics_ridges_by_cluster_v2(
                omics_data=omics_data,
                node_names=node_names_topk,
                cluster_labels=cluster_labels_topk.cpu().numpy(),
                total_clusters=best_k,  # or however many clusters you have
                top_n=12,
                output_path_prefix=os.path.join(output_dir, f"{args.model_type}_{args.net_type}_epo{args.num_epochs}")
            )

            plot_top_gene_ridge_all_clusters(
                saliency_scores=summary_bio_topk,
                feature_names=bio_feat_names,
                cluster_assignments=cluster_labels_topk,
                output_path=ridge_all_clusters_path,
                top_n_genes=12
            )
