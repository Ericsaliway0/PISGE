...
def plot_feature_importance_bio(
    relevance_scores,
    feature_names,
    node_name=None,
    output_path="plots",
    ...
    if len(relevance_scores) != len(feature_names):
        raise ValueError(f"Mismatch: {len(relevance_scores)} values vs {len(feature_names)} names")
    ...
    pretty_labels = []
    for omics in omics_order:
        pretty_labels.extend([f"{cancer}" for cancer in cancer_names])
    ...
    df["cancer"] = df["feature"].apply(lambda x: x.split("_")[0])
    df["omics"] = df["feature"].apply(lambda x: x.split("_")[1])
    df["label"] = df["feature"].apply(lambda x: x.split("_")[0])  # Placeholder
    df["label"] = df.apply(lambda row: f"{row['cancer']}", axis=1)
    df["color"] = df["omics"].map(omics_colors)

    # 🔹 Sort by omics then cancer to match pretty_labels
    df["omics"] = pd.Categorical(df["omics"], categories=omics_order, ordered=True)
    df["cancer"] = pd.Categorical(df["cancer"], categories=cancer_names, ordered=True)
    df = df.sort_values(by=["omics", "cancer"]).reset_index(drop=True)

    # 🔹 Update label order
    df["label"] = [f"{cancer}" for omics in omics_order for cancer in cancer_names]
    df["color"] = df["omics"].map(omics_colors)

    # 🔹 Plot
    plt.figure(figsize=(24, 5))
    ax = sns.barplot(
        x="label", y="relevance", data=df,
        palette=df["color"].tolist()
    )
    ax.set_title(f"{node_name} (Biological)", fontsize=16)
    ax.set_ylabel("Relevance", fontsize=14)
    ax.set_xlabel("Cancer Type (Grouped by Omics)", fontsize=14)

    # 🔹 Color the x-axis tick labels
    for tick, omics in zip(ax.get_xticklabels(), df["omics"]):
        tick.set_color(omics_colors.get(omics, "#888888"))
        tick.set_fontsize(10)
