...
def train(args):
    ...
    model.eval()
    with torch.no_grad():
        train_ed_embeddings = model(graph, features, return_embeddings=True)  # shape: [num_nodes, hidden_feats]
        logits = model(graph, features).squeeze()
    ...
    bio_feats_trained = train_ed_embeddings[:, :1024]
    ...
    bio_feats_trained = graph.ndata['feat'][:, :1024]  # biological features
