...
def compute_lrp_scores(model, graph, features, node_indices=None):
    ...
    with torch.enable_grad():
        ...
        saliency_scores = torch.zeros_like(features)
        ...
        for idx in node_indices:
            ...
            saliency_scores[idx] = features.grad[idx].detach()
...
def save_reduced_feature_saliency_scores(
    saliency_scores,      # Original 2048D matrix, shape [N, 2048]
    ...
    reduced_scores = extract_summary_features_np_skip(saliency_scores)  # shape [N, 64]
...
def compute_saliency_all_nodes(model, g, features, target_classes=None):
    ...
    Returns:
        saliency_scores: Tensor of shape [num_nodes, num_features] with saliency values.
    """
    model.eval()
    if isinstance(features, np.ndarray):
        features = torch.tensor(features, dtype=torch.float32)
    ...
    for node_id in range(num_nodes):
        ...
        saliency_scores[node_id] = features.grad[node_id].abs().detach()
    ...
    for gene in confirmed_genes:
        ...
        for n in neighbors:
            if n in topk_name_to_index:
                ...
                if rel_idx < saliency_scores.shape[0]:
                    rel_score = saliency_scores[rel_idx].sum().item()
                    #if rel_score > 0.1:
...
def plot_confirmed_neighbors_topo(
    ...
    for gene in confirmed_genes:
        ...
        for n in neighbors:
            if n in topk_name_to_index:
                rel_idx = topk_name_to_index[n]
                if rel_idx < saliency_scores.shape[0]:  # bounds check
                    rel_score = saliency_scores[rel_idx].sum().item()
                    # if rel_score > 0.1:
                    neighbor_scores_dict[rel_idx] = rel_score
...
def plot_confirmed_neighbor_relevance(
    ...
    for gene in confirmed_genes:
        ...
        neighbor_scores_dict = {
            i: saliency_scores[i].sum().item()
            for i in neighbor_indices
            if saliency_scores[i].sum().item() > 0.1
        }
...
def plot_topo_clusterwise_feature_contributions(
    args,
    saliency_scores,           # 2D array (samples x features)
    ...
    for cluster_id in sorted(unique_clusters):
        ...
        cluster_scores = saliency_scores[indices]
...
def plot_neighbor_relevance_by_mode(
    ...
    relevance_vals = [saliency_scores[i].sum().item() for i in neighbor_indices]
...
def plot_saliency_for_gene(
    ...
    bio_feat_names,
    topo_feat_names):
    bio_1024 = saliency_scores[node_idx]["bio"].cpu().numpy().reshape(1, -1)
    topo_1024 = saliency_scores[node_idx]["topo"].cpu().numpy().reshape(1, -1)
...
def plot_clusterwise_sorted_heatmaps(
    args,
    saliency_scores,          # [N, F] relevance scores for top genes
    ...
    for cluster_id in range(num_clusters):
        ...
        cluster_scores = saliency_scores[cluster_indices]  # [n_genes_cluster, F]
...
def compute_node_saliency(model, graph, features, node_indices=None, use_abs=True, normalize=True):
    ...
    Returns:
        saliency_scores: Tensor of shape [num_nodes, num_features]
    """
    model.eval()
    ...
    with torch.enable_grad():
        ...
        for i, idx in enumerate(node_indices):
            ...
            saliency_scores[idx] = relevance.detach()
...
def plot_neighbor_saliency_heatmap(
    ...
    for gene in confirmed_genes:
        ...
        gene_relevance = saliency_scores[idx].cpu().numpy()
        ...
        for neighbor_idx in neighbors:
            ...
            neighbor_relevance = saliency_scores[neighbor_idx].cpu().numpy()
...
def compute_combined_saliency_scores(
    model,
    graph,
    features,
    node_indices=None,
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            if gene not in label_to_idx:
                ...
                saliency = saliency_scores[rel_idx].sum().item()
            ...
            for neighbor_idx, neighbor_score in neighbors.items():
                ...
                if neighbor_name not in label_to_idx:
                    ...
                    saliency = saliency_scores[neighbor_idx].sum().item()
...
def compute_saliency_scores(model, graph, features, node_indices=None, use_abs=True):
    """
    ...
    Returns:
        saliency_scores: Tensor of shape [num_nodes, num_features] (0s for nodes not analyzed)
    """
    model.eval()
    if isinstance(features, np.ndarray):
        features = torch.tensor(features, dtype=torch.float32)
    ...
    with torch.enable_grad():
        ...
        for i, idx in enumerate(node_indices):
            ...
            grads = features.grad[idx]
            saliency_scores[idx] = grads.abs().detach() if use_abs else grads.detach()
...
def compute_saliency_scores_norm(
    model,
    graph,
    features,
    node_indices,
...
def apply_full_spectral_biclustering_bio_(graph, summary_bio_features, node_names, predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap,
                                         topk_node_indices=None):
    """
    Run spectral biclustering on the summary bio features (top-k genes only), assign cluster labels back to graph.
    """
    print(f"Running Spectral Biclustering on 64-dim summary bio features with {n_clusters} clusters...")
    assert summary_bio_features.shape[1] == 64, f"Expected 64 summary features, got {summary_bio_features.shape[1]}"
    ...
    # === Spectral biclustering
    bicluster = SpectralBiclustering(n_clusters=n_clusters, 
                                     method='log', 
                                     random_state=42)
    bicluster.fit(summary_bio_features)
    ...
    # === Save heatmaps and t-SNE visualizations
    output_path_unclustered_heatmap = output_path_heatmap.replace(".png", "_unclustered.png")
    '''plot_bio_heatmap_raw_unsorted(summary_bio_features, predicted_indices, output_path_unclustered_heatmap)
    plot_tsne(summary_bio_features, row_labels, predicted_indices, n_clusters, output_path_genes_clusters)
    plot_bio_heatmap_unsort(summary_bio_features, row_labels, col_labels, predicted_indices, output_path_heatmap)'''
...
def plot_topo_biclustering_heatmap_(
    ...
    Args:
        ...
        saliency_scores (np.ndarray): shape [num_nodes, 2048], full embedding.
    ...
    saliency_scores = extract_summary_features_np_topo(saliency_scores)
    # Normalize features-----------------------------------------------------------------------------------------------------------------
    # saliency_scores = StandardScaler().fit_transform(saliency_scores)*10
    ...
    feature_names = [f"{i+1:02d}" for i in range(saliency_scores.shape[1])]

    # 🔹 Sort columns (features) by total relevance across all genes
    col_sums = saliency_scores.sum(axis=0)
    col_order = np.argsort(-col_sums)
    saliency_scores = saliency_scores[:, col_order]
    ...
    for cluster in unique_clusters:
        ...
        cluster_scores = saliency_scores[cluster_idx]
    ...
    sorted_scores = saliency_scores[sorted_indices]
    ...
    plot_topo_clusterwise_feature_contributions(
        args=args,
        saliency_scores=saliency_scores,  # Not sorted for per-cluster breakdown
        cluster_labels=cluster_labels,
        feature_names=[f"{i+1:02d}" for i in range(saliency_scores.shape[1])],
...
def plot_bio_biclustering_heatmap_unsort_random_clusterbar(
    ...
    #saliency_scores = extract_summary_features_np_bio(saliency_scores)
    saliency_scores = (saliency_scores - saliency_scores.min()) / (saliency_scores.max() - saliency_scores.min()) * 20
    ...
    #     saliency_scores = saliency_scores[:, sorted_order]
    ...
    feature_means = saliency_scores.mean(axis=0)
    ...
    vmin = 0
    vmax = np.percentile(saliency_scores, 99)
    ...
    # LRP curve
    saliency_sums = saliency_scores.sum(axis=1)
    saliency_sums = (saliency_sums - saliency_sums.min()) / (saliency_sums.max() - saliency_sums.min())
    y = np.arange(len(saliency_sums))
    ax_curve.fill_betweenx(
        y, 0, saliency_sums,
    ...
    omics_means = {}
    for omics, (start, end) in omics_splits.items():
        group_scores = saliency_scores[:, start:end+1]
    ...
    for i, omics in enumerate(omics_order):
        ax.bar(
            ...
            bottom=len(saliency_scores) + 1.5,
...
def plot_bio_biclustering_heatmap_clusters_unsort_(
    ...
    sorted_scores = saliency_scores[cluster_order]
    sorted_clusters = cluster_labels[cluster_order]

    # Keep original feature (column) order
    original_col_indices = list(range(saliency_scores.shape[1]))
...
def plot_topo_heatmap_unsorted(
    ...
    scores_log = np.log1p(saliency_scores)
...
def plot_topo_biclustering_heatmap_unsorted_(
    ...
    '''plot_topo_clusterwise_feature_contributions(
        args=args,
        saliency_scores=saliency_scores,  # original, not sorted
        cluster_labels=cluster_labels,
        feature_names=feature_names,
        per_cluster_feature_contributions_output_dir=os.path.join(
            os.path.dirname(output_path), "per_cluster_feature_contributions_topo"
...
def plot_topo_biclustering_heatmap_clusters_unsort(
    ...
    feature_names = [f"Topo-{i}" for i in range(saliency_scores.shape[1])]
    ...
    for i, (val, color) in enumerate(zip(feature_means, feature_colors)):
        ax_bar.bar(
            x=i + 0.5, height=val, width=1.0,
            color=color, edgecolor='black', linewidth=0.5, alpha=0.3 + 0.7 * val
        )
    ...
    ax.set_xticks(np.arange(len(feature_names)) + 0.5)
    ax.set_xticklabels([f"{i}" for i in range(len(feature_names))], rotation=90, fontsize=14)
    for label, color in zip(ax.get_xticklabels(), feature_colors):
        label.set_color(color)
...
def plot_topo_biclustering_heatmap_unsorted(
    ...
    scores = np.log1p(saliency_scores)
...
def plot_bio_biclustering_heatmap_clusters_unsort(
    ...
    saliency_sums = (saliency_sums - saliency_sums.min()) / (saliency_sums.max() - saliency_sums.min())
    ax_curve.fill_betweenx(
        np.arange(len(saliency_sums)), 0, saliency_sums, 
        color='#a9cce3', 
...
def plot_bio_biclustering_heatmap(
    ...
    '''
    saliency_scores = (saliency_scores - saliency_scores.min(axis=1, keepdims=True)) / \
                    (saliency_scores.max(axis=1, keepdims=True) - saliency_scores.min(axis=1, keepdims=True) + 1e-6)'''
    # Normalize features-----------------------------------------------------------------------------------------------------------------
    ##saliency_scores = StandardScaler().fit_transform(saliency_scores)*20
    saliency_scores = (saliency_scores - saliency_scores.min()) / (saliency_scores.max() - saliency_scores.min())*20
    ...
    sorted_indices = np.argsort(cluster_labels)
    saliency_scores = saliency_scores[sorted_indices]
    cluster_labels = cluster_labels[sorted_indices]
    ...
    for cluster in unique_clusters:
        ...
        cluster_relevance_sums = saliency_scores[cluster_indices].sum(axis=1)
    ...
    sorted_scores = saliency_scores[new_order]
    ...
    return pd.DataFrame(saliency_scores, index=gene_names, columns=feature_names)

def normalize_saliency_zscore(saliency_scores, dim=0):
    """
    Z-score normalization of saliency scores along a specified dimension.
    
    Args:
        saliency_scores: Tensor of shape [num_nodes, num_features]
        dim: 0 = normalize each feature across nodes, 1 = normalize each node across features

    Returns:
        normalized_scores: Tensor of same shape, standardized
    """
    mean = saliency_scores.mean(dim=dim, keepdim=True)
    std = saliency_scores.std(dim=dim, keepdim=True) + 1e-8
    return (saliency_scores - mean) / std

def normalize_saliency_minmax(saliency_scores, dim=0):
    """
    Min-max normalization of saliency scores along a specified dimension.
    ...
    Returns:
        normalized_scores: Tensor of same shape, scaled to [0, 1]
    """
    min_vals = saliency_scores.min(dim=dim, keepdim=True)[0]
    max_vals = saliency_scores.max(dim=dim, keepdim=True)[0]
    normalized = (saliency_scores - min_vals) / (max_vals - min_vals + 1e-8)
    return normalized

def compute_saliency_scores(model, graph, features, node_indices=None, use_abs=True):
    """
    Computes saliency scores (gradient × input) for selected nodes using sigmoid outputs.

    Args:
        model: Trained GNN model
        graph: DGL graph
        features: Input node features (Tensor or numpy array)
        node_indices: List/Tensor of node indices to compute saliency for. If None, auto-select using probs > 0.5
        use_abs: Whether to return absolute saliency scores (recommended for visualization)
    ...
    with torch.enable_grad():
        ...
        # Auto-select positive-predicted nodes
        if node_indices is None:
            node_indices = torch.nonzero(probs > 0.5, as_tuple=False).squeeze()
            if node_indices.ndim == 0:
                node_indices = node_indices.unsqueeze(0)
        ...
        for i, idx in enumerate(node_indices):
            ...
            grads = features.grad[idx]
            saliency = grads * features[idx]
            saliency_scores[idx] = saliency.abs().detach() if use_abs else saliency.detach()

    return saliency_scores


def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names, predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap,
                                         topk_node_indices=None):
    ...
    Run spectral biclustering on the summary bio features (top-k genes only), assign cluster labels back to graph.
    """
    # Before using n_clusters, ensure it's a tuple
    '''if isinstance(n_clusters, int):
        n_clusters = (n_clusters, summary_bio_features.shape[1])
    elif isinstance(n_clusters, tuple) and len(n_clusters) == 1:
        n_clusters = (n_clusters, summary_bio_features.shape[1])'''
...
def apply_full_spectral_biclustering_topo(graph, summary_topo_features, node_names,
    ...
    '''if isinstance(n_clusters, int):
        ...
        n_clusters = (n_clusters, summary_topo_features.shape[1])'''
...
def train(args):
    ...
    saliency_scores = compute_saliency_scores(model, graph, features)
    saliency_scores = normalize_saliency_zscore(saliency_scores, dim=0)

    # Slice relevance scores for biological and topological embeddings
    saliency_scores_bio = saliency_scores[:, :1024]
    saliency_scores_topo = saliency_scores[:, 1024:]
    ...
    saliency_scores_topk = saliency_scores_bio[topk_node_indices_tensor]
    summary_bio_topk = extract_summary_features_np_bio(saliency_scores_topk.detach().cpu().numpy())

    print(summary_bio_topk.shape)
    saliency_matrix = summary_bio_topk
    ...
    graph, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio(
        graph=graph,
        summary_bio_features=saliency_matrix,  # shape (1000, 64)
        node_names=node_names,
    ...
    full_cluster_labels = torch.full((graph.num_nodes(),), -1, dtype=torch.long)

    # Then assign the cluster labels only to the relevant subset
    full_cluster_labels[top_gene_indices] = torch.tensor(cluster_labels_bio, dtype=torch.long)

    # Now safely assign it to the graph
    graph.ndata['cluster'] = full_cluster_labels
    ...
    for cluster_type, cluster_labels, saliency_scores_subset, tag in [
        ("bio", full_cluster_labels, saliency_matrix, "bio"),
        ("topo", cluster_labels_topo, saliency_scores_topo, "topo")
        ]:
        ...
        # Prepare for heatmap
        '''topk_node_indices_tensor = torch.tensor(top_gene_indices, dtype=torch.int64).view(-1)
        saliency_scores_topk = saliency_scores_subset[topk_node_indices_tensor]'''
        ...
        if tag == "bio":
            plot_bio_biclustering_heatmap(
                args=args,
                saliency_scores=saliency_matrix,#saliency_scores_topk.detach().cpu().numpy(),
            ...
            save_and_plot_confirmed_genes_bio(
                ...
                node_scores_topk=saliency_scores_topk.detach().cpu().numpy(),
            ...
            plot_confirmed_neighbors_bio(
                ...
                saliency_scores=saliency_scores_topk
            )     
            ...
            plot_neighbor_saliency_heatmap(
                ...
                node_names=node_names,
                name_to_index=name_to_index,
                saliency_scores=saliency_scores, # Full relevance scores (before slicing to topk)
                omics_splits=omics_splits,
            ...
            plot_topo_biclustering_heatmap(
                args=args,
                saliency_scores=saliency_scores_topk.detach().cpu().numpy(),
            ...
            # Extract 64D summary feature vectors for top-k only
            '''summary_topo_topk = extract_summary_features_np_topo(saliency_scores_topk.detach().cpu().numpy())
    ...
    for gene in confirmed_genes:
        plot_neighbor_relevance_by_mode(
            gene=gene,
            saliency_scores=saliency_scores_bio,
            mode='BIO',
        ...
        plot_neighbor_relevance_by_mode(
            gene=gene,
            saliency_scores=saliency_scores_topo,
            mode='TOPO',
            neighbor_scores=scores,
            neighbors_dict=neighbors_dict,
            name_to_index=name_to_index,
    ...
    saliency_scores = compute_saliency_scores_norm(
        model=model,
        graph=graph,
        features=features,
    ...
    for gene in confirmed_genes:
        ...
        if node_idx not in saliency_scores:
            print(f"⚠️ No relevance found for {gene} (node {node_idx})")
            continue

        if not isinstance(saliency_scores[node_idx], dict) or \
        "bio" not in saliency_scores[node_idx] or "topo" not in saliency_scores[node_idx]:
    ...
    for node_id, group_saliency in saliency_scores.items():
