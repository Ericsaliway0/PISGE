...
def save_and_plot_confirmed_genes_ori_(
    args,
    node_names_topk,
    node_scores_topk,
...
def plot_bio_biclustering_heatmap_feature_name_as_signiture(
    args,
    relevance_scores,
    cluster_labels,
    feature_names,
...
def plot_topo_biclustering_heatmap(
    ...
    ##feature_names = [f"TOPO_{i+1}" for i in range(relevance_scores.shape[1])]
    ...
    gs = fig.add_gridspec(nrows=12, ncols=50)  # reduced rows from 13 to 12
    ax = fig.add_subplot(gs[0:11, 2:45])
    ax_curve = fig.add_subplot(gs[0:11, 45:48], sharey=ax)
    ax_cbar = fig.add_subplot(gs[4:8, 49])

    # 🔹 Use dynamic vmax for better contrast
    vmax = np.percentile(sorted_scores, 99)
    ...
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✅ Saved topological clustering heatmap to {output_path}")
...
def save_and_plot_confirmed_genes_bio(
    ...
    # ✅ Load confirmed gene list
    with open(confirmed_gene_path) as f:
        known_cancer_genes = set(line.strip() for line in f if line.strip())

    confirmed_genes = [g for g in node_names_topk if g in known_cancer_genes]

    # ✅ Save confirmed genes to file
    with open(confirmed_genes_save_path, "w") as f:
        for gene in confirmed_genes:
            f.write(f"{gene}\n")
    ...
    # ✅ Reduce feature relevance to 64D biological summary
    summary_feature_relevance = extract_summary_features_np_bio(summary_feature_relevance)

    def get_scalar_score(score):
        """Convert score to a scalar float safely."""
...
def save_and_plot_confirmed_genes_topo(
    ...
    cancer_names = [
        'BRCA', 'KIRC', 'READ', 'PRAD', 'STAD', 'HNSC', 'LUAD', 'THCA',
    ...
    # ✅ Filter top-k confirmed genes
    confirmed_genes = [g for g in node_names_topk if g in known_cancer_genes]

    # ✅ Save confirmed gene names
    with open(confirmed_genes_save_path, "w") as f:
        for gene in confirmed_genes:
            f.write(f"{gene}\n")

    # ✅ Reduce relevance to 64D topo summary
    ...
    def get_scalar_score(score):
        """Safely convert a score to a scalar float."""
        if isinstance(score, np.ndarray):
            if score.size == 1:
                return score.item()
            return score[0]
...
def train(args):
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            # Replace summary_feature_relevance with relevance_scores_topk
            save_and_plot_confirmed_genes_bio(
                args=args,
                node_names_topk=node_names_topk,
                node_scores_topk=relevance_scores_topk.detach().cpu().numpy(),
                summary_feature_relevance=relevance_scores_topk.detach().cpu().numpy(),  # ✅ use topk bio relevance here
            ...
            save_and_plot_confirmed_genes_topo(
                ...
                node_names_topk=node_names_topk,
                node_scores_topk=relevance_scores_topk.detach().cpu().numpy(),
                summary_feature_relevance=relevance_scores_topk.detach().cpu().numpy(),  # ✅ use topk topo relevance here
                output_dir=output_dir,
                confirmed_genes_save_path=confirmed_genes_save_path,
                tag="topo"
            )
        ...
        # Slice topological relevance scores
        '''relevance_scores_topo = relevance_scores[:, 1024:]
        relevance_scores_topo_topk = relevance_scores_topo[topk_node_indices_tensor]

        # Plot topological heatmap
        spectral_biclustering_heatmap_path_topo = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_spectral_biclustering_heatmap_topo_epo{args.num_epochs}.png"
        )

        plot_topo_biclustering_heatmap(
            args=args,
            relevance_scores=relevance_scores_topo_topk.detach().cpu().numpy(),
            cluster_labels=cluster_labels_topk.cpu().numpy(),
            output_path=spectral_biclustering_heatmap_path_topo,
            gene_names=node_names_topk,
        )
    ...
    cancer_types = ['BRCA', 'KIRC', 'READ', 'PRAD', 'STAD', 'HNSC', 'LUAD', 
                    'THCA', 'BLCA', 'ESCA', 'LIHC', 'UCEC', 'COAD', 'LUSC', 'CESC', 'KIRP']
    feature_names = [f"{cancer}_{omics}" for omics in omics_types for cancer in cancer_types]
    ...
    # Plot biological biclustering heatmap
    spectral_biclustering_heatmap_path_bio = os.path.join(
        output_dir, f"{args.model_type}_{args.net_type}_spectral_biclustering_heatmap_bio_epo{args.num_epochs}.png"
    )

    plot_bio_biclustering_heatmap(
        args=args,
        relevance_scores=relevance_scores_bio_topk.detach().cpu().numpy(),
        cluster_labels=cluster_labels_topk.cpu().numpy(),
        feature_names=feature_names,
        gene_names=node_names_topk,
        omics_splits=omics_splits,
        output_path=spectral_biclustering_heatmap_path_bio
    )
    ...
    ##reduced_scores = extract_summary_features_np_skip(relevance_scores_bio.cpu().numpy())
    ...
    for gene in confirmed_genes:
        ...
        # Step 1: Pair neighbor indices with their relevance scores
        neighbor_scores_dict = {
            i: relevance_scores[i].sum().item()
            for i in neighbor_indices
            if relevance_scores[i].sum().item() > 0.1  # filter
        }
...
def train_graph_with_edge_node_lrp_pass(args):
    ...
    bfr_node_indices = list(set(top_gene_indices) | neighbor_indices)
    bfr_node_indices = torch.tensor(bfr_node_indices, dtype=torch.long)
    
    lrp_node_features = embeddings[bfr_node_indices].cpu().numpy()
    ...
    full_lrp_features[bfr_node_indices] = torch.tensor(lrp_node_features, dtype=torch.float32)
