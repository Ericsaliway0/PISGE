...
def plot_confirmed_neighbors_bio(
    ...
    for gene in confirmed_genes:
        ...
        plot_path = os.path.join(
            ...
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_bio_confirmed_neighbor_relevance_epo{args.num_epochs}.png"
        )
...
def plot_confirmed_neighbors_topo(
    ...
    for gene in confirmed_genes:
        ...
        plot_path = os.path.join(
            ...
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_topo_confirmed_neighbor_relevance_epo{args.num_epochs}.png"
        )
...
def plot_confirmed_neighbor_relevance(
    ...
    for gene in confirmed_genes:
        ...
        plot_path = os.path.join(
            ...
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_{mode}_neighbor_relevance_epo{args.num_epochs}.png"
        )
...
def plot_topo_clusterwise_feature_contributions(
    ...
    for cluster_id in sorted(unique_clusters):
        ...
        save_path = os.path.join(
            ...
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_cluster_{cluster_id}_feature_contributions_epo{args.num_epochs}.png"
        )
...
def plot_saliency_for_gene(
    ...
    plot_feature_importance_bio(
        ...
        output_path=os.path.join(
            save_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_bio_feature_importance_epo{args.num_epochs}.png"
        )
    )
    ...
    plot_feature_importance_topo(
        ...
        output_path=os.path.join(
            save_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_topo_feature_importance_epo{args.num_epochs}.png"
        )
    )
...
def plot_contingency_matrix(cluster_labels_bio_topk, cluster_labels_topo_topk, ari_score, nmi_score, output_dir, args):
    ...
    contingency_plot_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_contingency_matrix_epo{args.num_epochs}.png"
    )
    plt.tight_layout()
...
def save_and_plot_confirmed_genes_topo(
    ...
    for gene_name in confirmed_genes:
        ...
        plot_path = os.path.join(
            plot_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene_name}_{tag}_confirmed_feature_contributions_epo{args.num_epochs}.png"
        )
...
def plot_dynamic_sankey_topo_clusterlevel_each_gene(
    ...
    for gene in confirmed_genes:
        ...
        output_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_topo_confirmed_neighbor_sankey_epo{args.num_epochs}.html"
        )
...
def plot_dynamic_sankey_bio_clusterlevel_not_fixed_colors(
    ...
    plot_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_confirmed_dynamic_sankey_clusterlevel_epo{args.num_epochs}.html"
    )
...
def plot_dynamic_sankey_topo_clusterlevel(
    ...
    plot_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_confirmed_dynamic_sankey_clusterlevel_epo{args.num_epochs}.html"
    )
    sankey_fig.write_html(plot_path)
    print(f"✅ Cluster-level Sankey diagram saved to {plot_path}")
...
def plot_multilevel_sankey_bio_clusterlevel(
    ...
    for gene in confirmed_genes:
        ...
        save_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_{gene}_bio_multilevel_sankey_epo{args.num_epochs}.html"
        )
...
def plot_chord_diagram_topo(
    ...
    html_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_chord_epo{args.num_epochs}.html"
    )
    ...
    try:
        ...
        png_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_chord_epo{args.num_epochs}.png"
        )
...
def save_and_plot_enriched_pathways(enrichment_results, args, output_dir):
    ...
    enrichment_csv_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_enrichment_matrix_epo{args.num_epochs}.csv"
    )
    heatmap_data.to_csv(enrichment_csv_path, index_label='Enriched Pathway')
    ...
    topo_terms_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_cluster_top_terms_epo{args.num_epochs}.csv"
    )
    topo_terms_df.to_csv(topo_terms_path, index=False)
    ...
    bio_terms_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_cluster_top_terms_epo{args.num_epochs}.csv"
    )
    ...
    enriched_terms_heatmap_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_enriched_terms_heatmap_epo{args.num_epochs}.png"
    )
    plt.savefig(enriched_terms_heatmap_path, dpi=300)
    plt.close()
...
def plot_collapsed_clusterfirst_multilevel_sankey_topo(
    ...
    save_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_collapsed_clusterfirst_multilevel_sankey_epo{args.num_epochs}.html"
    )
    fig.write_html(save_path)
    ...
    try:
        ...
        png_save_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_topo_collapsed_clusterfirst_multilevel_sankey_epo{args.num_epochs}.png"
        )
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    save_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_collapsed_clusterfirst_multilevel_sankey_epo{args.num_epochs}.html"
    )
    fig.write_html(save_path)
    ...
    try:
        ...
        png_save_path = os.path.join(
            output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_collapsed_clusterfirst_multilevel_sankey_epo{args.num_epochs}.png"
        )
        fig.write_image(png_save_path, scale=2, width=1200, height=1200)
        print(f"🖼️ PNG also saved to: {png_save_path}")
    except Exception as e:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_entropy_bar_epo{args.num_epochs}.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_jaccard_heatmap_epo{args.num_epochs}.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_centrality_bar_epo{args.num_epochs}.png"))
    plt.close()
...
def extract_summary_features_np_bio_duplicate(bio_embeddings_np):
    """
...
def extract_summary_features_np_bio(bio_embeddings_np):
    """
    Extracts summary features from just the 1024 biological features (bio only),
    and ensures unique cancer names within each omics group.
    ...
    cancer_types = [
        "Liver", "LungSCC", "LungAC", "Kidney", "Prostate", "Bladder", "Pancreas", "Thyroid",
        "Esophagus", "Uterus", "Cervix", "Ovary", "Stomach", "Colon", "Breast", "HeadNeck"
    ]
    omics_groups = ["expression", "methylation", "mutation", "copy_number"]

    # Check uniqueness of cancer names within each omics group
    for o_idx, omics in enumerate(omics_groups):
        seen = set()
        for c_idx, cancer in enumerate(cancer_types):
            if cancer in seen:
                raise ValueError(f"Duplicate cancer name '{cancer}' found in omics group '{omics}'")
            seen.add(cancer)
...
def plot_bio_clusterwise_feature_contributions(
    ...
    for cluster_id in sorted(unique_clusters):
        ...
        save_path = os.path.join(
            per_cluster_feature_contributions_output_dir,
            f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_cluster_{cluster_id}_feature_contributions_epo{args.num_epochs}.png"
        )
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_topo_biclustering_heatmap_clusters_unsort_ori(
    args,
    relevance_scores,
    cluster_labels,
    output_path,
...
def plot_bio_biclustering_heatmap_clusters_unsort_ori(
    args,
    relevance_scores,
    cluster_labels,
    omics_splits,
...
def apply_full_spectral_biclustering_bio(
    ...
    saliency_matrix = filtered_features
...
def apply_full_spectral_biclustering_topo(
    ...
    bicluster = best_balanced_biclustering(summary_topo_features, n_clusters, n_trials)
    row_labels = bicluster.row_labels_
    col_labels = bicluster.column_labels_
...
def plot_topo_biclustering_heatmap_unsorted_ori(
    args,
    relevance_scores,
    cluster_labels,
    output_path,
...
def train(args):
    ...
    plot_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_silhouette_score_plot_epo{args.num_epochs}.png")
    best_k_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_best_k_epo{args.num_epochs}.txt")
    ...
    bio_output_img = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_clusters_epo{args.num_epochs}.png")
    topo_output_img = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_clusters_epo{args.num_epochs}.png")

    bio_output_path_heatmap = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_heatmap_epo{args.num_epochs}.png")
    topo_output_path_heatmap = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_heatmap_epo{args.num_epochs}.png")
    
    bio_graph_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_clustered_graph.pth")
    topo_graph_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_clustered_graph.pth")

    bio_cluster_labels_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_cluster_labels.npy")
    topo_cluster_labels_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_cluster_labels.npy")

    bio_total_per_cluster_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_total_genes_per_cluster.npy")
    topo_total_per_cluster_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_total_genes_per_cluster.npy")

    bio_pred_counts_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_BIO_pred_counts.npy")
    topo_pred_counts_path = os.path.join('data/', f"{args.model_type}_{args.net_type}_{args.hidden_feats}_TOPO_pred_counts.npy")

    plot_silhouette_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_silhouette_score_plot_epo{args.num_epochs}.png")
    '''best_k_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_best_k_epo{args.num_epochs}.txt")
    ...
    plot_eigengap_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_eigengap_epo{args.num_epochs}.png")
    ...
    saliency_matrix_bio = summary_bio_topk
    ...
    saliency_matrix_topo = summary_topo_topk
    
    best_k_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_best_k_epo{args.num_epochs}.txt")
    #best_k = 10#find_optimal_k(top_node_features, k_range=(5, 20), plot_path=plot_silhouette_path, save_best_k_path=best_k_path)
    # Automatically determine best_k via eigengap
    plot_eigengap_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_eigengap_epo{args.num_epochs}.png")
    assert saliency_matrix_bio.shape[1] == 64, f"Expected 64 summary features, got {saliency_matrix_bio.shape[1]}"
    assert saliency_matrix_topo.shape[1] == 64, f"Expected 64 summary features, got {saliency_matrix_topo.shape[1]}"
    #best_k, eigenvals = eigengap_analysis(saliency_matrix, max_clusters=11, normalize=True, plot_path=plot_eigengap_path)

    best_k=10
    print(f"Best number of clusters (k > 1): {best_k}")

    # Step 2: Convert to numpy
    #saliency_matrix = summary_bio_topk#.detach().cpu().numpy()
    #saliency_matrix = normalize(summary_bio_topk, axis=1)
    ...
    graph, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio(
        graph=graph,
        summary_bio_features=saliency_matrix_bio,  # shape (1000, 64)
        node_names=node_names,
    ...
    full_cluster_labels_bio = torch.full((graph.num_nodes(),), -1, dtype=torch.long)
    ...
    graph.ndata['cluster_bio'] = full_cluster_labels_bio
    ...
    graph, cluster_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo(
        ...
        summary_topo_features=saliency_matrix_topo,
        node_names=node_names,
        #feature_names=None,
    ...
    # First create a full tensor of -1 (default for unclustered)
    full_cluster_labels_topo = torch.full((graph.num_nodes(),), -1, dtype=torch.long)

    # Then assign the cluster labels only to the relevant subset
    full_cluster_labels_topo[top_gene_indices] = torch.tensor(cluster_labels_topo, dtype=torch.long)

    # Now safely assign it to the graph
    graph.ndata['cluster_topo'] = full_cluster_labels_topo
    ...
    plot_pcg_cancer_genes(
        ...
        output_path=os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_pcg_percent_bio_epo{args.num_epochs}.png')
    )
    ...
    plot_kcg_cancer_genes(
        ...
        node_names=node_names_topk,
        cluster_labels=cluster_labels_bio,
        output_path=os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_kcg_percent_bio_epo{args.num_epochs}.png')
    )
    ...
    suffix = 'bio'
    output_path_interactions_kcgs_bio = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_kcgs_interaction_{suffix}_epo{args.num_epochs}.png')
    output_path_interactions_pcgs_bio = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_pcgs_interaction_{suffix}_epo{args.num_epochs}.png')
    ...
    plot_pcg_cancer_genes(
        ...
        output_path=os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_pcg_percent_topo_epo{args.num_epochs}.png')
    )
    ...
    plot_kcg_cancer_genes(
        ...
        node_names=node_names_topk,
        cluster_labels=cluster_labels_topo,
        output_path=os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_kcg_percent_topo_epo{args.num_epochs}.png')
    )
    ...
    suffix = 'topo'
    output_path_interactions_kcgs_topo = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_kcgs_interaction_{suffix}_epo{args.num_epochs}.png')
    output_path_interactions_pcgs_topo = os.path.join(output_dir, f'{args.model_type}_{args.net_type}_{args.hidden_feats}_pcgs_interaction_{suffix}_epo{args.num_epochs}.png')
    ...
    confirmed_file = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_confirmed_genes_epo{args.num_epochs}.csv")
     
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ("bio", full_cluster_labels_bio, saliency_matrix_bio, "bio"),
        ("topo", full_cluster_labels_topo, saliency_matrix_topo, "topo")
        ]:
        ...
        confirmed_genes_save_path = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_confirmed_genes_{tag}_epo{args.num_epochs}.txt"
        )
        ...
        heatmap_path = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_spectral_biclustering_heatmap_{tag}_epo{args.num_epochs}.png"
        )

        heatmap_path_unsort = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_spectral_biclustering_heatmap_{tag}_epo{args.num_epochs}_unsort.png"
        )

        heatmap_path_clusters_unsort = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_spectral_biclustering_heatmap_{tag}_epo{args.num_epochs}_clusters_unsort.png"
        )
        saliency_ridge_path = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_saliency_ridge_{tag}_epo{args.num_epochs}.png"
        )

        ridge_all_clusters_path = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_ridge_all_clusters_{tag}_epo{args.num_epochs}.png"
        )

        ridge_from_bio_path_across = os.path.join(
            output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_ridge_from_{tag}_across_epo{args.num_epochs}"
        )        

        if tag == "bio":
            plot_bio_biclustering_heatmap(
                args=args,
                relevance_scores=saliency_matrix_bio,#relevance_scores_topk.detach().cpu().numpy(),
            ...
            plot_bio_biclustering_heatmap_unsort(
                args=args,
                relevance_scores=saliency_matrix_bio,#relevance_scores_topk.detach().cpu().numpy(),
                row_labels=cluster_labels_bio,#cluster_labels_topk.cpu().numpy(),
                gene_names=node_names_topk,
                omics_splits=omics_splits,
            ...
            save_and_plot_confirmed_genes_bio(
                ...
                node_scores_topk=saliency_matrix_bio,#relevance_scores_topk.detach().cpu().numpy(),
                summary_feature_relevance=saliency_matrix_bio,
            ...
            #sankey_bio_output_path=os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_dynamic_sankey_bio_epo{args.num_epochs}.html")
            ...
            plot_collapsed_clusterfirst_multilevel_sankey_bio(
                ...
                relevance_scores=saliency_matrix_bio,
                CLUSTER_COLORS=CLUSTER_COLORS 
            )

            '''chord_bio_output_path=os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_gene_neighbors_chord_bio_epo{args.num_epochs}.html")
            chord_bio_output_path_png=os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_gene_neighbors_chord_bio_epo{args.num_epochs}.png")
            ##selected_genes = ["PLK1", "SKP2", "SRC", "TRAF2"]
            ...
            '''neighbor_saliency_output = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_neighbor_saliency_heatmap_epo{args.num_epochs}.png")
            ...
            plot_neighbor_saliency_heatmap(
                ...
                node_names=node_names,
                name_to_index=name_to_index,
                relevance_scores=saliency_matrix_bio, # Full relevance scores (before slicing to topk)
                omics_splits=omics_splits,
            ...
            # saliency_graph_save_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_saliency_graph_{target_gene_name}_epo{args.num_epochs}.png")
            
        else:
            plot_topo_biclustering_heatmap(
                args=args,
                relevance_scores=saliency_matrix_topo,# relevance_scores_topk.detach().cpu().numpy(),
            ...
            plot_topo_biclustering_heatmap_unsorted(
                args=args,
                relevance_scores=saliency_matrix_topo,
                cluster_labels=cluster_labels_topo,
                gene_names=node_names_topk,
                output_path=heatmap_path_unsort,
                col_labels=col_labels_topo
            ...
            save_and_plot_confirmed_genes_topo(
                args=args,
                node_names_topk=node_names_topk,
                node_scores_topk=saliency_matrix_topo,#relevance_scores_topk.detach().cpu().numpy(),
                summary_feature_relevance=saliency_matrix_topo,  # full 1024D, will be reduced inside
                output_dir=output_dir,
                confirmed_genes_save_path=confirmed_file,
    ...
    if len(bio_scores) > 0 and len(topo_scores) > 0:
        ...
        plot_bio_topo_saliency(
            ...
            save_path=os.path.join(save_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_topo_comparison_lineplot_square_{args.num_epochs}.png")
        )
        ...
        plot_bio_topo_saliency_cuberoot(
            ...
            topo_means,
            title="Average Feature Relevance for Confirmed Genes (Bio vs Topo)",
            save_path=os.path.join(save_dir, f"{args.model_type}_{args.net_type}_{args.hidden_feats}_bio_topo_comparison_cuberoot_{args.num_epochs}.png")
        )
