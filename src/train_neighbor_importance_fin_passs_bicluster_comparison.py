...
def train(args):
    ...
    # Optional: perform enrichment here using cluster_gene_map
    enrichment_results = {}
    ...
    for cluster_type, cluster_map in cluster_gene_map.items():
        ...
        for cluster_id, genes in cluster_map.items():
            # Example mock: pretend you ran enrichment and return top 3 pathways
            # Replace this with real GO/Reactome enrichment using g:Profiler, Enrichr, etc.
            enrichment_results[cluster_type][cluster_id] = {
                'top_terms': [f"Pathway_{i}" for i in range(3)],
    ...
    for cluster_type in ['bio', 'topo']:
        term_counts = [len(res['top_terms']) for res in enrichment_results[cluster_type].values()]
        ax.bar([f"{cluster_type}_{i}" for i in enrichment_results[cluster_type].keys()],
               term_counts, label=cluster_type)
    ax.set_ylabel("Number of enriched terms")
    ax.set_title("Functional coherence: enriched term counts per cluster")
    ax.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig("results/term_counts_barplot.png")
    plt.close()

    # Optional: Venn diagram for shared terms
    from matplotlib_venn import venn2
    ...
    plt.savefig("results/shared_pathways_venn.png")
    plt.close()

    # Heatmap of -log10(p-values)
    all_terms = sorted(set(bio_terms | topo_terms))
    heatmap_data = pd.DataFrame(index=all_terms)
    for cluster_type in ['bio', 'topo']:
        for cid, res in enrichment_results[cluster_type].items():
            colname = f"{cluster_type}_{cid}"
            vals = {term: -np.log10(p) for term, p in zip(res['top_terms'], res['p_values'])}
            heatmap_data[colname] = pd.Series(vals)
    heatmap_data = heatmap_data.fillna(0)

    plt.figure(figsize=(10, max(6, 0.3 * len(all_terms))))
    ...
    # Assuming your embedding is of shape [N, 1024] or similar
