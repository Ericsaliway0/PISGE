...
def cluster_and_visualize_predicted_genes(graph, predicted_cancer_genes, node_names, 
    ...
    kmeans = KMeans(n_clusters=num_clusters, random_state=42, n_init=num_clusters)
...
def plot_gene_feature_contributions(gene_name, relevance_vector, feature_names, output_path=None):
    ...
    # Plot barplot for total omics contributions
    if output_path:
    ...
    # Optional reordering if you want a fixed order
    preferred_order = ['mf', 'cna', 'ge', 'meth']
    heatmap_data = heatmap_data.reindex(preferred_order)

    # Plot heatmap
    plt.figure(figsize=(8, 2.8))
    sns.heatmap(heatmap_data, cmap='OrRd', cbar=False, linewidths=0.3, linecolor='gray')
...
def cluster_and_visualize_predicted_genes(graph, non_labeled_nodes, predicted_cancer_genes, node_names, output_dir, num_clusters=10):
    """
    Clusters gene embeddings into 10 groups using KMeans, visualizes them using t-SNE, 
