...
def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names,
    ...
    ax_gap.axis("off")
    bluish_gray_gradient = LinearSegmentedColormap.from_list("bluish_gray_gradient", ["#F0F3F4", "#85929e"])
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=-2, vmax=2, ax=ax_heatmap, cbar_kws={"shrink": 0.5})
    ax_heatmap.set_title("Heatmap of Summary Bio Features Sorted by Spectral Biclusters", fontsize=14)
    ax_heatmap.set_yticks([])

    # === Real feature names for x-axis
    cancer_names = [
        'Bladder', 'Breast', 'Cervix', 'Colon', 'Esophagus', 'HeadNeck', 'Kidney', 'KidneyPap',
        'Liver', 'LungAd', 'LungSc', 'Prostate', 'Rectum', 'Stomach', 'Thyroid', 'Uterus'
    ]
    bio_feat_names_64 = [f"{omics}_{cancer}" for omics in ['cna', 'ge', 'meth', 'mf'] for cancer in cancer_names]
    if col_labels.shape[0] == len(bio_feat_names_64):
        sorted_names = np.array(bio_feat_names_64)[np.argsort(col_labels)]
    else:
        sorted_names = bio_feat_names_64

    '''ax_heatmap.set_xticks(np.arange(len(sorted_names)) + 0.5)
    ax_heatmap.set_xticklabels(sorted_names, rotation=90, fontsize=7)'''

    # === Force 4-group omics-major ordering
    cancer_names = [
        'Bladder', 'Breast', 'Cervix', 'Colon', 'Esophagus', 'HeadNeck', 'Kidney', 'KidneyPap',
        'Liver', 'LungAd', 'LungSc', 'Prostate', 'Rectum', 'Stomach', 'Thyroid', 'Uterus'
    ]
    ...
    # Override any bicluster column shuffle — preserve omics-major order
    sorted_matrix = summary_topk[sorted_indices, :]  # Already sorted by row cluster
    sorted_labels = row_labels_topk[sorted_indices]
    # No column reordering
    # sorted_matrix = sorted_matrix[:, np.argsort(col_labels)]  ← remove or comment this
    ...
    # === Set xticks and colored labels (omics-major order)
    ...
    fig.savefig(output_path_heatmap, dpi=300)
    plt.close(fig)
    print(f"✅ Summary bio heatmap with grouped, colored feature names saved to {output_path_heatmap}")

    fig.tight_layout()
    os.makedirs(os.path.dirname(output_path_heatmap), exist_ok=True)
    fig.savefig(output_path_heatmap, dpi=300)
    plt.close(fig)
    print(f"✅ Summary bio heatmap with feature names saved to {output_path_heatmap}")
