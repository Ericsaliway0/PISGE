        ...
        try:
            ...
            if not result.empty:
                for source in ["REAC", "KEGG"]:
                    ...
                    path = f"results/gene_prediction/{cancer_type}_{tag}_Cluster_{cluster}_{source}_enrichment.csv"
...
def plot_dot_enrichment_per_cluster_all(
    ...
    top_n=10,
    n_clusters=10,
    base_path="results/gene_prediction"
):
...
def draw_dot_plot_with_ratio(terms, cancer_type, source, top_n=20):
    ...
    out_path = Path(f"results/gene_prediction/{cancer_type}_{source}_dot_plot_ratio.png")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=300)
    plt.close()
...
def train(args):
    ...
    best_k, _ = eigengap_analysis(top_node_features, max_clusters=20, normalize=True, plot_path=plot_eigengap_path)
