...
def plot_spectral_biclustering_heatmap(
    ...
    gs = fig.add_gridspec(
        ...
        ncols=40,
        width_ratios=[
            0.02, *[1]*28, *[0.3]*2, 0.2, 0.1, *[0.02]*7
        ],
        height_ratios=[
            *[1]*10,
            0.3,
    ...
    ax = fig.add_subplot(gs[0:10, 1:29])
    ax_curve = fig.add_subplot(gs[0:10, 29:31], sharey=ax)
    ax_cbar = fig.add_subplot(gs[0:10, 32])
    ax_legend = fig.add_subplot(gs[11, 1:29])
...
def plot_gene_feature_contributions(gene_name, relevance_vector, feature_names, output_path=None):
    """
...
def plot_omics_barplot(df, output_path=None):
    ...
    plt.title("Total Contribution per Omics Type", fontsize=14)
...
def plot_gene_feature_contributions_(
    gene_name,
    relevance_scores,
    gene_names,
    feature_names,
    ...
    idx = np.where(np.array(gene_names) == gene_name)[0]
    if len(idx) == 0:
        print(f"❌ Gene '{gene_name}' not found in gene_names.")
        return
    
    gene_score = relevance_scores[idx[0]]
    
    # Create a 4x16 heatmap from 64D
    omics_types = ["mf", "cna", "meth", "ge"]
    cancer_types = sorted(set(name.split(":")[1] for name in feature_names))
    feature_grid = np.array(gene_score).reshape(4, 16)

    fig, ax = plt.subplots(figsize=(6, 2.5))
    im = ax.imshow(feature_grid, cmap='RdBu_r', aspect='auto')

    # Set axis labels
    ax.set_xticks(np.arange(16))
    ax.set_xticklabels(cancer_types, rotation=90)
    ax.set_yticks(np.arange(4))
    ax.set_yticklabels(["SNV", "CNA", "DNA Meth.", "Gene Expr."])

    ax.set_title(f"{gene_name}\nFeature Contributions", fontsize=12)
    plt.tight_layout()

    # Save
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✅ Saved heatmap for {gene_name} to {output_path}")
...
def save_and_plot_confirmed_genes(
    ...
    """
    Finds top-N confirmed cancer genes from the predictions, saves them, and plots their feature contributions.
    ...
    Args:
        ...
        top_n (int): Number of confirmed genes to save and plot.
    """
    ...
    # Select top confirmed genes from the predictions
    confirmed_genes = [g for g in node_names_topk if g in known_cancer_genes][:top_n]

    # Save to a text file
    confirmed_file = os.path.join(output_dir, f"top{top_n}_confirmed_genes_epo{num_epochs}.txt")
    with open(confirmed_file, "w") as f:
        for gene in confirmed_genes:
            f.write(f"{gene}\n")

    # Plot and save contribution plots
    for gene_name in confirmed_genes:
        idx = node_names_topk.index(gene_name)
        relevance_vector = summary_feature_relevance[idx]
        plot_path = os.path.join(output_dir, f"{gene_name}_confirmed_feature_contributions_epo{num_epochs}.png")
...
def plot_feature_importance_horizaontal(relevance_vector, feature_names=None, node_name=None, top_k=20):
    # Ensure numpy format
    ...
    plt.barh(top_names[::-1], top_scores[::-1], color='slategray')
    plt.xlabel("Relevance Score", fontsize=12)
    plt.title(f"Top {top_k} Feature Importances for {node_name}", fontsize=14)
    plt.tight_layout()
    plt.show()

def plot_feature_importance_no_save(relevance_vector, feature_names=None, node_name=None, top_k=20):
    # Ensure numpy format
    if isinstance(relevance_vector, torch.Tensor):
        relevance_vector = relevance_vector.detach().cpu().numpy()
    ...
    plt.bar(top_names, top_scores, color='slategray')
...
def plot_feature_importance_(args, relevance_vector, feature_names=None, node_name=None, top_k=10, output_dir="results/feature_importance"):
    ...
    # Normalize relevance scores
    norm_scores = normalize(torch.tensor(relevance_vector).reshape(1, -1)).numpy().flatten()
    ...
    # Save plot
    safe_name = node_name.replace("/", "_").replace(" ", "_")  # sanitize file name
    output_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{safe_name}_top{top_k}_features_epo{args.num_epochs}.png")
    plt.savefig(output_path, dpi=300)
    plt.close()

    print(f"✅ Saved feature importance plot for {node_name} to {output_path}")

def plot_feature_importance_keyerror(args, relevance_vector, gene_names=None, node_name=None, top_k=20):
    relevance_vector = relevance_vector.detach().cpu().numpy()
    norm_scores = normalize(torch.tensor(relevance_vector)).numpy()
    ...
    if gene_names is None:
        gene_names = [f"Gene {i}" for i in range(len(relevance_vector))]
    top_labels = [gene_names[i] for i in top_indices]

    plt.figure(figsize=(10, 5))
    plt.bar(top_labels, top_scores, color='skyblue')  # Vertical bars
    plt.ylabel("Relevance Score")
    plt.xticks(rotation=90)
    plt.title(f"Top {top_k} Feature Importances for {node_name}")

    plt.tight_layout()

    # ✅ Save
    save_path = os.path.join("results/gene_prediction/importance_plots", f"{node_name}_importance.png")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"📊 Saved importance plot for {node_name} → {save_path}")

def plot_feature_importance_horizontal_bar(relevance_vector, feature_names=None, node_name=None, top_k=8, gene_names=None):
    ##relevance_vector = relevance_vector.detach().cpu().numpy()
    ...
    # Make sure top_indices are within the valid range for gene_names
    if gene_names is not None:
        top_labels = [gene_names[i] if i < len(gene_names) else f"Unknown {i}" for i in top_indices]
    else:
...
def plot_feature_importance_no_save(relevance_vector, feature_names=None, node_name=None, top_k=8, gene_names=None):
    if isinstance(relevance_vector, torch.Tensor):
        relevance_vector = relevance_vector.detach().cpu().numpy()
    else:
        relevance_vector = np.array(relevance_vector)
...
def plot_feature_importance_nearly_1_for_topest_neighbor(relevance_vector, feature_names=None, node_name=None, top_k=8, gene_names=None, save_dir="results/feature_importance"):
    ...
    # Normalize and get top-k indices
    norm_scores = normalize(torch.tensor(relevance_vector).unsqueeze(0)).numpy().squeeze()
    top_indices = np.argsort(norm_scores)[-top_k:][::-1]
    top_scores = norm_scores[top_indices]
    ...
    # Get gene labels from provided names
    if gene_names is not None:
        top_labels = [gene_names[i] if i < len(gene_names) else f"Unknown {i}" for i in top_indices]
    else:
        top_labels = [feature_names[i] for i in top_indices]
    ...
    # Plot
    plt.figure(figsize=(10, 5))
    plt.bar(top_labels, top_scores, color='skyblue')
    plt.xticks(rotation=45, ha='right')
    plt.ylabel("Relevance Score")
    plt.title(f"Top {top_k} Feature Importances for {node_name}")
    plt.tight_layout()
    ...
    # Save and show
    filename = os.path.join(save_dir, f"{node_name}_feature_importance.png")
    plt.savefig(filename, dpi=300)
    plt.show()

    print(f"✅ Saved feature importance plot for {node_name} → {filename}")
...
def plot_feature_importance(
    ...
    bars = plt.bar(top_labels, top_scores, color='cornflowerblue')
    plt.ylabel("Relevance Score (Min-Max)")
    plt.title(f"Top {top_k} Neighbor Importances for {node_name}")
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
...
def train(args):
    ...
    print("Computing LRP relevance scores for the entire graph...")
    ...
    neighbors_dict = get_neighbors_gene_names(graph, node_names, name_to_index, target_genes)

    # Show neighbors of SRC
    print("SRC neighbors:===================================================================", neighbors_dict["SRC"])
    ...
    with torch.no_grad():
        ...
        for gene in target_genes:
            ...
            plot_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_{gene}_feature_importance_epo{args.num_epochs}.png")
...
def train_vertical_bar_pas(args):
    epoch_times = []
    cpu_usages = []
    gpu_usages = []
    ...
    with torch.no_grad():
        ...
        for gene in target_genes:
            ...
            neighbor_indices = [name_to_index[n] for n in neighbors if n in name_to_index]
            neighbor_relevance_scores = [relevance_scores[i].sum().item() for i in neighbor_indices]  # 👍 Fix
...
def train_pass_omics_contribution(args):
    ...
    save_and_plot_confirmed_genes(
        ...
        feature_names=feature_names,               # list of 64 summary feature names
        output_dir=output_dir,                     # your result directory
        num_epochs=args.num_epochs,                # or just pass an integer
        confirmed_gene_path="data/ncg_8886.txt",   # optional, this is the default
        top_n=5                                     # optional, this is also the default
    )
...
def train_ori_both_cluster_interaction_pass(args):
    ...
    with torch.no_grad():
        ...
        for gene in target_genes:
            ...
            print(f"{gene} → Node {node_idx} | Predicted score: {score:.4f}")

            plot_feature_importance(
                relevance_scores[node_idx],
                feature_names=feature_names,
                node_name=gene
            )
