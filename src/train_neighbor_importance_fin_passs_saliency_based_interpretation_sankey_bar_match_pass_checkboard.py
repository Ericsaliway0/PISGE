...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_(
    ...
    selected_known_genes = ["EGFR", "BRCA2"]
    selected_known_genes = ["EGFR", "SRC", "ACTB", "RBM39"]#, "HDAC2", "ETS1", "ATR"]
    ...
    for gene in combined_genes:
        ...
        # ✅ Skip genes with no valid neighbors
        # if len(neighbor_scores) >= 2:
        # if not neighbor_scores:
        #     continue

        if len(neighbor_scores) >= 10:
    ...
    for gene, neighbors in gene_to_neighbors.items():
        ...
        for neighbor_idx, score in neighbors.items():
            ...
            neighbor_cluster = int(row_labels[neighbor_idx])
            all_neighbor_rows.append([
                gene, neighbor_name, neighbor_idx, float(score), neighbor_cluster
            ])
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        cluster_idx = label_to_idx[cluster_label]
        genes_sorted = sorted(genes, key=lambda g: relevance_scores[topk_name_to_index[g]].sum(), reverse=False)[:10]
...
def plot_biclustering_input_feature_heatmap(
    ...
    - output_path: str
    - title: str
    - cmap: str or None — if None, uses bluish-gray gradient
    """
    assert feature_matrix.shape[0] == len(node_names_topk), \
        f"Mismatch: {feature_matrix.shape[0]} rows vs {len(node_names_topk)} gene names"
    ...
    # 🔹 Use bluish-gray if no colormap provided
    if cmap is None:
...
def apply_full_spectral_biclustering_bio(
    ...
    print("🧪 Running Spectral Biclustering with fixed clusters...")
    ...
    for i in range(n_trials):
        ...
        model.fit(summary_bio_features_topk)
        reconstructed = summary_bio_features_topk[
    ...
    # === Add cluster labels back to graph
    row_labels_tensor = torch.full((graph.num_nodes(),), -1, dtype=torch.long)
    row_labels_tensor[topk_node_indices] = torch.tensor(row_labels, dtype=torch.long)
    graph.ndata['cluster_bio_summary'] = row_labels_tensor
    ...
    # === Save sorted gene–omics matrix with cluster/group info
    print("📊 Saving sorted gene–omics matrix...")

    sorted_row_indices = np.argsort(row_labels)
    sorted_col_indices = np.argsort(col_labels)

    sorted_matrix = summary_bio_features_topk[sorted_row_indices][:, sorted_col_indices]

    df = pd.DataFrame(sorted_matrix)

    df.index = [
        node_names_topk[i] if i < len(node_names_topk) else f"node_{topk_node_indices[i]}"
        for i in sorted_row_indices
    ]

    if omics_splits and isinstance(omics_splits, list):
        col_labels_expanded = [
            f"{omics_splits[j]}" if j < len(omics_splits) else f"Omics_{j}"
            for j in sorted_col_indices
        ]
    else:
        col_labels_expanded = [f"Omics_{j}" for j in sorted_col_indices]

    df.columns = col_labels_expanded

    sorted_matrix_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_bio_sorted_gene_omics_matrix_epo{args.num_epochs}.csv"
    )
    df.to_csv(sorted_matrix_path)
    print(f"📄 Saved sorted gene–omics matrix: {sorted_matrix_path}")

    # === Save gene-cluster assignments CSV
    gene_assignment_path = os.path.join(
        output_dir,
        f"{args.model_type}_{args.net_type}_bio_gene_cluster_assignments_epo{args.num_epochs}.csv"
    )

    assignment_rows = []
    for i, (idx, cluster) in enumerate(zip(topk_node_indices, row_labels)):
        gene_name = node_names_topk[i] if i < len(node_names_topk) else f"node_{idx}"
        rel_score = summary_bio_features_topk[i].sum().item()
        assignment_rows.append([gene_name, idx, cluster, round(rel_score, 5)])
    assignment_rows.sort(key=lambda x: (x[2], -x[3]))
    ...
    with open(gene_assignment_path, "w", newline="") as f:
        ...
        writer.writerow(["Gene_Name", "Index", "Cluster", "Relevance_Score"])
        writer.writerows(assignment_rows)
    print(f"📄 Saved gene-cluster assignment CSV: {gene_assignment_path}")

    # === Save cluster heatmap and clusters plot (use your custom plot functions)
    plot_bio_biclustering_heatmap_unsort(
        args=args,
        relevance_scores=summary_bio_features_topk,
        omics_splits=omics_splits,
    ...
    #     relevance_scores=summary_bio_features_topk,
    #     omics_splits=omics_splits,
    #     output_path=output_path_genes_clusters,
    #     row_labels=row_labels,
    #     col_labels=col_labels
    # )
    plot_bio_biclustering_clustermap(
    ...
    print(f"📸 Saved cluster heatmap to: {output_path_heatmap}")
    print(f"📸 Saved cluster clustermap to: {output_path_genes_clusters}")
...
def plot_bio_biclustering_clustermap(
    ...
    # Normalize relevance scores
    relevance_scores = (relevance_scores - relevance_scores.min()) / (relevance_scores.max() - relevance_scores.min()) * 200
    ...
    # Column color strip (omics-level)
    col_colors = []
    for omics in omics_order:
        start, end = omics_splits[omics]
        color = omics_colors[omics]
    ...
    # Reorder rows/columns if provided
    if row_labels is not None:
        df = df.iloc[np.argsort(row_labels)]
    if col_labels is not None:
        df = df.iloc[:, np.argsort(col_labels)]
    ...
    # Use bluish-gray color gradient
    bluish_gray_gradient = LinearSegmentedColormap.from_list("bluish_gray_gradient", ["#F0F3F4", "#85929e"])

    # Clustermap with right-side color bar
    cg = sns.clustermap(
        df,
        cmap=bluish_gray_gradient,
        row_cluster=False,
        ...
        yticklabels=False,
        figsize=(18, 14),
        cbar_pos=(0.92, 0.3, 0.02, 0.4),  # right-side color bar
        cbar_kws={"label": "Relevance Score"},
        dendrogram_ratio=(0.01, 0.01)
    )

    # X-axis label coloring
    for label in cg.ax_heatmap.get_xticklabels():
        label_text = label.get_text()
        for omics in omics_order:
            if omics.upper() in label_text:
...
def plot_biclustering_input_feature_heatmap_do_not_delete(
    feature_matrix: np.ndarray,
    node_names_topk: list,
    output_path: str,
    title: str = "Biclustering Heatmap (Input Features)",
    ...
    """
    Applies spectral biclustering to input features of top-k predicted genes
    and visualizes the result as a heatmap with feature labels and cluster colors.

    Also saves:
    - Reordered heatmap matrix to CSV
    - Gene-to-cluster assignments to CSV
    """
    ...
    # 🔹 Reorder names and labels
    reordered_gene_names = [node_names_topk[i] for i in row_order]
    reordered_cluster_labels = model.row_labels_[row_order]
    reordered_feature_indices = col_order
    reordered_feature_labels = [f"F{i}" for i in reordered_feature_indices]
    ...
    cluster_df = pd.DataFrame({
        "Gene": reordered_gene_names,
        "Cluster": reordered_cluster_labels
    })
    cluster_path = output_path.replace(".png", "_cluster_labels.csv")
    cluster_df.to_csv(cluster_path, index=False)
    print(f"[Saved] Cluster labels → {cluster_path}")

    # 🔹 Save matrix
    heatmap_df = pd.DataFrame(clustered_matrix, index=reordered_gene_names, columns=reordered_feature_labels)
    matrix_csv_path = output_path.replace(".png", "_heatmap_matrix.csv")
    heatmap_df.to_csv(matrix_csv_path)
    print(f"[Saved] Reordered matrix → {matrix_csv_path}")
    ...
    sns.heatmap(
        clustered_matrix,
        cmap=cmap,
        xticklabels=reordered_feature_labels,
        yticklabels=False,
        ax=ax
    )

    # Add cluster color strips on the left
    for i, cluster in enumerate(reordered_cluster_labels):
        ax.add_patch(
            plt.Rectangle((-1.5, i), 1.0, 1.0, linewidth=0,
                          facecolor=to_rgba(CLUSTER_COLORS.get(cluster, "#FFFFFF")),
                          clip_on=False)
        )

    ax.set_title(title, fontsize=14)
    ax.set_xlabel("Reordered Input Features", fontsize=12)
    ax.set_ylabel("Top-k Genes (Cluster Ordered)", fontsize=12)
    ax.set_xticklabels(reordered_feature_labels, rotation=90, fontsize=9)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
...
def plot_bio_biclustering_clustermap(
    ...
    # Column color bar: match features with omics colors
    col_colors = []
    for omics in omics_order:
        start, end = omics_splits[omics]
        color = omics_colors[omics]
    ...
    # Apply row and column label ordering if provided
    if row_labels is not None:
        df = df.iloc[np.argsort(row_labels)]
    if col_labels is not None:
        df = df.iloc[:, np.argsort(col_labels)]
    ...
    cg = sns.clustermap(
        ...
        xticklabels=True,
        yticklabels=False,
        figsize=(20, 14),  # slightly wider for legend
        cbar_pos=(0.92, 0.3, 0.02, 0.4),  # right-side colorbar
        cbar_kws={"label": "Relevance Score"},
        dendrogram_ratio=(0.01, 0.01)
    )

    # Color x-axis labels by omics
    for label in cg.ax_heatmap.get_xticklabels():
        label_text = label.get_text()
        for omics in omics_order:
            if omics.upper() in label_text:
    ...
    # Create legend for omics types outside the plot
    legend_elements = [
        Patch(facecolor=color, edgecolor='black', label=omics.upper())
        for omics, color in omics_colors.items()
    ]
    ...
    plt.savefig(output_path, bbox_inches='tight')
    plt.close()
    print(f"[Saved] Omics clustermap → {output_path}")

def plot_bio_biclustering_clustermap_not_heatmap(
    args,
    relevance_scores,
    omics_splits,
    output_path,
    ...
    # Save top-k features per gene
    if gene_names is not None:
        topk_dir = os.path.join(os.path.dirname(output_path), "topk_features_per_gene")
        os.makedirs(topk_dir, exist_ok=True)
        sorted_indices = np.argsort(row_labels) if row_labels is not None else np.arange(df.shape[0])
        sorted_gene_names = [gene_names[i] for i in sorted_indices]
        sorted_relevance = df.values
        for i, gene in enumerate(sorted_gene_names):
            topk_indices = np.argsort(-sorted_relevance[i])[:top_k]
            topk_features = [feature_names[j] for j in topk_indices]
            topk_scores = sorted_relevance[i][topk_indices]
            gene_df = pd.DataFrame({"Feature": topk_features, "Score": topk_scores})
            gene_df.to_csv(os.path.join(topk_dir, f"{gene}_top{top_k}_features.csv"), index=False)

    # Colormap: bluish gray
    bluish_gray_gradient = LinearSegmentedColormap.from_list("bluish_gray_gradient", ["#F0F3F4", "#85929e"])

    # Row color bar (cluster stripes)
    row_colors = None
    if row_labels is not None:
        sorted_row_labels = row_labels[np.argsort(row_labels)]
        row_colors = [to_rgba(cluster_colors.get(cl, '#FFFFFF')) for cl in sorted_row_labels]
...
def plot_biclustering_input_feature_heatmap(
    ...
    assert feature_matrix.shape[0] == len(node_names_topk), "Feature matrix row count must match number of top-k genes"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    reordered_feature_labels = [feature_names[i] for i in reordered_feature_indices]
    ...
    topk_df.columns = [f"Top{i+1}" for i in range(top_k)]
    topk_path = output_path.replace(".png", f"_top{top_k}_features_per_gene.csv")
    topk_df.to_csv(topk_path)
    print(f"[Saved] Top-{top_k} features per gene → {topk_path}")
    ...
    # Color x-axis labels by omics color
    for tick_label in ax.get_xticklabels():
        text = tick_label.get_text()
        for omics_prefix, color in OMICS_COLORS.items():
            if text.startswith(omics_prefix):
                tick_label.set_color(color)
                tick_label.set_fontsize(8)
                tick_label.set_rotation(90)
                break

    # Add left bar for cluster color
    for i, cluster in enumerate(reordered_cluster_labels):
        ax.add_patch(
            plt.Rectangle(
                (-1.5, i), 1.0, 1.0,
    ...
    ax.set_xlabel("Reordered Features (Omics + Cancer)", fontsize=14)
    ax.set_ylabel("Top-k Genes (Cluster Ordered)", fontsize=14)

    # Remove all legends (including cluster)
    if ax.legend_:
        ax.legend_.remove()
...
def plot_bio_biclustering_clustermap(
    ...
    # Normalize and scale relevance scores
    # relevance_scores = (relevance_scores - relevance_scores.min()) / (relevance_scores.max() - relevance_scores.min()) * 200
    relevance_scores = normalize(relevance_scores, axis=1)

    # Default omics and cluster colors
    if omics_colors is None:
        omics_colors = {
            'cna': '#9370DB', 'ge': '#228B22', 'meth': '#00008B', 'mf': '#b22222'
        }
    ...
    if gene_names is not None:
        ...
        for i, gene in enumerate(sorted_gene_names):
            topk_indices = np.argsort(-sorted_relevance[i])[:top_k]
            topk_features = [df.columns[j] for j in topk_indices]
            topk_scores = sorted_relevance[i][topk_indices]
            gene_df = pd.DataFrame({"Feature": topk_features, "Score": topk_scores})
            gene_df.to_csv(os.path.join(topk_dir, f"{gene}_top{top_k}_features.csv"), index=False)
    ...
    # Color column tick labels by omics color
    for label in ax.get_xticklabels():
        text = label.get_text()
        for omics in omics_order:
            if text.startswith(omics.upper()):
                label.set_color(omics_colors[omics])
                label.set_rotation(90)
                label.set_fontsize(9)
                break

    # Color row cluster stripes (optional)
    if row_labels is not None:
        sorted_row_labels = row_labels[np.argsort(row_labels)]
        for i, cl in enumerate(sorted_row_labels):
            ax.add_patch(
                plt.Rectangle(
                    (-1.5, i), 1.0, 1.0,
                    linewidth=0,
                    facecolor=to_rgba(cluster_colors.get(cl, "#FFFFFF")),
                    clip_on=False
                )
            )

    ax.set_xlabel("Omics Feature (Omics + Cancer)", fontsize=14)
    ax.set_ylabel("Genes (Cluster Sorted)", fontsize=14)
    ax.set_title("Omics-Gene Relevance Heatmap", fontsize=16)
    ...
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"[Saved] Omics-gene heatmap → {output_path}")
...
def plot_bio_biclustering_heatmap(
    ...
    # Create output dirs and save CSVs
    cluster_output_dir = output_path.replace(".png", "_cluster_csvs")
    os.makedirs(cluster_output_dir, exist_ok=True)
    for cluster_id in np.unique(sorted_clusters):
        cluster_df = df_ordered_scores[df_ordered_scores["Cluster"] == cluster_id]
        cluster_df.to_csv(os.path.join(cluster_output_dir, f"cluster_{cluster_id}_genes.csv"))
    df_ordered_scores.to_csv(output_path.replace(".png", "_ordered_scores.csv"))
    ...
    # Cluster strips
    for i, cluster in enumerate(sorted_clusters):
        ax.add_patch(plt.Rectangle((-0.5, i), 1.0, 1.0, linewidth=0, facecolor=to_rgba(CLUSTER_COLORS.get(cluster, '#FFFFFF')), clip_on=False))
    ...
    for cluster, size in zip(unique_clusters, cluster_sizes):
        center_y = start_idx + size / 2
        ax.text(-1.5, center_y, f"{size}", va='center', ha='right', fontsize=20, fontweight='bold')
        start_idx += size
    ...
    for label, color in zip(ax.get_xticklabels(), feature_colors):
        label.set_color(color)
    ax.set_xlabel("Cancer Features (Sorted by Omics & Saliency)", fontsize=20)
    ...
    # Optional: clusterwise contributions
    plot_bio_clusterwise_feature_contributions(
        args=args,
        relevance_scores=relevance_scores,
        row_labels=row_labels,
...
def plot_biclustering_input_feature_heatmap_(
    ...
    assert saliency_matrix.shape[0] == len(node_names_topk), "Feature matrix row count must match number of top-k genes"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    cluster_df = pd.DataFrame({"Gene": reordered_gene_names, "Cluster": reordered_cluster_labels})
    cluster_path = output_path.replace(".png", "_cluster_labels.csv")
    cluster_df.to_csv(cluster_path, index=False)
    print(f"[Saved] Cluster labels → {cluster_path}")
    ...
    ax.set_xlabel("Reordered Features (Omics + Cancer)", fontsize=20)
    ax.set_ylabel("Top-k Genes (Cluster Ordered)", fontsize=20)
...
def apply_full_spectral_biclustering_bio_dark(
    graph,
    summary_bio_features,
    node_names_topk,
    omics_splits,
    ...
    n_clusters_col=5,
    cap_negatives=True,
    filter_gene_contrib=True,
    contrib_range=(50, 800),  # tuned for IG
    normalize_rows=True,      # normalize each gene profile
    apply_smoothing=True      # optional Gaussian smoothing
):
    print("🧪 Running Spectral Biclustering on IG-based summary bio features...")

    if topk_node_indices is None:
        raise ValueError("`topk_node_indices` must be provided.")

    matrix = summary_bio_features.copy()
    assert matrix.shape[1] == 64, f"Expected 64 summary features, got {matrix.shape[1]}"

    # === Cap negative values before filtering
    if cap_negatives:
        matrix[matrix < 0] = 0
        print("🔻 Negative values capped to 0.")

    # === Then do filtering based on capped matrix
    row_sums = matrix.sum(axis=1)
    print("🔥 Row sum stats after capping:")
    print(f"  min: {row_sums.min()}, max: {row_sums.max()}, mean: {row_sums.mean()}")

    if filter_gene_contrib:
        mask = (row_sums >= contrib_range[0]) & (row_sums <= contrib_range[1])
        if np.sum(mask) == 0:
            raise ValueError(f"No genes passed filter with range {contrib_range}")
        matrix = matrix[mask]
        topk_node_indices = np.array(topk_node_indices)[mask]
        node_names_topk = np.array(node_names_topk)[mask]
        print(f"📉 Filtered genes by sum ∈ {contrib_range}: {np.sum(mask)} genes retained")


    # === Optional: normalize row profiles (l1-normalization)
    if normalize_rows:
        matrix = normalize(matrix, norm="l1", axis=1)
        print("📐 Normalized gene rows (L1 norm)")

    # === Optional: Gaussian smoothing
    if apply_smoothing:
        matrix = gaussian_filter(matrix, sigma=1)
        print("🌫️ Applied Gaussian smoothing")
    ...
    pred_counts, _ = count_predicted_genes_per_cluster(
        row_labels, node_names_topk, predicted_cancer_genes, n_clusters_row
    )
    save_predicted_counts(pred_counts, save_predicted_counts_path)
    ...
    sorted_row_indices = np.argsort(row_labels)
    sorted_col_indices = np.argsort(col_labels)
    sorted_matrix = matrix[sorted_row_indices][:, sorted_col_indices]

    df = pd.DataFrame(sorted_matrix)
    df.index = [node_names_topk[i] for i in sorted_row_indices]

    if isinstance(omics_splits, dict):
        col_feature_labels = [""] * matrix.shape[1]
        for omics, (start, end) in omics_splits.items():
            for j in range(start, end + 1):
                if j < len(col_feature_labels):
                    col_feature_labels[j] = f"{omics}_{j - start}"
        df.columns = [col_feature_labels[j] for j in sorted_col_indices]
    else:
        df.columns = [f"Omics_{j}" for j in sorted_col_indices]
    ...
    df.to_csv(sorted_matrix_path)
    print(f"📄 Saved sorted matrix: {sorted_matrix_path}")
    ...
    print(f"✅ Spectral biclustering complete.")
    print(f"📸 Heatmap saved to: {output_path_heatmap}")
    print(f"📸 Clustermap saved to: {output_path_genes_clusters}")
...
def plot_biclustering_input_feature_heatmap(
    ...
    feature_names = [f"{omics}: {cancer}" for omics in omics_order for cancer in cancer_names]
    feature_names = feature_names[:saliency_matrix.shape[1]]
    reordered_feature_labels = [feature_names[i] for i in col_order]

    cluster_df = pd.DataFrame({"Gene": reordered_gene_names, "Cluster": reordered_cluster_labels})
    cluster_df.to_csv(output_path.replace(".png", "_cluster_labels.csv"), index=False)

    matrix_csv_path = output_path.replace(".png", "_heatmap_matrix.csv")
    pd.DataFrame(clustered_matrix, index=reordered_gene_names, columns=reordered_feature_labels).to_csv(matrix_csv_path)
    ...
    topk_df.columns = [f"Top{i+1}" for i in range(top_k)]
    topk_df.to_csv(output_path.replace(".png", f"_top{top_k}_features_per_gene.csv"))
    ...
    # === Tick label coloring by omics
    for label in ax.get_xticklabels():
        text = label.get_text()
        for omics, color in OMICS_COLORS.items():
            if text.startswith(omics):
                label.set_color(color)
...
def train(args):
    ...
    '''graph_bio, row_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_full_spectral_biclustering_bio(
        graph=graph,
        summary_bio_features=summary_bio_features[top_gene_indices],
    ...
    '''graph_bio, row_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts = apply_bipartite_biclustering_from_saliency(
        graph=graph,
    ...
    for cluster_type, row_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            # 🟢 Apply bio spectral biclustering
            ...
            # print(f"✅ Finished novel PCG plot for {tag} clusters.")


            # # 🟢 Call the KCG plot function
            # plot_kcg_cancer_genes(
            #     clusters=list(bio_total_counts.keys()),
            #     kcg_count=kcg_count_per_cluster,
            #     total_genes_per_cluster=bio_total_counts,
            ...
            # print(f"✅ Finished KCG plot for {tag} clusters.")
