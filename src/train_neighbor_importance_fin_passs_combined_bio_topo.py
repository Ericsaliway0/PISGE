...
def train(args):
    ...
    # Build heatmap matrix with -log10(p-value) where p < 0.05
    for cluster_type in ['bio', 'topo']:
        for cid, df in enrichment_results[cluster_type].items():
            colname = f"{cluster_type}_{cid}"
            vals = {}
    ...
    # Filter rows where max -log10(p-value) across columns is > 1
    heatmap_data = heatmap_data[heatmap_data.max(axis=1) > 1]

    # Select 60 rows evenly from the filtered matrix
    if heatmap_data.shape[0] > 60:
        step = max(1, heatmap_data.shape[0] // 60)
        selected_indices = heatmap_data.index[::step][:60]
        heatmap_data = heatmap_data.loc[selected_indices]

    # Plot
    fig, ax = plt.subplots(figsize=(12, 0.25 * len(heatmap_data)))

    # Heatmap with narrow colorbar
    sns.heatmap(
        heatmap_data,
        cmap='YlOrRd',
        linewidths=0.4,
        ...
        vmax=5,
        ax=ax,
        cbar_kws={'shrink': 0.3, 'aspect': 10, 'label': '-log10(p-value)'}
    )

    # Tidy plot
    ax.set_title("Top Enriched Pathways per Cluster (p < 0.05)", fontsize=14)
    ax.set_xlabel("Cluster", fontsize=12)
    ax.set_ylabel("Pathway (Source)", fontsize=12)
    plt.xticks(rotation=90)
    sns.despine(ax=ax, trim=True)

    plt.tight_layout()
    enriched_terms_heatmap_path = os.path.join(output_dir, f"{args.model_type}_{args.net_type}_enriched_terms_heatmap_epo{args.num_epochs}.png")
    plt.savefig(enriched_terms_heatmap_path)
    plt.close()
