...
def compute_saliency_relevance(model, graph, features, target_class=None):
    """
    Compute node-level saliency relevance using input gradients.

    Parameters:
    - model: trained GAT model
    - graph: DGL graph
    - features: input feature tensor
    - target_class: optional specific class to compute gradients against
    ...
    if target_class is not None:
        loss = scores[target_class]
    else:
        loss = scores.sum()

    loss.backward()
    saliency = features.grad.detach().abs()

    return saliency

def compute_node_attention_relevance(g, attention_weights):
    """
    Compute node relevance based on attention weights from GATConv.

    Parameters:
    - g: DGL graph
    - attention_weights: Tensor of shape [num_edges, num_heads]
    ...
    relevance = torch.zeros(g.num_nodes(), device=device)

    src, dst = g.edges()
    attn_mean = attention_weights.mean(dim=1)  # [num_edges]
    relevance = relevance.index_add(0, dst, attn_mean)

    return relevance

def combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=0.5):
    """
    Combine attention and saliency relevance scores.

    Parameters:
    - attn_relevance: [num_nodes] tensor
    - saliency_relevance: [num_nodes, feature_dim] tensor
    - alpha: float, weight for attention (0.0–1.0)

    Returns:
    - Combined relevance scores [num_nodes, feature_dim]
    """
    attn_norm = attn_relevance.unsqueeze(1)
    combined = alpha * attn_norm + (1 - alpha) * saliency_relevance
    return combined

def get_prediction_scores(model, graph, features):
    model.eval()
    with torch.no_grad():
        logits, attn_weights = model(graph, features, return_attention=True)
        scores = torch.sigmoid(logits).squeeze()
    return scores.cpu().numpy(), attn_weights

def compute_attention_relevance(g, attn_tuple):
    """
    Compute node attention relevance from a tuple of (attn1, attn2) from GAT layers.
    
    Each attn is [num_edges, num_heads].
    Returns per-node relevance [num_nodes].
    """
    import torch
    attn1, attn2 = attn_tuple
    device = attn1.device
    relevance = torch.zeros(g.num_nodes(), device=device)
    ...
    relevance /= 2  # average over layers
    return relevance

def compute_attention_relevance(g, attention_weights_tuple):
    """
    Compute node attention relevance scores from all GAT layers.

    Parameters:
    - g: DGL graph
    - attention_weights_tuple: Tuple of attention tensors from GAT layers

    Returns:
    - relevance: [num_nodes] tensor
    """
    import torch

    attn1, attn2 = attention_weights_tuple
    device = attn1.device
    relevance = torch.zeros(g.num_nodes(), device=device)

    src, dst = g.edges()

    for attention_weights in attention_weights_tuple:
        # attention_weights: [num_edges, num_heads] -> [num_edges]
        attn_mean = attention_weights.mean(dim=1).flatten()

        # Accumulate per-destination node attention
        relevance = relevance.index_add(0, dst, attn_mean)

    # Optionally normalize
    relevance /= len(attention_weights_tuple)

    return relevance

def compute_saliency_relevance(model, graph, features, target_class=None):
    """
    Compute saliency relevance as gradient magnitude w.r.t. inputs.

    Returns: [num_nodes, num_features] tensor
    """
    model.eval()
    features = features.clone().detach().requires_grad_(True)
...
def compute_node_attention_relevance(g, attention_weights_tuple):
    """
    Compute node relevance from multiple layers of attention weights.

    Parameters:
    - g: DGL graph
    - attention_weights_tuple: Tuple of attention weights from GAT layers,
                               each of shape [num_edges, num_heads]

    Returns:
    - relevance_scores: Tensor of shape [num_nodes], per-node relevance
    """
    import torch
    attn1, attn2 = attention_weights_tuple
    device = attn1.device
    ...
    relevance /= len(attention_weights_tuple)  # average relevance over layers
    return relevance

def combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=0.5):
    """
    Combine per-node attention and saliency relevance scores.
    - attn_relevance: [num_nodes]
    - saliency_relevance: [num_nodes, feature_dim]
    
    Returns:
    - combined_relevance: [num_nodes, feature_dim]
    """
    attn_norm = attn_relevance.unsqueeze(1)
    return alpha * attn_norm + (1 - alpha) * saliency_relevance

def interpret_with_attention_saliency(model, graph, features, topk_gene_indices, alpha=0.5):
    """
    Run interpretability with attention and saliency, combine, and split into bio/topo.
    """
    # Step 1: Get logits and attention
    model.eval()
    with torch.no_grad():
        logits, (attn1, attn2) = model(graph, features, return_attention=True)
        scores = torch.sigmoid(logits).squeeze()

    # Step 2: Attention relevance
    attn_relevance = compute_attention_relevance(graph, (attn1, attn2))  # [num_nodes]

    # Step 3: Saliency relevance
    saliency_relevance = compute_saliency_relevance(model, graph, features)  # [num_nodes, feature_dim]

    # Step 4: Combine
    combined_relevance = combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=alpha)

    # Step 5: Split into bio and topo
    relevance_bio = combined_relevance[:, :1024]
    relevance_topo = combined_relevance[:, 1024:]

    # Step 6: Get relevance for top-K genes
    topk_indices_tensor = torch.tensor(topk_gene_indices, dtype=torch.long)
    topk_bio = relevance_bio[topk_indices_tensor]
    topk_topo = relevance_topo[topk_indices_tensor]

    return {
        'scores': scores,
        'relevance_bio': topk_bio,
        'relevance_topo': topk_topo,
        'combined': combined_relevance,
        'attention_only': attn_relevance,
        'saliency_only': saliency_relevance
    }
...
def compute_attention_relevance(g, attention_weights_tuple):
    ...
    for attention_weights in attention_weights_tuple:
        # Robust flattening to 1D
        attn_mean = attention_weights.mean(dim=1).view(-1)

        # Accumulate relevance for destination nodes
        relevance = relevance.index_add(0, dst, attn_mean)

    relevance /= len(attention_weights_tuple)
    return relevance

def compute_saliency_relevance(model, g, features):
    """
    Compute saliency relevance (gradient-based) for each input feature per node.

    Returns:
    - relevance: Tensor of shape [num_nodes, feature_dim]
    """
    import torch
    model.eval()
    features = features.clone().detach().requires_grad_(True)
    logits = model(g, features)
    probs = torch.sigmoid(logits).squeeze()
    probs.sum().backward()  # Sum to get gradients for all outputs

    saliency = features.grad.detach() * features.detach()
    return saliency

def combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=0.5):
    """
    Combine attention-based and saliency-based relevance.

    Parameters:
    - attn_relevance: [num_nodes] tensor
    - saliency_relevance: [num_nodes, feature_dim] tensor
    - alpha: blending factor (0 = saliency only, 1 = attention only)

    Returns:
    - combined: [num_nodes, feature_dim] tensor
    """
    # Normalize attention relevance to [0, 1]
    attn_norm = (attn_relevance - attn_relevance.min()) / (attn_relevance.max() - attn_relevance.min() + 1e-6)
    attn_norm = attn_norm.unsqueeze(1)  # Shape: [num_nodes, 1]

    # Broadcast attention over features and combine
    combined = alpha * attn_norm + (1 - alpha) * saliency_relevance
    return combined
...
def interpret_with_attention_saliency(model, graph, features, topk_gene_indices, alpha=0.5):
    ...
    attn_relevance = compute_attention_relevance(graph, (attn1, attn2))
    saliency_relevance = compute_saliency_relevance(model, graph, features)
    combined_relevance = combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=alpha)
...
def compute_node_attention_relevance(g, attention_weights_tuple):
    """
    Compute node relevance by aggregating attention weights from multiple GAT layers.

    Parameters:
    - g: DGL graph.
    - attention_weights_tuple: Tuple of attention tensors, each of shape [num_edges, num_heads].

    Returns:
    - relevance_scores: Tensor of shape [num_nodes], per-node attention relevance.
    """
    import torch
    ...
    # Use the device of the first layer's attention weights
    device = attention_weights_tuple[0].device
    relevance = torch.zeros(num_nodes, device=device)

    for attn in attention_weights_tuple:
        # Mean over heads and flatten to 1D
        attn_mean = attn.mean(dim=1).flatten()  # shape: [num_edges]
        relevance = relevance.index_add(0, dst, attn_mean)
...
def compute_node_attention_relevance(g, attention_weights_tuple):
    ...
    for attn in attention_weights_tuple:
        # Ensure shape is [num_edges]
        attn_mean = attn.mean(dim=1).flatten()
        relevance = relevance.index_add(0, dst, attn_mean)
...
def train(args):
    ...
    # === Get prediction scores and attention ===
    '''scores, attn_weights = get_prediction_scores(model, graph, features)

    # === Compute attention relevance ===
    attn_relevance = compute_node_attention_relevance(graph, attn_weights)

    # === Compute saliency relevance ===
    saliency_relevance = compute_saliency_relevance(model, graph, features)

    # === Combine them ===
    relevance_scores = combine_attention_and_saliency(attn_relevance, saliency_relevance, alpha=0.5)
    ...
    results = interpret_with_attention_saliency(model, graph, features, top_gene_indices, alpha=0.6)

    # For visualization:
    relevance_scores_bio = results['relevance_bio'].cpu().numpy()
    relevance_scores_topo = results['relevance_topo'].cpu().numpy()
    relevance_scores = results['combined'].cpu().numpy()
    ##relevance_scores = extract_summary_features_np_bio(relevance_scores)
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        relevance_scores_topk = relevance_scores_subset
