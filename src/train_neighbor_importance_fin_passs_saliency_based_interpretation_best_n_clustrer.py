...
def plot_bio_topo_saliency_cuberoot(bio_scores, topo_scores, title="", save_path=None):
    ...
    plt.ylim(0, 1)
    plt.xlabel("Feature Index (0 – 63)", fontsize=16)
    plt.ylabel("Cube Root Transformed Score", fontsize=16)
    plt.title(title, fontsize=18)
    sns.despine()
    plt.tight_layout()
...
def best_balanced_IG_biclustering(ig_score_matrix, max_clusters=10, n_trials=20):
    """
    Performs spectral biclustering on the IG-based feature relevance matrix.

    Parameters:
        ig_score_matrix (np.ndarray): A 2D matrix (genes × omics features) of IG scores.
        max_clusters (int): Maximum number of biclusters (rows and columns) to test.
        n_trials (int): Number of trials to sample random seeds for balance optimization.

    Returns:
        best_model (SpectralBiclustering): The biclustering model with the most balanced row clustering.
        best_n_clusters (int): Number of biclusters selected via eigengap-style balance scoring.
    """
    best_model = None
    best_balance_score = float('inf')
    best_n_clusters = 2
...
def apply_full_spectral_biclustering_IG(
        graph, summary_bio_features, node_names, 
    ...
    print(f"Running Spectral Biclustering on 64-dim IG summary bio features with up to {max_clusters} clusters...")
    assert summary_bio_features.shape[1] == 64, f"Expected 64 summary features, got {summary_bio_features.shape[1]}"

    # === Optional filtering: focus on high-saliency genes
    # row_sums = summary_bio_features.sum(axis=1)
    # threshold = np.percentile(row_sums, 20)
    # high_saliency_indices = np.where(row_sums > threshold)[0]
    # filtered_features = summary_bio_features[high_saliency_indices]

    # === Run best-balanced biclustering (with automatic cluster selection)
    bicluster_model, best_n_clusters = best_balanced_IG_biclustering(
        #filtered_features,
        summary_bio_features,
    ...
    row_labels = bicluster_model.row_labels_
    col_labels = bicluster_model.column_labels_
    row_labels = (best_n_clusters - 1) - row_labels  # Flip label order if desired
    ...
    print(f"✅ Spectral Biclustering complete. Best cluster count: {best_n_clusters}")

    # === Save results
    save_graph_with_clusters(graph, save_path)
    save_cluster_labels(row_labels, save_cluster_labels_path)
    total_genes_per_cluster = compute_total_genes_per_cluster(row_labels, best_n_clusters)
    save_total_genes_per_cluster(total_genes_per_cluster, save_total_genes_per_cluster_path)

    pred_counts, predicted_indices = count_predicted_genes_per_cluster(
        row_labels, node_names, predicted_cancer_genes, best_n_clusters
    )
    save_predicted_counts(pred_counts, save_predicted_counts_path)
    ...
    output_path_unclustered_heatmap = output_path_heatmap.replace(".png", "_unclustered.png")
    # plot_bio_heatmap_raw_unsorted(summary_bio_features, predicted_indices, output_path_unclustered_heatmap)
    # plot_tsne(summary_bio_features, row_labels, predicted_indices, best_n_clusters, output_path_genes_clusters)
    # plot_bio_heatmap_unsort(summary_bio_features, row_labels, col_labels, predicted_indices, output_path_heatmap)

    return graph, row_labels, col_labels, total_genes_per_cluster, pred_counts, best_n_clusters
...
def train(args):
    ...
    graph_bio, cluster_labels_bio, col_labels_bio, bio_total_counts, bio_pred_counts, best_k = apply_full_spectral_biclustering_IG(
        graph=graph,  # DGL graph
        summary_bio_features=relevance_matrix_bio, 
        node_names=node_names,  # List of all node names (indexed by original graph node ID)
        predicted_cancer_genes=predicted_cancer_genes,  # Set or list of predicted cancer genes

        # Paths to save intermediate and final outputs
        save_path=bio_graph_path,
        save_cluster_labels_path=bio_cluster_labels_path,
        save_total_genes_per_cluster_path=bio_total_per_cluster_path,
        save_predicted_counts_path=bio_pred_counts_path,
        ...
        topk_node_indices=top_gene_indices,
        max_clusters=10,  # Max number of biclusters to try during model selection
        n_trials=20       # Number of random seeds to try per cluster count
    )
    # Assign cluster labels
