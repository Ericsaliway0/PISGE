...
def plot_neighbor_relevance_one_color_pass(neighbor_scores, gene_name="Gene", node_id_to_name=None, output_path=None):
    ...
    # Top 10 only
    top_10 = dict(sorted(filtered_scores.items(), key=lambda x: -x[1])[:1000])
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio(
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        genes_sorted = sorted(genes, key=lambda g: scores[name_to_index[g]], reverse=True)[:1000]
    ...
    try:
        ...
        fig.write_image(png_save_path, scale=2, width=800, height=1200)
