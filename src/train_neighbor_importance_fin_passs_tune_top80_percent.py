...
def plot_topo_biclustering_heatmap(
    ...
    for cluster in unique_clusters:
        ...
        # Sum of relevance per gene in this cluster
        cluster_relevance_sums = relevance_scores[cluster_indices].sum(axis=1)
        ...
        # Keep top 50%
        top_n = max(1, int(len(sorted_within) * top_fraction))  # at least 1 gene
        top_sorted = sorted_within[:top_n]
        
        new_order.extend(top_sorted)

    # Concatenate results
    '''sorted_scores = np.vstack(top_sorted)
    sorted_clusters = np.array(top_sorted)
    if gene_names is not None:
    ...
    sorted_clusters = cluster_labels[new_order]
    if gene_names is not None:
        gene_names = [gene_names[i] for i in new_order]
...
def plot_bio_biclustering_heatmap(
    ...
    # Normalize features-----------------------------------------------------------------------------------------------------------------
    relevance_scores = StandardScaler().fit_transform(relevance_scores)#*5
