...
# Call the plot function
    ...
    data = pd.DataFrame(columns=["Cluster", "Interactions"])
    cluster_colors = get_tab20_colors(data)
    ...
    # Step 1: Generate cluster colors ONCE
    ## cluster_colors = get_tab20_colors(data)

    # Step 2: Generate plots using consistent colors
    plot_interactions_with_kcgs(data, "box_plot.png", cluster_colors)
    cluster_and_visualize_predicted_genes(graph, predicted_cancer_genes, node_names, 
                                          "tsne_plot.png", "predicted_cancer_genes.csv", 
                                          num_clusters=len(cluster_colors), cluster_colors=cluster_colors)
...
def get_tab20_colors(data):
    """
    Extracts the colors used by seaborn's 'tab20' palette for the given clusters.

    Args:
        data (pd.DataFrame): The dataset containing a 'Cluster' column.

    Returns:
        dict: A dictionary mapping each unique cluster to a hex color code.
    """
    unique_clusters = sorted(data['Cluster'].unique())  # Ensure ordering
    n_clusters = len(unique_clusters)  # Number of unique clusters

    # Get colors from the 'tab20' palette
    tab20_colors = sns.color_palette("tab20", n_clusters)

    # Convert colors to hex format and map to clusters
    cluster_colors = {cluster: to_hex(color) for cluster, color in zip(unique_clusters, tab20_colors)}

    return cluster_colors


def plot_interactions_with_kcgs(data, output_path, cluster_colors):
    """
    ...
    Args:
        ...
        cluster_colors (dict): A dictionary mapping clusters to colors.
    """
    plt.figure(figsize=(10, 5))
    sns.set_style("white")

    # Apply consistent colors to box plot
    palette = [cluster_colors[cluster] for cluster in sorted(data['Cluster'].unique())]
...
def cluster_and_visualize_predicted_genes(graph, predicted_cancer_genes, node_names, 
                                          output_path_genes_clusters, output_path_all_predicted_genes, 
                                          num_clusters=12, cluster_colors=None):
    """
    ...
    Args:
        graph: Graph containing node embeddings.
        predicted_cancer_genes (list): Indices of predicted cancer genes.
        node_names (list): List of gene names.
        output_path_genes_clusters (str): Path to save the cluster visualization.
        output_path_all_predicted_genes (str): Path to save predicted gene clusters.
        num_clusters (int): Number of clusters.
        cluster_colors (dict): Cluster-to-color mapping.
    ...
    plt.savefig(output_path_genes_clusters, dpi=300)
    plt.close()
    
    print(f"Cluster visualization saved to {output_path_genes_clusters}")

    return cluster_labels, cluster_predicted_genes, total_genes_per_cluster
