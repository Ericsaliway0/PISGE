...
def train(args):
    ...
    # Step 1: Collect only significant terms where -log10(p) > 1
    all_labeled_terms = set()
    heatmap_data = pd.DataFrame()
        ...
        for cid, df in enrichment_results[cluster_type].items():
            ...
            for _, row in df.iterrows():
                ...
                if log_p > 1:
                    ...
                    vals[term] = log_p
                    all_labeled_terms.add(term)
            heatmap_data[colname] = pd.Series(vals)

    # Step 2: Keep only terms with at least one visible entry
    ...
    plt.title("Enriched Pathways per Cluster (-log10(p) > 1)")
    plt.xlabel("Cluster")
    plt.ylabel("Pathway (with Source)")
    plt.tight_layout()
    plt.savefig("results/enriched_terms_heatmap_logp_gt1.png")
    plt.close()
