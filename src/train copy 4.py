...
def apply_spectral_biclustering(graph, embeddings, n_clusters, save_path):
    """
    Applies Spectral Biclustering and optionally saves the graph.
    ...
    Args:
        ...
        n_clusters (int)
        normalize (bool)
        save_path (str or None): Path to save the graph components (e.g., 'saved_graph.pth')
    """
...
def apply_spectral_biclustering_no_saving(graph, embeddings, n_clusters=12, normalize=True):
    """
...
def train(args):
    ...
    topk_indices = torch.topk(scores_tensor, topk).indices
    pcg_nodes = topk_indices.tolist()
    ...
    topk_named_scores = [non_labeled_scores[i] for i in topk_indices]
    ...
    node_names_topk = [node_names[i] for i in topk_indices] 
    
    # Convert to tensor for indexing
    topk_node_indices_tensor = torch.tensor(topk_indices)
