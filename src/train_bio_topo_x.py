...
def compute_relevance_scores_for_feature(model, graph, features, node_indices=None, use_abs=True):
    """
...
def compute_relevance_scores(
    ...
    with torch.enable_grad():
        ...
        logits = model(graph, features)
        probs = torch.sigmoid(logits.squeeze())  # Use sigmoid for probabilities
        ...
        for i, node_idx in enumerate(node_indices):
            ...
            # Backward pass on the selected probability
            probs[node_idx].backward(retain_graph=(i != len(node_indices) - 1))

            grads = features.grad[node_idx]
            saliency = grads.abs().detach()  # Take absolute gradient as relevance

            # Optional normalization
            if normalize:
                saliency = saliency / (saliency.sum() + 1e-9)

            # Split into feature groups if specified
            if feature_groups:
                group_scores = {}
...
def train(args):
    ...
    redicted_cancer_genes = [i for i, _ in ranking[:1000]]   
    top_gene_indices = [name_to_index[name] for name in predicted_cancer_genes if name in name_to_index]
    ...
    # Convert list of top node indices to tensor
    topk_node_indices_tensor = torch.tensor(top_gene_indices)

    # ✅ Ensure the tensor is of type int64 (required by torch indexing)
    if topk_node_indices_tensor.dtype != torch.int64:
        topk_node_indices_tensor = topk_node_indices_tensor.to(torch.int64)

    # ✅ Ensure the tensor is 1D (important for indexing)
    if topk_node_indices_tensor.dim() != 1:
        topk_node_indices_tensor = topk_node_indices_tensor.view(-1)

    # Define feature groups if using the 1024+1024 bio/topo split
    '''feature_groups = {
        "bio": (0, 1024),
        "topo": (1024, 2048)
    }'''
    ...
    # ✅ Compute relevance scores for selected top-K nodes
    ...
    feature_group_order = ['mf', 'cna', 'ge', 'meth']  # make sure it matches your actual dict

    relevance_scores_tensor = torch.stack([
        torch.cat([relevance_scores[node_idx][group] for group in feature_group_order])
        for node_idx in top_gene_indices
    ])

    # Convert dict to tensor
    '''relevance_scores_tensor = torch.stack([
        relevance_scores[node_idx]["all"]
        for node_idx in top_gene_indices
    ])'''
    # ✅ Filter relevance scores and cluster labels
    relevance_scores_topk = relevance_scores[relevance_scores_tensor]
    cluster_labels_topk = graph.ndata['cluster'][topk_node_indices_tensor]

    # ✅ Define feature names (16 cancers × 4 omics = 64)
    omics_types = ['mf', 'cna', 'ge', 'meth']
    cancer_types = ['BRCA', 'KIRC', 'READ', 'PRAD', 'STAD', 'HNSC', 'LUAD',
                    'THCA', 'BLCA', 'ESCA', 'LIHC', 'UCEC', 'COAD', 'LUSC', 'CESC', 'KIRP']
    feature_names = [f"{cancer}_{omics}" for omics in omics_types for cancer in cancer_types]

    # ✅ Define omics split for grouping in visualizations
    omics_splits = {
        'mf': (0, 15),
        'cna': (16, 31),
        'ge': (32, 47),
    ...
    # ✅ Path to save spectral biclustering heatmap
    spectral_biclustering_heatmap_path = os.path.join(
        output_dir, f"{args.model_type}_{args.net_type}_spectral_biclustering_heatmap_epo{args.num_epochs}.png"
    )

    # ✅ Plot and return DataFrame of sorted gene-feature contributions
    ...
    # ✅ Compute summarized feature scores (top_k × 4 features)
    ...
    # ✅ Prepare to plot neighbor relevance
    ...
    # ✅ Get 1-hop + 2-hop neighbors of confirmed genes
    neighbors_dict = get_neighbors_gene_names(graph, node_names, name_to_index, confirmed_genes)

    # ✅ Visualize neighbor contributions for confirmed genes
    for gene in confirmed_genes:
        if gene not in name_to_index:
...
def train_graph_with_edge_node_lrp_pass(args):
    ...
    for gene in confirmed_genes:
        ...
        # Define which nodes to explain — for example, top predicted ones
        node_indices = top_genes_indice  # or a manual list like [123, 456, ...]
