...
# Your other imports like load_graph_data, process_predictions, choose_model etc.
...
def train(args):
    ...
    # Prepare inputs for IG
    top_gene_indices = torch.tensor(top_gene_indices, dtype=torch.long, device=device)

    # Use top genes for attribution
    subset_features = embeddings[top_gene_indices].detach().clone().requires_grad_(True)

    # Define forward function for IG — still uses full feature matrix

    def ig_forward_fn(batch_subset_features):
        # batch_subset_features: [n_steps, 1000, 2048]
        batch_size = batch_subset_features.shape[0]
        
        full_features = embeddings.clone().detach().unsqueeze(0).repeat(batch_size, 1, 1)  # [n_steps, num_nodes, feat_dim]
        full_features[:, top_gene_indices] = batch_subset_features  # replace only top genes

        output = model(graph, full_features)  # expects batch input
        return output.squeeze()[:, top_gene_indices]  # return [n_steps, 1000]

    relevance_scores = []
    for i in range(0, len(top_gene_indices), 100):  # chunk size = 100
        idx_chunk = top_gene_indices[i:i+100]
        chunk_features = embeddings[idx_chunk].detach().clone().requires_grad_(True)
        def chunk_forward_fn(batch_subset_features):
            total_steps, feat_dim = batch_subset_features.shape
            chunk_size = len(idx_chunk)
            n_steps = total_steps // chunk_size

            batch_subset_features = batch_subset_features.view(n_steps, chunk_size, feat_dim)
            full_features = embeddings.detach().unsqueeze(0).repeat(n_steps, 1, 1)

            for step in range(n_steps):
                full_features[step, idx_chunk] = batch_subset_features[step]

            output = model(graph, full_features)
            return output.squeeze()[:, idx_chunk]


        ig = IntegratedGradients(chunk_forward_fn)
        chunk_relevance = ig.attribute(
            inputs=chunk_features,
            baselines=torch.zeros_like(chunk_features),
            target=None,
            n_steps=len(top_gene_indices)
        )
        relevance_scores.append(chunk_relevance.cpu())

    relevance_scores = torch.cat(relevance_scores, dim=0)  # final shape: [len(top_gene_indices), feat_dim]




    print("Attribution complete. Shape:", relevance_scores.shape)

    # You can now save or visualize relevance_scores as needed


    ##return relevance_scores, lrp_node_indices


    # Save or return relevance_scores
    np.save(os.path.join(output_dir, f"{args.model_type}_relevance_scores.npy"), relevance_scores.detach().cpu().numpy())
    print("Attributions saved.")

    ##return model, ranking, relevance_scores
    ...
    ##print("Computing Integrated Gradients relevance scores for predicted cancer genes and their neighbors...")
    ...
    class WrappedModel(nn.Module):
        ...
        def forward(self, features):
            return self.model(self.graph, features).squeeze()

    # Wrap the model for IG
    wrapped_model = WrappedModel(model, graph).to(device)
    ig = IntegratedGradients(model)

    # Create baseline (e.g., zero vector) and input for IG
    input_tensor = embeddings.clone().detach().requires_grad_(True).to(device)
    baseline = torch.zeros_like(input_tensor).to(device)

    # Combine top gene indices and neighbors
    target_indices = torch.tensor(top_gene_indices, dtype=torch.long).to(device)
    ig_node_indices = torch.tensor(list(set(top_gene_indices) | neighbor_indices), dtype=torch.long).to(device)

    # Preallocate relevance matrix
    relevance_scores = torch.zeros(graph.num_nodes(), input_tensor.shape[1]).to(device)

    # Compute IG attributions for each top gene
    for idx in tqdm(target_indices.tolist(), desc="IG Attribution"):
        ...
        relevance_scores[idx] = attributions[idx]
    '''
    # Move to CPU for downstream plotting
