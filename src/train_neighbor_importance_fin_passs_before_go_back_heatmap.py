...
def plot_pcg_cancer_genes_wide_left_space(
    clusters,
...
def apply_spectral_biclustering_bio_row(graph, bio_embeddings, node_names, predicted_cancer_genes,
    ...
    ##from your_module import extract_summary_features_np_bio  # adjust this import
    bio_np = bio_embeddings.cpu().numpy()
    node_features = extract_summary_features_np_bio(bio_np)
...
def apply_spectral_biclustering_topo_row(graph, topo_embeddings, node_names, predicted_cancer_genes,
                                     n_clusters, save_path, save_cluster_labels_path,
                                     save_total_genes_per_cluster_path, save_predicted_counts_path,
                                     output_path_genes_clusters):
...
def plot_bio_clusterwise_feature_contributions_not_group(
    args,
    relevance_scores,           # 2D array (samples x features)
    cluster_labels,             # 1D array of cluster assignments
    feature_names,              # List of feature names (e.g., MF: BRCA, ...)
...
def apply_full_spectral_biclustering_topo(graph, topo_embeddings, node_names,
    ...
    print(f"Running FULL Spectral Biclustering (topo) with {n_clusters} clusters...")
    ...
    # Fit model (and get row/column order attributes)
    ##bicluster = SpectralBiclustering(n_clusters=n_clusters, method='log', random_state=42)
    ##bicluster_matrix = bicluster.fit_transform(node_features)
    ...
    print("Spectral Biclustering (topo) complete. Row and column cluster labels assigned.")

    # Save graph with topo clusters
    torch.save({
        'edges': graph.edges(),
        'features': graph.ndata['feat'],
        'labels': graph.ndata.get('label', None),
    ...
    plt.savefig(output_path_genes_clusters, bbox_inches="tight")
    plt.close()
    print(f"✅ t-SNE cluster visualization saved to {output_path_genes_clusters}")
    ...
    #sorted_matrix = node_features[bicluster.row_order_, :][:, bicluster.column_order_]
    sorted_matrix = node_features[np.argsort(bicluster.row_labels_), :][:, np.argsort(bicluster.column_labels_)]
    plt.figure(figsize=(16, 12))
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=-2, vmax=2)
    plt.title("Heatmap of Topo Features Sorted by Spectral Biclusters")
    ...
    plt.close()
    print(f"✅ Bicluster heatmap (topo) saved to {output_path_heatmap}")
...
def plot_topo_biclustering_heatmap_(
    ...
    if col_labels is not None:
        sorted_scores = sorted_scores[:, np.argsort(col_labels)]
    ...
    # Top contribution bar – separate top row (row 0)
    ax_bar = fig.add_subplot(gs[0, 2:45])

    # Main heatmap body (rows 1–12)
    ax = fig.add_subplot(gs[1:13, 2:45])

    # LRP curve beside heatmap
    ...
    # 🔹 Plot contribution bars above heatmap
    feature_means = sorted_scores.mean(axis=0)
    feature_means = feature_means / feature_means.max()
    bar_height = 0.15

    # Add contribution bar axis
    ax_bar = fig.add_subplot(gs[0, 2:45], sharex=ax)

    ax_bar.bar(
        np.arange(len(feature_means)) + 0.5,
        feature_means * bar_height,
        width=1.0,
        color="#B0BEC5",
        linewidth=0,
        alpha=0.6
    ...
    # Create LRP patch
    lrp_patch = Patch(facecolor='#7b241c', alpha=0.8, label='Saliency Sum')
...
def plot_topo_biclustering_heatmap_sort(
    args,
    relevance_scores,
    cluster_labels,
    output_path,
    ...
    sorted_scores = relevance_scores
    sorted_clusters = cluster_labels
    # Preserve original gene order if gene_names are provided
    if gene_names is not None:
...
def plot_sorted_unified_heatmap_9_plot_for_each(
    args,
    relevance_scores,          # [N, F] relevance scores for top genes
    cluster_labels,            # [N] cluster assignments for top genes
    gene_names,                # List[str], top-k gene names
    omics_splits,              # Dict[str, Tuple[int, int]], omics feature ranges
    output_path,               # str, path to save the unified heatmap
    tag="bio"                  # 'bio' or 'topo'
):
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    # === Within each cluster, sort by gene relevance ===
    final_row_order = []
    for cluster_id in np.unique(sorted_clusters):
        cluster_idx = np.where(sorted_clusters == cluster_id)[0]
        if len(cluster_idx) == 0:
            continue

        cluster_scores = sorted_scores[cluster_idx]
        gene_totals = cluster_scores.sum(axis=1)
        within_cluster_sort = cluster_idx[np.argsort(-gene_totals)]
        final_row_order.extend(within_cluster_sort)

    sorted_scores = relevance_scores[final_row_order]
    sorted_gene_names = [gene_names[i] for i in final_row_order]
    sorted_clusters = cluster_labels[final_row_order]
    ...
    for _, (start, end) in omics_splits.items():
        ...
        col_totals = group_scores.sum(axis=0)
        sorted_group_columns = np.argsort(-col_totals) + start
        final_col_order.extend(sorted_group_columns)

    final_scores = sorted_scores[:, final_col_order]

    # === Heatmap ===
    plt.figure(figsize=(14, max(6, 0.25 * len(sorted_gene_names))))
    sns.heatmap(
        final_scores,
        cmap='YlGnBu',
        yticklabels=sorted_gene_names,
    ...
    plt.title(f"Unified Cluster-Sorted Relevance Heatmap ({tag})")
    plt.xlabel("Sorted Features")
    plt.ylabel("Genes")
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    ...
    print(f"✅ Saved unified heatmap to {output_path}")

def plot_bio_biclustering_heatmap_no_good(
    args,
    relevance_scores,
    cluster_labels,
    omics_splits,
    ...
    # 🔹 Setup figure
    fig = plt.figure(figsize=(18, 17))
    gs = fig.add_gridspec(nrows=15, ncols=50)  # one more row
    ax_bar = fig.add_subplot(gs[0, 2:45])      # top row for bar
    ax = fig.add_subplot(gs[1:13, 2:45])       # heatmap goes below bar
    ...
    bar_height = 0.15  # or 0.07 to make it compact
    ...
    ax_bar.set_ylim(0, bar_height * 1.5)
    ax_bar.axis("off")
...
def plot_clusterwise_sorted_heatmap_bio_sorted(
    args,
    relevance_scores,
    cluster_labels,
    gene_names,
    ...
    if torch.is_tensor(relevance_scores):
        relevance_scores = relevance_scores.detach().cpu().numpy()

    # === Sort genes by cluster, then by within-cluster relevance sum ===
    sorted_indices = np.lexsort((
        -np.sum(relevance_scores, axis=1),  # sort by descending relevance
        cluster_labels  # then by cluster
    ))
    sorted_scores = relevance_scores[sorted_indices]
    sorted_gene_names = [gene_names[i] for i in sorted_indices]
    sorted_clusters = cluster_labels[sorted_indices]

    # === Sort columns by omics relevance within each group ===
    sorted_columns = []
    for _, (start, end) in omics_splits.items():
        group_scores = sorted_scores[:, start:end]
        col_totals = group_scores.sum(axis=0)
        sorted_group_columns = np.argsort(-col_totals) + start
        sorted_columns.extend(sorted_group_columns)
    final_scores = sorted_scores[:, sorted_columns]

    # === Feature colors for omics ===
    num_features = final_scores.shape[1]
    feature_colors = []
    for i in sorted_columns:
        for omics, (start, end) in omics_splits.items():
            if start <= i <= end:
                feature_colors.append(omics_colors.get(omics, "#AAAAAA"))
                break
        else:
            feature_colors.append("#AAAAAA")
    ...
    # === Plot heatmap ===
    vmax = np.percentile(final_scores, 99)
    sns.heatmap(
        final_scores,
        cmap=bluish_gray_gradient,
        vmin=0,
    ...
    # === Add x-tick labels and color them by omics group ===
    ax.set_xticks(np.arange(num_features) + 0.5)
    ax.set_xticklabels([""] * num_features, rotation=90, fontsize=14)

    # Optional: You can name them based on omics and order, or just leave blank.
    # To name them:
    feature_names = []
    for i in sorted_columns:
        for omics, (start, end) in omics_splits.items():
            if start <= i <= end:
                cancer_idx = i - start  # or use a fixed cancer_names list if available
                feature_names.append(f"{omics.upper()}:{cancer_idx}")
                break
        else:
    ...
    # === X-tick labels & omics bar annotation ===
    ax.set_xticks(np.arange(num_features) + 0.5)
    ax.set_xticklabels([""] * num_features, rotation=90)
    for label, color in zip(ax.get_xticklabels(), feature_colors):
        label.set_color(color)

    # === Omics contribution bar ===
    omics_means = {
        omics: final_scores[:, start:end+1].mean()
        for omics, (start, end) in omics_splits.items()
    }
    group_centers = {
        omics: (omics_splits[omics][0] + omics_splits[omics][1]) / 2 + 0.5
        for omics in omics_splits
    }
    mean_vals = np.array(list(omics_means.values()))
    norm_means = (mean_vals - mean_vals.min()) / (mean_vals.max() - mean_vals.min() + 1e-6)
    for i, omics in enumerate(omics_splits):
        ax.bar(
            x=group_centers[omics],
            height=0.15,
            width=omics_splits[omics][1] - omics_splits[omics][0] + 1,
            bottom=len(final_scores) + 1.5,
            color=omics_colors[omics],
            edgecolor='black',
            linewidth=1,
            alpha=0.3 + 0.7 * norm_means[i]
        )

    # === Relevance curve (right) ===
    lrp_sums = final_scores.sum(axis=1)
    lrp_sums = (lrp_sums - lrp_sums.min()) / (lrp_sums.max() - lrp_sums.min())
    y = np.arange(len(lrp_sums))
    ax_curve.fill_betweenx(
        y, 0, lrp_sums,
    ...
    ax_curve.set_xticks([0, 1])
    ax_curve.set_xticklabels(['0', '1'], fontsize=16)
    ax_curve.hlines(y=1.01, xmin=0, xmax=1, color='black', linewidth=1.5,
                    transform=ax_curve.get_xaxis_transform())
    ax_curve.xaxis.set_label_position('top')
    ax_curve.xaxis.set_ticks_position('top')
    ax_curve.set_ylim(0, len(lrp_sums))
    ax_curve.spines[['right', 'left', 'bottom', 'top']].set_visible(False)
    ax_curve.tick_params(axis='y', length=0)

    # === Legend ===
    ax_legend.axis("off")
    legend_handles = [Patch(color=color, label=omics.upper()) for omics, color in omics_colors.items()]
    legend_handles.append(Patch(color='#7b241c', label='Saliency Sum', alpha=0.8))
    ax_legend.legend(
    ...
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✅ Saved combined cluster-sorted heatmap to {output_path}")
...
def save_and_plot_confirmed_genes_bio_(
    args,
    node_names_topk,
    node_scores_topk,
...
def plot_bio_biclustering_heatmap_unsort(
    ...
    # 🔹 Reorder columns if col_labels are given
    if col_labels is not None:
    ...
    # 🔹 Compute per-feature mean relevance
    ...
    # 🔹 Plot omics bar
    for i, (mean_val, color) in enumerate(zip(normed_feature_means, feature_colors)):
        ax_bar.bar(
            x=i + 0.5,
            height=mean_val,
            width=1.0,
    ...
    # 🔹 LRP curve
    lrp_sums = relevance_scores.sum(axis=1)
    lrp_sums = (lrp_sums - lrp_sums.min()) / (lrp_sums.max() - lrp_sums.min())
    y = np.arange(len(lrp_sums))
    ax_curve.fill_betweenx(
        y, 0, lrp_sums,
    ...
    # 🔹 Optional: Cluster-wise feature contribution plot
    plot_bio_clusterwise_feature_contributions(
        args=args,
        relevance_scores=relevance_scores,
        cluster_labels=cluster_labels,
...
def plot_bio_biclustering_heatmap_oriii(
    args,
    relevance_scores,
    cluster_labels,
    omics_splits,
...
def plot_saliency_graph_0(G, target_idx, neighbor_saliencies, node_names, save_path=None):
    pos = nx.spring_layout(G, seed=42)  # Force-directed layout
    
    # Get edge weights based on saliency
    edges = list(G.edges())
    weights = [neighbor_saliencies.get(v, 0) for u, v in edges]

    # Normalize weights for better thickness
    max_weight = max(weights) if weights else 1
    normalized_weights = [w / max_weight * 5 for w in weights]  # up to 5 thickness

    # Draw nodes
    node_colors = []
    for node in G.nodes():
        if node == target_idx:
            node_colors.append('gold')  # Target gene
        else:
            node_colors.append('lightcoral')  # Neighbors

    nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=600)

    # Draw edges with saliency thickness
    ...
    plt.title(f"Neighbor Saliency for {node_names[target_idx]}")
    plt.axis('off')
    plt.tight_layout()
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✅ Saved saliency graph to {save_path}")
    else:
        plt.show()
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_(
    ...
    # Now build collapsed node structures
    source = []
    target = []
    value = []
    link_colors = []
    ...
    fig.update_layout(
        title_text="Collapsed Cluster-First Multi-Level Sankey: Confirmed Cluster → Gene → Shared Neighbor → Neighbor Cluster",
        font_size=10,
        width=1600,
        height=1000
    )
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_(
    ...
    # ====== 🔥 Filter confirmed_genes to Top 20 by scores 🔥 ======
    confirmed_genes = sorted(
        confirmed_genes,
        key=lambda g: scores[name_to_index[g]],
        reverse=True
    )[:10]  # <-- Only top 20 confirmed genes!
    ...
    # === Build cluster-to-gene and gene-to-neighbor mapping ===
    for gene in confirmed_genes:
        if gene not in topk_name_to_index:
            continue
        ...
        if neighbor_scores:
            neighbor_scores = dict(sorted(neighbor_scores.items(), key=lambda x: -x[1])[:5])  # Top 5 neighbors
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            if gene not in label_to_idx:
                ...
                color = saliency_to_color(saliency, min_saliency=0.0, max_saliency=1.0)
    ...
    # Add vertical colorbar for saliency
    gradient_y = np.linspace(0.3, 0.7, 50)
    gradient_colors = [saliency_to_color(y, min_saliency=0.3, max_saliency=0.7) for y in gradient_y]
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        marker=dict(
            size=0.1,  # Invisible, just to trigger colorbar
            color=[0.3, 0.7],
            colorscale=[
                [0.0, "#f1f1f1"],  # Low saliency color
                [1.0, "#ff0000"]   # High saliency color
            ],
            cmin=0.3,
            cmax=0.7,
            colorbar=dict(
...
def plot_collapsed_clusterfirst_multilevel_sankey_topo_(
    args,
    graph,
    node_names,
    name_to_index,
    ...
    # === Map node names and IDs ===
    topk_name_to_index = {name: i for i, name in enumerate(node_names)}
    node_id_to_name = {i: name for i, name in enumerate(node_names)}

    # === 🔥 Filter top 10 confirmed genes by scores ===
    confirmed_genes = sorted(
        confirmed_genes,
        key=lambda g: scores[name_to_index[g]],
        reverse=True
    ...
    for gene in confirmed_genes:
        ...
        node_idx = topk_name_to_index[gene]
        gene_cluster = graph.ndata["cluster_topo"][node_idx].item()  # 🔥 topo cluster!

        cluster_label = f"Confirmed Topo Cluster {gene_cluster}"
        if cluster_label not in cluster_to_genes:
            cluster_to_genes[cluster_label] = []
        cluster_to_genes[cluster_label].append(gene)
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes:
            ...
            for neighbor_idx, neighbor_score in neighbors.items():
                neighbor_name = node_id_to_name[neighbor_idx]
                neighbor_cluster = graph.ndata["cluster_topo"][neighbor_idx].item()
                neighbor_cluster_label = f"Topo Cluster {neighbor_cluster}"
    ...
    fig.update_layout(
        title_text="Collapsed Cluster-First Multi-Level Sankey: Confirmed Topo Cluster → Gene → Shared Neighbor → Neighbor Topo Cluster",
        font_size=12,
        margin=dict(l=200, r=200, t=100, b=100),
        width=1200,
        height=1200,
    ...
    fig.write_html(save_path)
    print(f"✅ Collapsed Cluster-First Topo Sankey saved: {save_path}")

def plot_collapsed_clusterfirst_multilevel_sankey_topo_sorted(
    args, graph, node_names, name_to_index, confirmed_genes,
    scores, cluster_labels, total_clusters, relevance_scores
):
    import os
    import numpy as np
    import plotly.graph_objects as go
    ...
    confirmed_genes = sorted(confirmed_genes, key=lambda g: scores[name_to_index[g]], reverse=True)[:10]
    neighbors_dict = get_neighbors_gene_names(graph, node_names, name_to_index, confirmed_genes)

    cluster_to_genes, gene_to_neighbors, cluster_saliency = {}, {}, {}

    for gene in confirmed_genes:
        idx = topk_name_to_index.get(gene)
        if idx is None: continue

        cluster = graph.ndata['cluster_topo'][idx].item()
        cluster_label = f"Confirmed Cluster {cluster}"
        cluster_to_genes.setdefault(cluster_label, []).append(gene)

        neighbors = neighbors_dict.get(gene, [])
        neighbor_scores = {
            topk_name_to_index[n]: relevance_scores[topk_name_to_index[n]].sum().item()
            for n in neighbors if n in topk_name_to_index
        }
        gene_to_neighbors[gene] = dict(sorted(neighbor_scores.items(), key=lambda x: -x[1])[:5])

    # === Sort clusters by avg saliency
    for cluster_label, genes in cluster_to_genes.items():
        avg_saliency = np.mean([relevance_scores[topk_name_to_index[g]].sum().item() for g in genes])
        cluster_saliency[cluster_label] = avg_saliency
    sorted_clusters = sorted(cluster_to_genes.items(), key=lambda x: -cluster_saliency[x[0]])

    # === Prepare Sankey
    label_to_idx, all_labels, all_colors, font_sizes = {}, [], [], []
    source, target, value, link_colors = [], [], [], []

    def add_node(label, color, font_size=14):
        if label not in label_to_idx:
            label_to_idx[label] = len(all_labels)
            all_labels.append(label)
            all_colors.append(color)
            font_sizes.append(font_size)
        return label_to_idx[label]

    for cluster_label, genes in sorted_clusters:
        cluster_idx = add_node(cluster_label, CLUSTER_COLORS.get(int(cluster_label.split()[-1]), "#000000"))

        # Sort genes inside cluster by their saliency
        genes_sorted = sorted(genes, key=lambda g: -relevance_scores[topk_name_to_index[g]].sum().item())

        for gene in genes_sorted:
            gene_saliency = relevance_scores[topk_name_to_index[gene]].sum().item()
            gene_idx = add_node(
                gene,
                saliency_to_color(gene_saliency, 0.0, 1.0),
                font_size=18 if gene_saliency > 0.5 else 10
            )
            ...
            for neighbor_idx, neighbor_score in gene_to_neighbors.get(gene, {}).items():
                ...
                neighbor_node_idx = add_node(
                    neighbor_name,
                    saliency_to_color(relevance_scores[neighbor_idx].sum().item(), 0.0, 1.0),
                    font_size=16 if relevance_scores[neighbor_idx].sum().item() > 0.5 else 10
                )
                neighbor_cluster_idx = add_node(
                    neighbor_cluster_label,
                    CLUSTER_COLORS.get(neighbor_cluster, "#000000")
                )
    ...
    fig = go.Figure(data=[go.Sankey(
        arrangement="snap",
        node=dict(pad=30, thickness=30, line=dict(color="black", width=0.5),
                  label=all_labels, color=all_colors),
        link=dict(source=source, target=target, value=value, color=link_colors)
    )])

    fig.update_layout(
        title="Collapsed Cluster-First Multilevel Sankey (Topo, Sorted)",
        font_size=12,
        margin=dict(l=200, r=200, t=100, b=100),
        width=1200,
        height=1200,
    ...
    # Gradient colorbar
    gradient_y = np.linspace(0.3, 0.7, 50)
    gradient_colors = [saliency_to_color(y, 0.3, 0.7) for y in gradient_y]
    fig.add_trace(go.Scatter(x=[1.05]*len(gradient_y), y=gradient_y, mode="markers",
                             marker=dict(size=10, color=gradient_colors), showlegend=False))
    fig.add_annotation(x=1.07, y=0.7, text="High Saliency", showarrow=False, font=dict(size=12))
    fig.add_annotation(x=1.07, y=0.3, text="Low Saliency", showarrow=False, font=dict(size=12))

    # Save
    out_dir = "results/gene_prediction/topo_collapsed_clusterfirst_multilevel_sankey/"
    os.makedirs(out_dir, exist_ok=True)
    save_path = os.path.join(
        out_dir, f"{args.model_type}_{args.net_type}_topo_collapsed_clusterfirst_multilevel_sankey_sorted_epo{args.num_epochs}.html"
    )
    fig.write_html(save_path)
    print(f"✅ Sorted Collapsed Topo Cluster-First Multilevel Sankey saved: {save_path}")
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_(
    ...
    CLUSTER_COLORS = {i: f"#{np.random.randint(0, 0xFFFFFF):06x}" for i in range(total_clusters)}

    # Map node names to indices
    topk_name_to_index = {name: i for i, name in enumerate(node_names)}
    node_id_to_name = {i: name for i, name in enumerate(node_names)}

    # ====== 🔥 Filter confirmed_genes to Top 10 by scores 🔥 ======
    confirmed_genes = sorted(
        confirmed_genes,
        key=lambda g: scores[name_to_index[g]],
        reverse=True
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            if gene not in label_to_idx:
                ...
                rel_idx = topk_name_to_index[gene]
                saliency = relevance_scores[rel_idx].sum().item()
                color = saliency_to_color(saliency)
                all_colors.append(color)
...
def plot_collapsed_clusterfirst_multilevel_sankey_topo(
    ...
    # === Build cluster-to-gene and gene-to-neighbor mappings ===
    for gene in confirmed_genes:
        if gene not in topk_name_to_index:
            continue
    ...
    for cluster_label, genes in cluster_to_genes.items():
        ...
        for gene in genes_sorted:
            if gene not in label_to_idx:
                ...
                font_sizes.append(18)  # You can keep font size based on saliency if you want, or just use fixed size
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_pa(
    ...
    # === Filter confirmed genes to top 10 by scores
    confirmed_genes = sorted(
        confirmed_genes,
        key=lambda g: scores[name_to_index[g]],
        reverse=True
    ...
    if highlight_node_indices:
        # Very rough node positioning, Sankey doesn't expose exact node positions
        x_positions = []
        y_positions = []
...
def plot_collapsed_clusterfirst_multilevel_sankey_topo_pa(
    args,
    graph,
    node_names,
    name_to_index,
    ...
    # === Layout
    fig.update_layout(
        title_text="Collapsed Cluster-First Multi-Level Sankey: Confirmed Cluster → Gene → Shared Neighbor → Neighbor Cluster (Topo)",
        font_size=14,
        margin=dict(l=200, r=200, t=100, b=100),
        width=1200,
        height=1200,
...
def plot_collapsed_clusterfirst_multilevel_sankey_bio_entropy_bar(
    args,
    graph,
    node_names,
    name_to_index,
...
def plot_enriched_pathways_heatmap_(
    enrichment_results,
    output_dir,
    model_type,
    net_type,
...
def apply_full_spectral_biclustering_bio_(graph, summary_bio_features, node_names,
    ...
    reduced_embeddings = tsne.fit_transform(summary_bio_features)
    ...
    # ↓ Sorted matrix heatmap
    sorted_matrix = summary_bio_features[np.argsort(row_labels), :]
    if col_labels.shape[0] == sorted_matrix.shape[1]:
        sorted_matrix = sorted_matrix[:, np.argsort(col_labels)]
    else:
        print("⚠️ Column labels shape does not match feature shape — skipping column sort.")

    plt.figure(figsize=(16, 8))
    sns.heatmap(sorted_matrix, cmap="vlag", center=0)
    plt.title("Heatmap of Summary Bio Features Sorted by Spectral Biclusters")
    plt.xlabel("64 Summary Features (Clustered)")
    plt.ylabel("Genes (Clustered)")
    plt.tight_layout()
    os.makedirs(os.path.dirname(output_path_heatmap), exist_ok=True)
    plt.savefig(output_path_heatmap, dpi=300)
    plt.close()
    print(f"✅ Summary bio heatmap saved to {output_path_heatmap}")
...
def plot_top_gene_omics_ridges_by_cluster_v2(
    omics_data,             # dict of {omics_type: np.array [num_genes, 256]}
    node_names,             # list of gene names
    cluster_labels,         # array/list of cluster assignments
    total_clusters,         # int
    top_n=12,               # top genes per cluster
    ...
    cancer_feat_names = [f"{cancer}" for cancer in cancer_types]

    for omics in omics_types:
        print(f"Preparing ridge plots for omics type: {omics}")

        matrix = omics_data[omics]  # shape: [num_genes, 256]
        ...
        for cluster in range(total_clusters):
            ...
            for _, row in top_genes.iterrows():
                gene_name = row["Gene"]
                feature_values = row.iloc[:-3].values  # shape: (256,)

                for i, cancer in enumerate(cancer_types):
                    # each cancer has 16 features → take average or first value
                    start = i * 16
                    end = (i + 1) * 16
                    avg_value = feature_values[start:end].mean()

                    df_list.append({
                        "Gene": gene_name,
                        "Cancer": cancer,
                        "OmicsValue": avg_value,
                        "Cluster": f"clus{cluster + 1}"
                    })
        ...
        for idx, cluster in enumerate(clusters):
            ...
            ax.set_title(f"{omics} - {cluster} (top {top_n})")
            ax.set_xlabel("Avg Feature Value (per Cancer)")
            ax.set_ylabel("")
...
def plot_top_gene_ridge_all_clusters_four_omics_not_top12_genes(
    saliency_scores,
    feature_names,
    cluster_assignments,
    output_path,
    ...
    # Extract gene names (strip omics prefixes)
    df_long["Gene"] = df_long["Feature"].apply(lambda x: x.split("_", 1)[-1])
    
    # Get top N genes by mean saliency per cluster
    ...
    # Merge to filter only top genes
    df_filtered = df_long.merge(top_genes_per_cluster[["Cluster", "Gene"]], on=["Cluster", "Gene"])

    # Sort genes within each cluster for consistent display
    df_filtered["Gene"] = df_filtered["Gene"].astype("category")
...
def extract_summary_features_np_bio(bio_embeddings_np):
    """
    Extracts summary features from 1024-dimensional biological features.
    ...
    for o_idx in range(4):  # 4 omics
        for c_idx in range(16):  # 16 cancers
            base = o_idx * 16 * 16 + c_idx * 16
            group = bio_embeddings_np[:, base:base + 16]  # shape [num_nodes, 16]
            max_vals = group.max(axis=1, keepdims=True)
            summary_features.append(max_vals)

    return np.concatenate(summary_features, axis=1)  # shape [num_nodes, 64]

def plot_top_gene_ridge_from_bio_features__(
    ...
    for cluster in clusters:
        ...
        # Order genes for display (bottom to top in increasing order)
        ...
        # Plot ridge plot
        plt.figure(figsize=(10, 0.6 * top_n + 1))
        sns.set(style="white", rc={"axes.facecolor": (0, 0, 0, 0)})
        ...
        ax.set_xlabel("Feature Value (Max across 16 per Cancer)")
        ax.set_ylabel("")
        ax.set_title(f"Cluster {cluster}: Top {top_n} Genes", loc="left", fontsize=14, weight='bold')
        plt.tight_layout()
...
def plot_top_gene_ridge_from_bio_features_(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
    ...
    for cluster in clusters:
        ...
        # Order genes for better display
        gene_order = plot_df.groupby("Gene")["Value"].mean().sort_values().index
        plot_df["Gene"] = pd.Categorical(plot_df["Gene"], categories=gene_order, ordered=True)
        ...
        # Show gene names on the left
        g.set_titles(row_template="{row_name}")
        g.set(xlabel="Feature Value (Max across 16 per Cancer)", ylabel="")
        g.set(yticks=[])
        g.despine(bottom=True, left=True)
...
def plot_top_gene_ridge_from_bio_features_x(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
    ...
    for cluster in clusters:
        ...
        # Plot without legend or vertical lines
        g.map_dataframe(sns.kdeplot, x="Value", bw_adjust=0.6, fill=True, alpha=0.8, lw=0)
        g.map_dataframe(sns.kdeplot, x="Value", bw_adjust=0.6, color="black", lw=1)

        # Remove legend and extra lines
        if g._legend:
            g._legend.remove()

        # Set clean titles
        g.set_titles(template="{row_name}", size=10, loc="left")
        g.set(xlabel="Feature Value (Max across 16 per Cancer)", ylabel=None)
        g.set(yticks=[])

        # Despine and adjust spacing
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=0.3, left=0.25)  # Increase left padding
...
def plot_top_gene_ridge_from_bio_features_center_title(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
...
def plot_top_gene_ridge_from_bio_features_no_showing_up(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
    ...
    print("🔍 Extracting 64 summary features from 1024 bio features...")
    summary_features = extract_summary_features_np_bio(bio_embeddings_np)  # shape [num_genes, 64]
    ...
    for cluster in clusters:
        ...
        # Remove default titles inside plots
        g.set_titles("")

        # Add gene names as external labels
        for ax, gene in zip(g.axes.flat, gene_order):
            ax.set_ylabel(gene, rotation=0, ha='right', va='left', fontsize=12, labelpad=40)

        # Final formatting
        g.set(xlabel="Feature Value (Max across 16 per Cancer)", ylabel="")
        g.set(yticks=[])
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=0.3, left=0.35)  # left=0.25 allows space for gene names
...
def plot_top_gene_ridge_from_bio_features_no_x_axis(
    bio_embeddings_np,           # shape [num_genes, 1024]
    node_names,                # list of gene names
    cluster_labels,            # shape [num_genes]
    output_path, 
    ...
    for cluster in clusters:
        ...
        # Add gene names as external labels manually
        for ax, gene in zip(g.axes.flat, gene_order):
            ax.set_ylabel(gene, rotation=0, ha='left', va='center', fontsize=18, labelpad=60)

        # Final formatting
        g.set(xlabel="Feature Value (Summarized Bio Features)", ylabel="")
        g.set(yticks=[])
        g.despine(bottom=True, left=True)

        # Increase left margin to show labels
        g.fig.subplots_adjust(hspace=0.3, left=0.25)
...
def plot_top_gene_ridge_from_specific_omics_(
    ...
    cluster_labels,            # shape [num_genes]
    output_path,
    omics_target='mf',         # 'cna', 'ge', 'meth', or 'mf'
    cancer_target='BRCA',      # one of the 16 cancer types
    top_n=12
):
    import pandas as pd
    import seaborn as sns
    ...
    # 1. Extract 16 features for this omics-cancer pair
    features_16 = extract_specific_omics_cancer(bio_embeddings_np, omics_target, cancer_target)  # [num_genes, 16]

    # 2. Create output directory
    os.makedirs(output_path, exist_ok=True)

    # 3. Build DataFrame with gene and cluster info
    ...
    for cluster in clusters:
        ...
        # Add min/max padding for smooth plot shape
        ...
        # Extend padding (duplicated points) to force KDE drop to zero
        pad_df = []
        for gene in gene_order:
            # Add multiple copies to strengthen KDE tails at edges
            for _ in range(5):  # duplicate to give more weight
                pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmin - buffer})
                pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmax + buffer})
        plot_df = pd.concat([plot_df, pd.DataFrame(pad_df)], ignore_index=True)
        ...
        '''g.map(sns.kdeplot, "Value", bw_adjust=0.3, fill=True, alpha=0.8)
        g.map(sns.kdeplot, "Value", bw_adjust=0.3, color="black", lw=1)'''

        g.map(sns.kdeplot, "Value", bw_adjust=0.3, fill=True, alpha=0.8, cut=0, clip=(xmin - buffer, xmax + buffer))
        g.map(sns.kdeplot, "Value", bw_adjust=0.3, color="black", lw=1, cut=0, clip=(xmin - buffer, xmax + buffer))

        g.set_titles("")  # Remove inner titles
        g.set(xlim=(xmin, xmax), xlabel="Feature Value", ylabel="", yticks=[])

        # Set gene names manually on Y-axis
        for ax, gene in zip(g.axes.flat, gene_order):
            ax.set_ylabel(gene, rotation=0, ha='right', va='center', fontsize=12, labelpad=10)

        g.despine(bottom=True, left=True)
        #g.fig.subplots_adjust(hspace=0.3, left=0.3, top=0.93)
        #g.fig.subplots_adjust(hspace=0.3, left=0.7, right=0.65, top=0.93)
        g.fig.subplots_adjust(hspace=0.3, left=0.4, right=0.95, top=0.90)


        g.fig.suptitle(f"Cluster {cluster}", fontsize=18, x=0.5)

        #g.fig.suptitle(f"{omics_target.upper()}-{cancer_target} | Cluster {cluster}", fontsize=18)

        # Save plot
        out_path = os.path.join(output_path, f"{omics_target}_{cancer_target}_cluster{cluster}_top{top_n}_genes_ridge.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✅ Saved: {out_path}")

def plot_top_gene_ridge_from_specific_omics_not_zero(
    bio_embeddings_np,
    node_names,
    cluster_labels,
    output_path,
    ...
    cancer_target='BRCA',
    top_n=12,
    buffer=0.5  # extra padding for x-axis
):
    import pandas as pd
    import seaborn as sns
    import matplotlib.pyplot as plt
    ...
    for cluster in clusters:
        ...
        # Padding for min/max to prevent cutoff
        xmin = plot_df["Value"].min() - 3
        xmax = plot_df["Value"].max() + 3
        pad_df = []
        for gene in gene_order:
            pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmin - buffer})
            pad_df.append({"Gene": gene, "FeatureIndex": -1, "Value": xmax + buffer})
        ...
        g.set_titles("")
        g.set(xlim=(xmin - buffer, xmax + buffer), xlabel="Feature Value", ylabel="", yticks=[])
        ...
        g.fig.suptitle(f"Cluster {cluster}", fontsize=18)
...
def plot_top_gene_ridge_from_specific_omics_down_zero_not_align(
    bio_embeddings_np,
    node_names,
    cluster_labels,
    output_path,
...
def plot_top_gene_ridge_from_specific_omics_(
    ...
    cancer_target='BRCA',
    top_n=12,
    buffer=0.5  # padding to extend KDE curves
):
    import pandas as pd
    import seaborn as sns
    import matplotlib.pyplot as plt
    ...
    for cluster in clusters:
        ...
        # Set gene order by mean
        gene_order = plot_df.groupby("Gene")["Value"].mean().sort_values().index
        plot_df["Gene"] = pd.Categorical(plot_df["Gene"], categories=gene_order, ordered=True)

        # Calculate consistent x-range
        xmin = plot_df["Value"].min()
        xmax = plot_df["Value"].max()
        range_buffer = (xmax - xmin) * 0.5
        xmin -= range_buffer
        xmax += range_buffer

        # Add dummy min/max values to each gene to force same x-span in KDE
        pad_df = []
        for gene in gene_order:
            pad_df.extend([
                {"Gene": gene, "FeatureIndex": -1, "Value": xmin - buffer},
                {"Gene": gene, "FeatureIndex": -1, "Value": xmax + buffer}
            ])
        plot_df = pd.concat([plot_df, pd.DataFrame(pad_df)], ignore_index=True)
...
def plot_all_cancer_ridges_all_omics_(
    bio_embeddings_np,
    node_names,
    cluster_labels,
    output_base_path,
    ...
    for cancer in cancer_list:
        print(f"\n📊 Plotting ridge across ALL omics for {cancer}...")
        output_path = os.path.join(output_base_path, f"ALL_OMICS_{cancer}")
        plot_top_gene_ridge_across_omics(
            bio_embeddings_np,
            node_names,
            cluster_labels,
            output_path,
...
def plot_top_gene_ridge_across_omics_(
    ...
    omics_list = ['mf', 'cna', 'ge', 'meth']

    # Extract and concatenate 16 features per omics
    omics_features = []
    for omics in omics_list:
        feat = extract_specific_omics_cancer(bio_embeddings_np, omics, cancer_target)
        omics_features.append(feat)

    # Shape: (N, 64)
    features_64 = np.concatenate(omics_features, axis=1)
    ...
    for cluster in clusters:
        ...
        # Ridge plot by gene, color by omics type
        sns.set(style="white", rc={"axes.facecolor": (0, 0, 0, 0)})
        g = sns.FacetGrid(
            plot_df,
            row="Gene",
        ...
        g.add_legend(title="Omics Type")
        g.despine(bottom=True, left=True)
        g.fig.subplots_adjust(hspace=-0.3, left=0.3, right=0.95, top=0.93)
        g.fig.suptitle(f"{cancer_target} - Cluster {cluster} (All Omics)", x=0.6, fontsize=20)
...
def plot_top_gene_ridge_from_all_omics_(
    bio_embeddings_np,
    node_names,
    cluster_labels,
    output_path,
    ...
    # 1. Extract 64-dim features (4 omics × 16 features) for the selected cancer
    features_64 = extract_all_omics_for_cancer(bio_embeddings_np, cancer_target)
    ...
    for cluster in clusters:
        ...
        g.fig.subplots_adjust(hspace=-0.3, left=0.3, right=0.95, top=0.93)
        g.fig.suptitle(f"Cluster {cluster}", x=0.6, fontsize=20)
        ##g.fig.suptitle(f"Cluster {cluster} — All Omics — {cancer_target}", x=0.6, fontsize=20)
        ...
        out_path = os.path.join(
            output_path,
            f"ALL_OMICS_{cancer_target}_cluster{cluster}_top{top_n}_genes_ridge.png"
        )
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✅ Saved: {out_path}")
...
def plot_dot_enrichment_per_cluster_all_(cancer_types, source="REAC", top_n=10):
    """
    Plot dot enrichment plots for each cluster of each cancer type.

    Parameters:
        cancer_types (list): List of cancer type strings (e.g., ["ucec", "brca"]).
        source (str): Enrichment source, "REAC" or "KEGG".
        top_n (int): Number of top enriched pathways to plot.
    """
    for cancer_type in cancer_types:
        files = glob.glob(f"results/gene_prediction/{cancer_type.lower()}_*_{source}_enrichment.csv")
        if not files:
            print(f"No enrichment files found for {cancer_type.upper()} and source {source}")
            continue
        ...
        for f in files:
            try:
                cluster = Path(f).stem.split("_")[1]  # Assumes format: cancer_cluster_source_enrichment.csv
                cluster_files[cluster] = f
            except IndexError:
                print(f"Could not parse cluster from filename: {f}")
                continue

        for cluster_key, f in cluster_files.items():
            try:
                df = pd.read_csv(f)
            except Exception as e:
            ...
            if df.empty or "intersection_size" not in df or "query_size" not in df:
                print(f"Invalid or empty dataframe for {f}")
                continue
            ...
            plt.title(f"{source} Enrichment for {cancer_type.upper()} - {cluster_key}")
            plt.xlabel("Gene Ratio")
            plt.ylabel("Pathway")
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.tight_layout()

            out_path = f"results/gene_prediction/{cancer_type}_{cluster_key}_{source}_dotplot.png"
            plt.savefig(out_path, dpi=300)
            plt.show()
            print(f"Saved dot plot for {cancer_type} {cluster_key} at {out_path}")
...
def plot_dot_enrichment_per_cluster_all_(cancer_types, tag="bio", source="REAC", top_n=10, n_clusters=10):
    base_path = Path("results/gene_prediction")

    for cancer_type in cancer_types:
        print(f"\n== {cancer_type.upper()} [{tag}] ==")
        for cluster_id in range(n_clusters):
...
def draw_horizontal_bar_plot_(terms, cancer_type, source):
    if not terms:
    ...
    fig, ax = plt.subplots(figsize=(10, 0.5 * len(terms)))
    ...
    ax.set_title(f"Top Enriched Pathways — {cancer_type.upper()} ({source})", fontsize=14)
    plt.tight_layout()
...
def draw_horizontal_bar_plot_(terms, cancer_type, source):
    ...
    plt.subplots_adjust(top=0.92, bottom=0.08, left=0.25, right=0.95)

    # Add more x-ticks for better granularity
    max_score = max(scores)
    ax.set_xticks(np.linspace(0, max_score, num=10))  # 10 x-ticks from 0 to max
...
def collect_enrichment_with_ratios_no_limit_length_character(cancer_type, tag="bio", source="REAC", top_n=10):
    base_path = Path("results/gene_prediction")
    terms = []
...
def apply_full_spectral_biclustering_bio_(graph, summary_bio_features, node_names,
    ...
    # Save row cluster labels and stats
    save_cluster_labels(row_labels, save_cluster_labels_path)

    total_genes_per_cluster = {i: np.sum(row_labels == i) for i in range(n_clusters)}
    save_total_genes_per_cluster(total_genes_per_cluster, save_total_genes_per_cluster_path)

    # Count predicted genes per cluster
    pred_counts = {i: 0 for i in range(n_clusters)}
    name_to_index = {name: idx for idx, name in enumerate(node_names)}
    predicted_indices = [name_to_index[name] for name in predicted_cancer_genes if name in name_to_index]
    for idx in predicted_indices:
    ...
    # ↓ Sorted matrix heatmap
    ##sorted_matrix = summary_bio_features[np.argsort(row_labels), :]
    # ↓ Filter top-k predicted genes for heatmap
    topk = 1000  # You can make this a parameter if needed
    name_to_index = {name: idx for idx, name in enumerate(node_names)}
    predicted_indices = [name_to_index[name] for name in predicted_cancer_genes if name in name_to_index]
    predicted_indices = predicted_indices[:topk]

    # Subset data for top-k only
    summary_topk = summary_bio_features[predicted_indices]
    row_labels_topk = row_labels[predicted_indices]

    # Sort by cluster labels for heatmap
    ...
    if col_labels.shape[0] == sorted_matrix.shape[1]:
        ...
        print("⚠️ Column labels shape mismatch — skipping column sort.")
    ...
    # === Fixed cluster color map (cluster ID -> hex color)
    '''CLUSTER_COLORS = {
        0: '#0077B6',  1: '#0000FF',  2: '#00B4D8',  3: '#48EAC4',
        4: '#F1C0E8',  5: '#B9FBC0',  6: '#32CD32',  7: '#bee1e6',
        8: '#8A2BE2',  9: '#E377C2'
    ...
    # === Prepare color bar for sorted clusters
    sorted_labels = row_labels_topk[sorted_indices]  # shape: (num_genes,)
    cluster_colors = [to_rgb(CLUSTER_COLORS[label]) for label in sorted_labels]  # RGB format for imshow
    cluster_colors_array = np.array(cluster_colors).reshape(-1, 1, 3)

    # === Count genes per cluster (for label annotation)
    cluster_counts = Counter(sorted_labels)

    # === Create the figure with grid spec layout
    fig = plt.figure(figsize=(18, 12))
    gs = fig.add_gridspec(nrows=1, ncols=3, width_ratios=[1, 0.2, 13.8])  # ✅ Fixed here
    ax_cluster = fig.add_subplot(gs[0, 0])
    ax_gap = fig.add_subplot(gs[0, 1])
    ax_heatmap = fig.add_subplot(gs[0, 2])

    # === Plot cluster color bar
    ...
    # Annotate number of genes per cluster on the color bar
    start_idx = 0
    for cluster_id in sorted(cluster_counts):
        count = cluster_counts[cluster_id]
        mid_idx = start_idx + count // 2
        ax_cluster.text(0.5, mid_idx, f'{count}', va='center', ha='center', fontsize=8, color='black')
        start_idx += count

    # === Gap axis (invisible spacing)
    ax_gap.axis("off")
    ...
    ax_heatmap.set_title("Heatmap of Summary Bio Features Sorted by Spectral Biclusters", fontsize=14)
    ax_heatmap.set_xlabel("64 Summary Features (Clustered)", fontsize=12)
    #ax_heatmap.set_ylabel("Genes (Clustered)", fontsize=12)
    ax_heatmap.set_yticks([])  # ✅ This removes y-axis tick labels
    ...
    # Grouped xtick labels (omics-wise)
    omics_group_positions = {
        omics: (omics_splits[omics][0] + omics_splits[omics][1]) / 2 + 0.5
        for omics in omics_order
    }

    # Add omics labels under the x-axis ticks
    for omics, x_pos in omics_group_positions.items():
        ax_heatmap.text(
            x_pos,
            -6,  # lower than tick labels
            omics.upper(),
            ha='center',
            va='center',
            fontsize=16,
            fontweight='bold',
            color=omics_colors.get(omics, 'black'),
            rotation=0,
            clip_on=False
        )
    ...
    os.makedirs(os.path.dirname(output_path_heatmap), exist_ok=True)
    fig.tight_layout()
    fig.savefig(output_path_heatmap, dpi=300)
    plt.close(fig)
    print(f"✅ Summary bio heatmap with fixed cluster patches saved to {output_path_heatmap}")

    # ↪️ Return row_labels for downstream cluster-based plots
    return graph, row_labels, col_labels, total_genes_per_cluster, pred_counts


def eigengap_analysis_grid(feature_matrix, max_clusters=20, normalize=True, plot_path=None):
    # Step 1: Normalize features (optional)
    '''if normalize:
        from sklearn.preprocessing import StandardScaler
        feature_matrix = StandardScaler().fit_transform(feature_matrix)'''

    # Step 2: Similarity matrix (cosine similarity or RBF kernel)
    ...
    # Step 5: Plot eigengap
    '''plt.figure(figsize=(8, 5))
    plt.plot(range(1, max_clusters + 1), eigenvals[1:max_clusters + 1], marker='o')
    gaps = np.diff(eigenvals[1:max_clusters + 1])
    best_k = np.argmax(gaps) + 1  # +1 because diff shifts index
    plt.axvline(x=best_k, color='r', linestyle='--', label=f'Eigengap → k={best_k}')
    ...
    if plot_path:
        ...
        gaps = np.diff(eigenvals[1:max_clusters + 1])
        best_k = np.argmax(gaps) + 1  # +1 because diff shifts index
        plt.axvline(x=best_k, color='red', linestyle='--', label=f'Eigengap → k={best_k}')
        plt.xlabel("Eigenvalue Index")
        plt.ylabel("Eigenvalue")
        plt.title("Eigengap Analysis")
        plt.grid(True)
...
def eigengap_analysis(feature_matrix, max_clusters=20, normalize=True, plot_path=None):
    # Step 1: Normalize features (optional)
    '''if normalize:
        from sklearn.preprocessing import StandardScaler
        feature_matrix = StandardScaler().fit_transform(feature_matrix)'''
...
def plot_topo_biclustering_heatmap(
    ...
    relevance_scores = extract_summary_features_np_topo(relevance_scores)
    # Normalize features-----------------------------------------------------------------------------------------------------------------
    relevance_scores = StandardScaler().fit_transform(relevance_scores)
...
def plot_bio_clusterwise_feature_contributions_(
    args,
    relevance_scores,           # 2D array (samples x features)
    cluster_labels,             # 1D array of cluster assignments
    feature_names,              # List of feature names (e.g., MF: BRCA, ...)
    ...
    def get_omics_color(feature_name):
        return omics_colors.get(get_omics_type(feature_name), "#AAAAAA")

    # Step 1: Group features by omics type
    omics_grouped = defaultdict(list)
    for i, name in enumerate(feature_names):
        omics_type = get_omics_type(name)
        omics_grouped[omics_type].append((i, name))  # store original index + name

    # Step 2: Sort by omics type order
    ordered_omics = list(omics_colors.keys())  # follow given color order
    ordered_features = []
    for omics in ordered_omics:
        ordered_features.extend(omics_grouped.get(omics, []))
    # Flatten: indices and names reordered
    ordered_indices = [idx for idx, _ in ordered_features]
    ordered_names = [name for _, name in ordered_features]
    ...
    for cluster_id in sorted(unique_clusters):
        ...
        # Reorder contributions and colors
        reordered_contribution = avg_contribution[ordered_indices]
        colors = [get_omics_color(name) for name in ordered_names]

        fig, ax = plt.subplots(figsize=(12, 2.7))

        x = np.arange(len(ordered_names))
        bars = ax.bar(x, reordered_contribution, color=colors, width=0.95)

        # Omics label cleaning
        clean_labels = [name.split(":")[1] if ":" in name else name for name in ordered_names]
        ax.set_xticks(x)
        ax.set_xticklabels(clean_labels, rotation=90)
        for label, name in zip(ax.get_xticklabels(), ordered_names):
            label.set_color(get_omics_color(name))
        ...
        # Optional: Add vertical lines between omics groups
        '''current_pos = 0
        for omics in ordered_omics:
            if omics in omics_grouped:
                group_size = len(omics_grouped[omics])
                ax.axvline(x=current_pos - 0.5, color='gray', linestyle=':', linewidth=0.7)
...
def apply_full_spectral_biclustering_bio_topk(graph, summary_bio_features, node_names,
                                         predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap):
...
def apply_full_spectral_biclustering_bio_all_genes(graph, summary_bio_features, node_names,
                                         predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap):
...
def apply_full_spectral_biclustering_bio(graph, summary_bio_features, node_names,
                                         predicted_cancer_genes, n_clusters,
                                         save_path, save_cluster_labels_path,
                                         save_total_genes_per_cluster_path, save_predicted_counts_path,
                                         output_path_genes_clusters, output_path_heatmap):
    ...
    # === Heatmap of Top-k Predicted Genes
    topk = 1000
    predicted_indices = predicted_indices[:topk]
    summary_topk = summary_bio_features[predicted_indices]
    row_labels_topk = row_labels[predicted_indices]
    ...
    # === Sort columns within each omics group based on col_labels
    ...
    name_to_col_idx = {name: i for i, name in enumerate(bio_feat_names_64)}

    # Compute contribution per feature
    feature_contributions = {
        name: np.sum(np.abs(summary_bio_features[:, idx]))
        for name, idx in name_to_col_idx.items()
    }

    # Group features by omics and compute total per group
    omics_groups = {'cna': [], 'ge': [], 'meth': [], 'mf': []}
    for name in bio_feat_names_64:
        omics = name.split('_')[0]
        omics_groups[omics].append(name)

    # Sort omics groups by total contribution
    omics_contrib = {
        omics: sum(feature_contributions[name] for name in feats)
        for omics, feats in omics_groups.items()
    }
    omics_list = sorted(omics_contrib, key=omics_contrib.get, reverse=True)
    print(f"🔍 Omics groups sorted by contribution: {omics_list}")

    # Sort features within each omics group by contribution
    sorted_columns = []
    sorted_names = []
    for omics in omics_list:
        feats = omics_groups[omics]
        sorted_feats = sorted(feats, key=lambda name: feature_contributions[name], reverse=True)
        sorted_columns.extend([name_to_col_idx[name] for name in sorted_feats])
        sorted_names.extend(sorted_feats)

    # Reorder matrix columns
    sorted_matrix = sorted_matrix[:, sorted_columns]

    fig = plt.figure(figsize=(18, 12))
    gs = fig.add_gridspec(nrows=2, ncols=4, height_ratios=[0.5, 10], width_ratios=[1, 0.2, 13.3, 1.5])

    # Axes definitions
    ax_cluster = fig.add_subplot(gs[1, 0])      # left color bar (clusters)
    ax_gap = fig.add_subplot(gs[1, 1])          # gap
    ax_heatmap = fig.add_subplot(gs[1, 2])      # heatmap main area
    ax_contrib = fig.add_subplot(gs[1, 3])      # right-side per-gene density
    ax_feat_contrib = fig.add_subplot(gs[0, 2]) # top feature contribution bar

    # === Cluster color bar (left)
    cluster_colors_array = np.array([to_rgb(CLUSTER_COLORS[label]) for label in sorted_labels]).reshape(-1, 1, 3)
    ax_cluster.imshow(cluster_colors_array, aspect='auto')
    ax_cluster.set_xticks([])
    ax_cluster.set_yticks([])
    ax_cluster.set_title("Cluster", fontsize=10)

    # Annotate cluster counts
    start_idx = 0
    for cluster_id in sorted(cluster_counts):
        count = cluster_counts[cluster_id]
        mid_idx = start_idx + count // 2
    ...
    bluish_gray_gradient = LinearSegmentedColormap.from_list("bluish_gray_gradient", ["#F0F3F4", "#85929e"])
    sns.heatmap(sorted_matrix, cmap=bluish_gray_gradient, center=0, vmin=-2, vmax=2,
                ax=ax_heatmap, cbar_kws={"shrink": 0.5})
    ax_heatmap.set_title("Heatmap of Summary Bio Features (Omics-wise Column Sort)", fontsize=14)
    ax_heatmap.set_yticks([])

    xticks = np.arange(len(sorted_names)) + 0.5
    ax_heatmap.set_xticks(xticks)
    ax_heatmap.set_xticklabels(sorted_names, rotation=90, fontsize=7)
    for tick_label, name in zip(ax_heatmap.get_xticklabels(), sorted_names):
        omics = name.split('_')[0]
        tick_label.set_color(omics_colors.get(omics, 'black'))

    ax_heatmap.set_xlabel("Summary Bio Features (Omics-wise Column Sort)", fontsize=12)

    # === Top feature contribution bar (per column)
    feature_contrib_values = np.mean(np.abs(sorted_matrix), axis=0)
    feature_contrib_values = feature_contrib_values / np.max(feature_contrib_values) if np.max(feature_contrib_values) > 0 else feature_contrib_values

    for i, (val, name) in enumerate(zip(feature_contrib_values, sorted_names)):
        omics = name.split('_')[0]
        color = omics_colors.get(omics, 'gray')
        ax_feat_contrib.bar(i, val, color=color, edgecolor='none', width=1.0)

    ax_feat_contrib.set_xlim(-0.5, len(sorted_names) - 0.5)
    ax_feat_contrib.set_xticks([])
    ax_feat_contrib.set_yticks([])
    ax_feat_contrib.set_title("Feature\nContribution", fontsize=10)

    # === Right-side predicted gene contribution (per row)
    gene_contributions = np.mean(np.abs(sorted_matrix), axis=1)
    gene_contributions = gene_contributions / np.max(gene_contributions) if np.max(gene_contributions) > 0 else gene_contributions
    from scipy.ndimage import gaussian_filter1d
    smooth_curve = gaussian_filter1d(gene_contributions, sigma=2)

    y = np.arange(len(smooth_curve))
    ax_contrib.fill_betweenx(y, 0, smooth_curve, color='gray', alpha=0.5)
    ax_contrib.set_xlim(0, 1.05)
    ax_contrib.set_xticks([])
    ax_contrib.set_yticks([])
    ax_contrib.set_title("Predicted\nGene Density", fontsize=10)
    ax_contrib.invert_yaxis()  # align with heatmap
    ...
    print(f"✅ Summary bio heatmap with *omics-wise sorted columns* saved to {output_path_heatmap}")
...
def get_sorted_feature_indices_by_omics_contribution(summary_bio_features, omics_order, cancer_names):
    feature_names = [f"{omics}_{cancer}" for omics in omics_order for cancer in cancer_names]
    name_to_col_idx = {name: i for i, name in enumerate(feature_names)}

    # Total contribution per feature (e.g., LRP sum or abs sum)
    feature_contributions = {
        name: np.sum(np.abs(summary_bio_features[:, idx]))
        for name, idx in name_to_col_idx.items()
    }
    ...
    omics_groups = {omics: [] for omics in omics_order}
    for name in feature_names:
        omics = name.split('_')[0]
        omics_groups[omics].append(name)

    # Sort omics by total contribution
    omics_contrib = {
        omics: sum(feature_contributions[name] for name in feats)
        for omics, feats in omics_groups.items()
    }
    sorted_omics = sorted(omics_contrib, key=omics_contrib.get, reverse=True)
    ...
    return sorted_columns, sorted_names, sorted_omics

def get_row_sort_order_by_cluster_and_relevance(cluster_labels, relevance_matrix):
    sorted_indices = np.argsort(cluster_labels)
    cluster_labels = cluster_labels[sorted_indices]
    relevance_matrix = relevance_matrix[sorted_indices]

    new_order = []
    for cluster in np.unique(cluster_labels):
        cluster_idxs = np.where(cluster_labels == cluster)[0]
        relevance_sums = relevance_matrix[cluster_idxs].sum(axis=1)
        within_sorted = cluster_idxs[np.argsort(-relevance_sums)]
        new_order.extend(within_sorted)
...
def plot_bio_biclustering_heatmap_(
    ...
    for cluster in unique_clusters:
        cluster_indices = np.where(cluster_labels == cluster)[0]
        cluster_relevance_sums = relevance_scores[cluster_indices].sum(axis=1)
        sorted_within = cluster_indices[np.argsort(-cluster_relevance_sums)]
        new_order.extend(sorted_within)
    ...
    for omics in sorted_omics:
        ...
        group_indices = list(range(start, end + 1))
        col_means = sorted_scores[:, group_indices].mean(axis=0)
        group_sorted_indices = [group_indices[i] for i in np.argsort(-col_means)]
        sorted_col_indices.extend(group_sorted_indices)
        sorted_feature_names.extend([feature_names[i] for i in group_sorted_indices])
        feature_colors.extend([omics_colors[omics]] * len(group_sorted_indices))
    ...
    vmin = np.percentile(sorted_scores, 1)   # Lower bound
    vmax = np.percentile(sorted_scores, 99) # Upper bound vmin=-2, vmax=2,
    ...
    for cluster_id, center_y, count in zip(unique_clusters, cluster_centers, counts):
        ax.text(-2.0, center_y, f"{count}", va='center', ha='right', fontsize=18, fontweight='bold')
    ...
    ax_curve.fill_betweenx(np.arange(len(sorted_scores)), 0, (sorted_scores.sum(axis=1) - sorted_scores.sum(axis=1).min()) / (sorted_scores.sum(axis=1).max() - sorted_scores.sum(axis=1).min()), color='#7b241c', alpha=0.8, linewidth=3)
    ax_curve.set_xticks([0, 1])
    ax_curve.set_xticklabels(['0', '1'], fontsize=16)
    ax_curve.tick_params(axis='x', direction='out', pad=1)
    ax_curve.hlines(y=1.01, xmin=0, xmax=1, color='black', linewidth=1.5, transform=ax_curve.get_xaxis_transform())
    ax_curve.set_ylim(0, len(sorted_scores))
    ax_curve.axis('off')

    group_centers = {omics: (omics_splits[omics][0] + omics_splits[omics][1]) / 2 + 0.5 for omics in omics_order}
    mean_vals = np.array([omics_means[om] for om in omics_order])
    min_mean, max_mean = mean_vals.min(), mean_vals.max()
...
def train(args):
    ...
    best_k=10#, _ = eigengap_analysis(top_node_features, max_clusters=20, normalize=True, plot_path=plot_eigengap_path)
    ...
    graph, cluster_labels_topo, col_labels_topo, topo_total_counts, topo_pred_counts = apply_full_spectral_biclustering_topo(
        ...
        topo_embeddings=torch.tensor(summary_topo_features),
        node_names=node_names,
        #feature_names=None,
        predicted_cancer_genes=node_names_topk,
        n_clusters=best_k,
    ...
    ##relevance_scores = compute_node_saliency(model, graph, features)
    ##relevance_scores = compute_combined_relevance_scores(model, graph, features, use_abs=True, normalize=False)
    ...
    for cluster_type, cluster_labels, relevance_scores_subset, tag in [
        ...
        if tag == "bio":
            ...
            #plot_all_omics_cancer_ridges(
            #plot_top_gene_ridge_across_omics(


            cluster_dict = cluster_to_genes  # from earlier code
            
            ##plot_dot_enrichment_per_cluster_all(cancer_types, source="REAC", top_n=20)
            
            ##plot_dot_enrichment_per_cluster_all(cancer_types, tag="bio", source="REAC", top_n=10, n_clusters=10)
            ...
            for cancer_type in cancer_types:
                ...
                #terms = collect_top_enrichments(cancer_type=cancer_type, tag="bio", source="REAC", top_n=20)
                #draw_horizontal_bar_plot(terms, cancer_type=cancer_type, source="REAC")
            ...
            '''plot_top_gene_ridge_from_bio_features(
                bio_embeddings_np=bio_embeddings_np,
                node_names=node_names,
                cluster_labels=cluster_labels,
                output_path=ridge_from_bio_path,
                top_n=12
            )   '''         
            ...
            sankey_stats = analyze_sankey_structure(
                    ...
                    gene_to_neighbors=gene_to_neighbors,
                    cluster_labels=cluster_labels,
                ...
                entropy_df = pd.DataFrame(
                    list(sankey_stats["cluster_entropy"].items()),
                    columns=["Cluster", "Entropy"]
                )
                                
                # === Save CSV: Entropy per Cluster
                entropy_df.to_csv(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_entropy_epo{args.num_epochs}.csv"), index=False)

                # === Save CSV: Jaccard Similarity Matrix
                jaccard_df.to_csv(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_jaccard_epo{args.num_epochs}.csv"))

                # === Save CSV: Gene Centrality
                centrality_df.to_csv(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_centrality_epo{args.num_epochs}.csv"), index=False)

                # === Save CSV: Cluster Participation Count
                participation_df = pd.DataFrame(
                    list(sankey_stats["cluster_participation"].items()),
                    columns=["Gene", "Num_Clusters"]
                ).sort_values("Num_Clusters", ascending=False)

                participation_df.to_csv(os.path.join(output_dir, f"{args.model_type}_{args.net_type}_participation_epo{args.num_epochs}.csv"), index=False)
                '''
    ...
    def collect_top_enrichments(cancer_type, tag="bio", source="REAC", top_n=10):
        ...
        return sorted(terms, key=lambda x: x[1], reverse=True)  # sort by score


    def draw_lollipop_plot(terms, cancer_type, source):
        if not terms:
            print(f"No enrichment terms for {cancer_type.upper()} ({source})")
            return
        ...
        ax.hlines(y=y_pos, xmin=0, xmax=scores, color='skyblue')
        scatter = ax.scatter(scores, y_pos, c=clusters, cmap='tab10', s=100, edgecolor='black')
        ...
        out_path = Path(f"results/gene_prediction/{cancer_type}_{source}_lollipop_plot.png")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(out_path, dpi=300)
        plt.close()


    # 🔁 Generate lollipop plots for BRCA & UCEC
    for cancer in ["brca", "ucec"]:
        for source in ["REAC", "KEGG"]:
            terms = collect_top_enrichments(cancer_type=cancer, tag="bio", source=source, top_n=10)
            draw_lollipop_plot(terms, cancer_type=cancer, source=source)
    ...
    def build_enrichment_network(cancer_type, tag="bio", source="REAC", top_n=10, jaccard_threshold=0.25):
        base_path = Path("results/gene_prediction")
        G = nx.Graph()
        node_data = []
        
        for cluster_id in range(10):  # assuming clusters are Cluster_0 to Cluster_9
            file = base_path / f"{cancer_type}_{tag}_Cluster_{cluster_id}_{source}_enrichment.csv"
            if not file.exists():
                continue
            ...
            for _, row in df.iterrows():
                term = f"{row['name']} (C{cluster_id})"
                genes = set(str(row['intersection_size']).split(',')) if pd.notnull(row['intersection_size']) else set()
                G.add_node(term, cluster=cluster_id, score=-np.log10(row["p_value"] + 1e-10), genes=genes)
                node_data.append((term, genes))

        for (term1, genes1), (term2, genes2) in combinations(node_data, 2):
            sim = jaccard_similarity(genes1, genes2)
            if sim >= jaccard_threshold:
                G.add_edge(term1, term2, weight=sim)

        return G

    def draw_enrichment_network(G, cancer_type, source="REAC"):
        plt.figure(figsize=(14, 12))
        pos = nx.spring_layout(G, seed=42)
        node_colors = [data["cluster"] for _, data in G.nodes(data=True)]
        node_sizes = [data["score"] * 50 for _, data in G.nodes(data=True)]

        nx.draw_networkx_nodes(G, pos, node_size=node_sizes, node_color=node_colors, cmap=plt.cm.tab10)
        nx.draw_networkx_edges(G, pos, alpha=0.3)
        nx.draw_networkx_labels(G, pos, font_size=8)

        plt.title(f"Enrichment Map — {cancer_type.upper()} ({source})", fontsize=16)
        plt.axis("off")
        plt.tight_layout()
        out_path = Path(f"results/gene_prediction/{cancer_type}_{source}_enrichment_network.png")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(out_path, dpi=300)
        plt.close()

    # 🧬 Build and draw enrichment networks
    for cancer in ["brca", "ucec"]:
        for source in ["REAC", "KEGG"]:
            G = build_enrichment_network(cancer_type=cancer, tag="bio", source=source, top_n=10, jaccard_threshold=0.25)
            if len(G) > 0:
                draw_enrichment_network(G, cancer_type=cancer, source=source)
'''
    '''metrics = analyze_sankey_structure(
        source, target, value, label_to_idx, node_names,
        cluster_to_genes, gene_to_neighbors, cluster_labels,
        name_to_index, scores
    )

    save_metrics_to_csv(metrics)


    print(metrics["gene_degree_centrality"])
    print(metrics["gene_cluster_counts"])
    print(metrics["cluster_entropy"])
    print(metrics["cluster_jaccard_similarity"])'''
    ...
    #plot_dot_enrichment_from_results(enrichment_results, top_n=15)
    ...
    # Layout adjustments: more top and right space
    sns.despine(ax=ax, trim=True)
    ax.tick_params(axis='both', which='both', length=0)
    plt.tight_layout(rect=[0, 0, 0.95, 0.93])  # Add more top and right margin
...
def plot_lrp_cluster_heatmap_ori(
    ...
    # Limit number of neighbors plotted
    MAX_NEIGHBORS = 10  # or 200 if you want a bigger heatmap

    if heatmap_data_norm.shape[0] > MAX_NEIGHBORS:
        # Compute importance per neighbor (e.g., mean absolute relevance across features)
        neighbor_importance = heatmap_data_norm.abs().mean(axis=1)

        # Sort neighbors by importance, descending
        top_indices = neighbor_importance.sort_values(ascending=False).index[:MAX_NEIGHBORS]

        # Select only the top neighbors for both data and labels
        heatmap_data_norm = heatmap_data_norm.loc[top_indices]
        heatmap_labels = [heatmap_labels[i] for i in top_indices]
